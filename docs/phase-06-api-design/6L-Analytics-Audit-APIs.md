...
**Controlled Cross-Phase Amendment (Phase 6M, 2026-09-03) — status update, original five-item spec above left intact.** Item 4 is now **delivered**: `105_5B4.sql` adds a `purposes TEXT[]` column to `organization.break_glass_grants` with `SENSITIVE_MEDIA_ACCESS` as one of six allow-listed values (`chk_bgg_purposes_allowed`), wildcard/superuser purposes rejected at issuance (`chk_bgg_purposes_no_wildcard`), and `purposes` immutable post-insert — precisely the purpose marker this item called for. The "currently an interim Redis record, DEP-6B-01" clause is also stale: DEP-6B-01 is RESOLVED (Phase 5L, 2026-08-24; extended Phase 6M) — `organization.break_glass_grants` is a durable table (6B §18.3b). Items 1, 2, 3, and 5 are unchanged and remain the binding contract; item 5's `RECORDING_ACCESS_GRANTED` ‖ audit marker is untouched by this pass's 5J §14.3 vocabulary amendment (which added only four billing/quota/refund values, ✦ marker — see 5J §14.3 footnote). **This status update does not itself add a platform-admin recording/transcript route** — whether one is built, and any accompanying owner decision on tenant visibility of such access, is recorded in `6M-Admin-Platform-APIs.md`. Full detail: 6B §18.3b, 6D §25's matching amendment, `5B-Identity-Organization-Multitenancy-Security.md`'s Phase 6M amendment.
