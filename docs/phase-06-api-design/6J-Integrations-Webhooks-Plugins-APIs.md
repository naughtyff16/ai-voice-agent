...
**V1 SCOPE DECISION (`DEP-6J-06`, CLOSED, §56):** the executed 5I DDL has **no `IntegrationSyncJob` or `IntegrationSyncCursor` table**, and V1 deliberately does not add one. The only sync-related persistence is two columns directly on `integration_connections`: `last_sync_at` (TIMESTAMPTZ) and `last_sync_error` (TEXT, ≤1000 chars, DB-CHECKed). There is no per-sync-job history, no resumable cursor, no concurrent-sync-tracking, and no way to list past sync attempts beyond "when did the most recent one happen and did it fail" — this is V1's explicit, frozen scope, not a gap awaiting remediation.
...
- **Full/incremental, resumability, per-item conflict/partial-failure reporting, sync-history listing, an `IntegrationSyncJob` resource, a cursor API, progress percentage, cancellation, a durable integration-specific `/jobs/{id}` API:** all explicit **V1 non-goals** per this section's own scope decision (`DEP-6J-06`, **CLOSED BY V1 SCOPE DECISION**, §56) — not gaps awaiting remediation. If a future phase's product requirements demand job-history semantics, the concrete schema addition sketched historically (`integrations.integration_sync_jobs` with `cursor JSONB`, `status`, `items_processed`, `items_failed`, `error_summary`) remains available as a `FUTURE ENHANCEMENT`, contingent on that product decision being made — not on any technical blocker.
...
```
- **Validation (fails `422 WEBHOOK_URL_UNSAFE` before any DB write):** `target_url` must be `https://`; every §30 SSRF check runs at registration time (resolve-then-pin DNS, reject loopback/link-local/RFC1918/cloud-metadata unless enterprise allow-listed, reject non-standard ports where policy restricts them). `topics` non-empty, every value must be a recognized topic from §19's governed catalog (`422 WEBHOOK_TOPIC_INVALID` with `details.invalid_topics` for any unrecognized value — this is an application-layer allow-list check; the DB column itself is free TEXT[], §5I §12).
...
| `IntegrationSyncJob` / `IntegrationSyncCursor` | **Does not exist — by V1 scope decision, not omission** | — | **`DEP-6J-06` — CLOSED BY V1 SCOPE DECISION** (§56) |
...
**Final closure classification (2026-08-29, PostgreSQL 18 baseline pass, governing task §23):** every row below ends as exactly one of `CLOSED`, `CLOSED BY V1 SCOPE`, or `FUTURE ENHANCEMENT — NON-BLOCKING` — no row is left `OPEN`. The `CLOSED` verdicts below restate this pass's own re-confirmation on a fresh PostgreSQL 18.6 disposable cluster (§63); the underlying SQL is unchanged from the prior PostgreSQL-16-validated pass (§60's original evidence remains valid supporting history, not superseded, since the SQL itself did not change).

| ID | Severity | Gap | Affected endpoints | Status / remediation |
|---|---|---|---|---|
| **DEP-6J-01** | **P0 — CLOSED** | Originally: no SECURITY DEFINER function transitioned `integration_connections.status` `CONNECTING → ACTIVE`, `ACTIVE ⇄ DEGRADED`, or any status `→ DISCONNECTED`. | §11.2 (completion half), §11.4, §11.6, §11.7 (status half), §15.3 (result-write half) | Closed by `5K/migrations/101_5I1.sql` (§8.6): `fn_activate_integration_connection`, `fn_fail_integration_connection`, `fn_degrade_integration_connection`, `fn_disconnect_integration_connection`, `fn_update_integration_connection_config`, `fn_record_integration_sync_result` — each additionally hardened with the tenant-forgery guard (§31.1, ADR-6J-11). **Live-validated** (PostgreSQL 18.6): fresh + incremental migration PASS, full integration-connection lifecycle matrix PASS (legal/illegal/idempotent transitions, terminal-guard re-confirmed) — `5K/validation/6J_FINAL_BLOCKER_REMEDIATION_VALIDATION_REPORT.md` §10. |
| **DEP-6J-02** | **P0 — CLOSED** | Originally: no function suspended/reactivated a `plugin_installations` row, and no function updated `configuration`/`credential_ref` in place. | §27.4, §27.4a, §27.6 | Closed by `101_5I1.sql` (§27.1): `fn_suspend_plugin_installation`, `fn_reactivate_plugin_installation`, `fn_update_plugin_installation_config`, `fn_rotate_plugin_installation_credential` — tenant-forgery-guarded. **Live-validated**: full plugin lifecycle matrix PASS, including live-proven capability preservation across suspend→reactivate and capability reset+re-pin across version upgrade — validation report §11. |
| DEP-6J-03 | P2 -- **FUTURE ENHANCEMENT -- NON-BLOCKING** | `integration_connections.external_account_ref` has no DB-level uniqueness constraint per `(definition_id, external_account_ref)` — two connections (across different orgs, or theoretically the same org after a disconnect/reconnect cycle) could reference the same external account without the DB objecting | §11.2 | Non-blocking; `idx_ic_ext_account` already supports an application-layer duplicate-check query; a DB constraint is a future hardening, not required for V1 correctness |
| **DEP-6J-04** | **CLOSED** | Originally: `oauth_attempts` had no `connection_id` column — correlation was inferred via `(organization_id, definition_id)`. | §13.2 | Closed by `101_5I1.sql`: `oauth_attempts.connection_id` (nullable FK to `integration_connections`), populated at `INSERT` time. Correlation is now explicit, remaining correct even if Decision J1 (§57) later allows multiple connections per provider. Column presence and population confirmed live throughout the OAuth test matrix (validation report §9). |
| **DEP-6J-05** | **CLOSED (was P0-adjacent -- folded into the OAuth-callback tenant-bootstrap fix, then further split by ADR-6J-13)** | Originally: no function marked an `oauth_attempts` row `FAILED` on provider-denied authorization, and (more severely) the redemption path itself had a P0 tenant-bootstrap circular dependency (first-pass remediation §4), further found to have a P0 provider-binding gap and a P0 state-machine contradiction (second-pass remediation §5-§6). | §13.3, §13.5, §13.5a | Closed by `101_5I1.sql`: `fn_redeem_oauth_callback_state(state, expected_definition_id)`, `fn_fail_oauth_callback_state(state, expected_definition_id, reason)`, and the new `fn_record_oauth_exchange_failure(state, reason)` (§13.3/§13.5/§13.5a, ADR-6J-08/6J-12/6J-13). **Live-validated**: 14/14 OAuth test matrix — validation report §9. |
| **DEP-6J-06** | **CLOSED BY V1 SCOPE DECISION (2026-08-29)** | No `IntegrationSyncJob`/`IntegrationSyncCursor` table exists — sync history, resumable cursors, per-item conflict/partial-failure reporting are all unimplementable; only `last_sync_at`/`last_sync_error` on the connection itself persist | §15 (entire synchronization model) | **Explicit V1 scope decision, not a deferred gap.** V1 sync is frozen as `POST /integrations/connections/{id}/sync` — an async trigger only, backed exactly by the two columns that already exist (`last_sync_at`, `last_sync_error`). V1 does **not** include, and does not need, a sync-job table: no sync-history listing, no `IntegrationSyncJob` resource, no cursor API, no resumability, no progress percentage, no cancellation, no durable integration-specific `/jobs/{id}` API. A tenant can see whether the most recent sync succeeded (`last_sync_error IS NULL`) and when it last ran (`last_sync_at`) — nothing more is promised by this document's V1 contract, and §15 does not claim otherwise. Re-opening this as a real gap would require a **product decision** to add job-history semantics to V1, not a technical remediation — until that decision is made, `integrations.integration_sync_jobs` remains a named, well-specified **future enhancement** (§15.1's own sketch), not an open blocker. |
| DEP-6J-07 | **FUTURE ENHANCEMENT -- NON-BLOCKING** | No auto-suspend function/trigger exists for a `webhook_endpoint` whose deliveries are chronically `DEAD_LETTER` — `SUSPENDED` status exists but is system-driven-only by design (4F §8.1) and nothing drives it | §22.5 | A future migration or Celery-side policy (not necessarily DB-level) implementing the 20-consecutive-dead-letter threshold this document proposes as a default |
| DEP-6J-08 | **FUTURE ENHANCEMENT -- NON-BLOCKING** | `integration_definitions` has no `category`, `configuration_schema`, or `feature_maturity` column — these are presented in §9's response examples as **application-layer metadata** (a config file/admin-tool-managed overlay, not a DB column), consistent with the executed DDL's actual, narrower column set (`slug, name, description, auth_type, capabilities, required_scopes, manifest_version, documentation_url, is_active`) | §9.1, §9.2 | Non-blocking — this document's response contract is achievable entirely at the application layer; flagged so implementers don't search for a non-existent DB column |
| **DEP-6J-09** | **CLOSED** | Originally: `fn_create_integration_connection()` did not itself check `integration_definitions.is_active`. | §11.2, §43.1 | Closed by `101_5I1.sql`'s `CREATE OR REPLACE` on `fn_create_integration_connection()` (same signature, added `is_active` check plus the tenant-forgery guard, ADR-6J-11). Live-validated as part of the tenant-forgery matrix, test 1 (validation report §8). |
| **DEP-6J-10** (new, second pass) | **P0 -- CLOSED** | `SECURITY DEFINER` tenant-forgery: every `app_api`-callable, `p_organization_id`-taking function (new and 8 pre-existing 059-066 functions alike) trusted its parameter unconditionally, with RLS not applicable inside a `BYPASSRLS`-owned function's own queries. | Every endpoint in §48 backed by a tenant-bound guarded function | Closed by `101_5I1.sql`'s tenant-forgery guard, ADR-6J-11, §31.1. **Live-validated**: 11-test adversarial matrix, zero forged mutations succeeded — validation report §8. |
| **DEP-6J-11** (new, second pass) | **Was P0-equivalent -- CLOSED, platform-wide impact beyond 6J** | `public.gen_uuid_v7()` (`001_5B.sql`, pre-existing, not a 6J defect) has no fixed `search_path`, breaking inside 84 of 99 `SECURITY DEFINER` functions platform-wide whenever their own search_path excludes `public`. Live-discovered blocking 6J's own functional writes. | All of §48 (was blocking every write path) | Closed by `101_5I1.sql`'s `CREATE OR REPLACE FUNCTION public.gen_uuid_v7() ... SET search_path = public, pg_catalog` (ADR-6J-14). **Live-validated** for `integrations`/`plugins`/`webhooks`. **A full audit of the other 83 affected functions outside these three schemas is explicitly out of scope for this document** — forward finding for the owning phases (5B-5H, 5J). |
| **DEP-6J-12** (new, second pass) | **CLOSED (2026-08-29)** | 6I §11's frozen `WebhookNodeConfig`/`ApiCallNodeConfig` did not define the `plugin_installation_id`/`plugin_version_id`/`credential_source` fields §30.2's design targets. | §30.2, §30.5 | **Closed by a small, controlled 6I compatibility amendment** (`6I-Workflow-APIs.md` §67, 2026-08-29) — narrowly scoped exactly to this gap, per the governing task's explicit authorization to perform a bounded 6I amendment rather than reopen Phase 6I broadly. `WebhookNodeConfig`/`ApiCallNodeConfig` now carry a `credential_reference` field matching this section's three-shape contract verbatim (§30.2), with the full publish-time (org/status/version/capability) and runtime re-validation logic specified on 6I's side (6I §67.3), returning 6I's own `PLUGIN_VERSION_PINNED_MISMATCH` for a version-drift condition — the identical canonical code this document's own error catalogue (§35) and ADR-6J-09 already define. **6I's broader `WEBHOOK`/`API_CALL` execution-block (ADR-6I-04) is unchanged** — the field-shape gap is closed; the egress-adapter implementation itself remains a forward item, exactly as disclosed for every other application-code component in this document series (no application code exists anywhere in Phase 5/6, by governing constraint). |

No table, column, or function name anywhere in this document is invented and presented as already existing — every reference above either cites the exact 5I/`101_5I1` migration/function or is explicitly listed here as absent. **Live validation status — PostgreSQL 18.x is this document's authoritative database baseline** (§63; superseding the earlier-stated PostgreSQL 16 target, per the controlled baseline-reconciliation decision) **for every "CLOSED"/"LIVE-VALIDATED" row above:** `5K/validation/6J_FINAL_BLOCKER_REMEDIATION_VALIDATION_REPORT.md` (original PostgreSQL 18.6 execution, retained as valid supporting evidence — the underlying SQL is unchanged) plus `5K/validation/POSTGRESQL_18_BASELINE_AND_6J_FINAL_VALIDATION_REPORT.md` (this pass's fresh disposable-cluster re-confirmation), full raw evidence in `5K/execution_logs/` (prefix `20260829T210000Z_`, 14 files, original PostgreSQL 18.6 pass; prefix `20260829T220000Z_`, 7 files, this pass's fresh disposable-cluster re-confirmation).
...
- [x] Missing persistence is flagged (§56 — DEP-6J-01/02/04/05/09/10/11 **CLOSED**; DEP-6J-06 **CLOSED BY V1 SCOPE DECISION**; DEP-6J-12 **CLOSED** via 6I §67 compatibility amendment; DEP-6J-03/07/08 remain genuine, non-blocking `FUTURE ENHANCEMENT` items — see §56, no row left `OPEN`).
...
**No item above is marked "EXECUTION-BLOCKED."** Every endpoint in §48 has a designed, DB-backed, **live-validated** execution path. `DEP-6J-06` and `DEP-6J-12` are both now **CLOSED** (§56, §63) — V1 scope decision and 6I §67 compatibility amendment respectively. What remains open is limited to the SSRF application-layer tests (untestable at this repository's current layer — no deployed application code exists to test against) and three named, non-blocking `FUTURE ENHANCEMENT` items (`DEP-6J-03/07/08`, §56).
...
- [x] Sync semantics implementation-ready **for what 5I's schema actually supports** (§15) — full job-history semantics are an explicit V1 non-goal, not a fabricated feature and not an open gap (`DEP-6J-06`, **CLOSED BY V1 SCOPE DECISION**, §56).
...
**Status: EXECUTED — PostgreSQL 18.6, this document's authoritative database baseline (§63).** Every DB-layer-testable category below (§60.1-§60.7 as originally specified, covering OAuth, integration/plugin lifecycle, and — via the tenant-forgery/webhook-rotation matrices actually run — the equivalent of §60.2's tenant-scoping concerns) was executed as a real adversarial test suite against a genuine, isolated, throwaway PostgreSQL instance this pass, not merely specified. Full raw evidence, exact commands, and complete output: `5K/validation/6J_FINAL_BLOCKER_REMEDIATION_VALIDATION_REPORT.md` and `5K/execution_logs/` (14 files, prefix `20260829T210000Z_`). **A further, independent re-confirmation of the core matrices ran on a brand-new PostgreSQL 18.6 disposable cluster during the baseline-reconciliation pass (§63)** — `5K/validation/POSTGRESQL_18_BASELINE_AND_6J_FINAL_VALIDATION_REPORT.md`, `5K/execution_logs/` (prefix `20260829T220000Z_`). The table below is retained as the original test specification — each row is annotated with its actual, live result. **§60.6 (SSRF) was not executed** — it is application-layer logic with no deployed application code anywhere in this repository to run it against; disclosed as untestable at this layer, not skipped. §60.7's underlying DB-layer capability check-enforced-capabilities test) was executed via the plugin lifecycle matrix (§26 renamed §60.3 below); the workflow-publish-time/execution-time re-check itself is 6I-owned code this document cannot execute (§56 — `DEP-6J-12` is now closed at the *field-shape/contract* level via 6I §67, but no runtime code exists anywhere in this document series to execute).
...
| 33 | **Pass-3 — closure of finding #31 (`DEP-6J-12`)** | Row 31 above disclosed the field-shape gap as unresolved, correctly, since 6I amendment authority did not yet exist for this task | Governing task explicitly authorized a small, controlled 6I amendment this pass — `6I-Workflow-APIs.md` §67 adds the `credential_reference` field matching this document's own §30.2 contract, with full publish/runtime validation | Yes (6I document, narrowly scoped, `WEBHOOK`/`API_CALL` node config and validation logic only — no 6I migration/function/grant touched) | N/A (6I is a design document; no SQL) | Closes the field-shape gap; 6I's broader `WEBHOOK`/`API_CALL` execution-block (ADR-6I-04) deliberately left unchanged | **Closed** — 6I §67 |
| 34 | **Pass-3 — `DEP-6J-06` reclassification** | Rows in §56/§62 previously carried `DEP-6J-06` as "P1, still open" | Explicit V1 scope decision: sync stays an async trigger only; job-history semantics are a named `FUTURE ENHANCEMENT` contingent on a future product decision, not a technical blocker | No (scope decision, not a fix) | N/A | Removes a P1 that was never actually blocking any endpoint's own internal consistency (§15.2 already specified an honest, self-consistent V1 contract) | **Closed by V1 scope decision** — §56 |
| 35 | **Pass-3 — PostgreSQL baseline reconciliation** | Passes 1-2 each disclosed their PostgreSQL 18.6 execution as a deviation from "the requested 16" | PostgreSQL 18.x declared the platform's authoritative baseline; PostgreSQL 16 evidence retained as valid legacy-compatibility history, not rewritten | No (declarative baseline decision + extension re-verification) | N/A | Removes recurring version-drift disclosure; extensions (`vector`/`pgcrypto`/`pg_stat_statements`) independently re-verified on PostgreSQL 18.6, not assumed | **Closed** — §63, no P0 found |
...
## 63. PostgreSQL 18 Baseline Reconciliation (2026-08-29)

### 63.1 Decision

**Previous baseline:** PostgreSQL 16 (stated as the deployment target in `5K-Database-Migration-and-Implementation.md` §5, and treated as the "requested" engine version by remediation passes 1-2, both of which disclosed their PostgreSQL 18.6 execution as a deviation from it).

**New baseline:** PostgreSQL 18.x is now the authoritative database major version — for development, CI, and production — superseding PostgreSQL 16.

**Reason:** the platform is pre-production; PostgreSQL 18 was already the active development-environment engine in practice (every one of this document's own live validation passes, including the original pass-2 closure, ran on PostgreSQL 18.6, not 16); continuing to declare PostgreSQL 16 as the "true" target while every actual validation runs on 18 is version drift the platform should not carry into its Phase 6 freeze. This is a deliberate architecture decision made once, now, rather than a per-document disclosed deviation repeated indefinitely.

**Scope — what this decision does NOT change:** the data model, bounded contexts, RLS design, authorization model, API contracts, modular-monolith architecture, Redis/outbox patterns, or Alembic migration strategy are all **unchanged**. This is a runtime-engine-version decision only. No table, column, function signature, RLS policy, or API endpoint defined anywhere in Phase 5 or Phase 6 is altered by this section.

**Extensions verified directly on PostgreSQL 18.6** (not assumed from PostgreSQL 16 evidence, per the governing instruction not to fake compatibility): `vector` (pgvector) 0.8.6, `pgcrypto` 1.4, `pg_stat_statements` 1.12 — all three installed cleanly (`CREATE EXTENSION IF NOT EXISTS ...`, zero errors) on a fresh PostgreSQL 18.6 instance; `vector` additionally functional-tested with a real `CREATE TABLE ... vector(3)` / `INSERT` / `<->` distance query, returning correct results. **No P0 found** — every required extension is confirmed compatible.

### 63.2 What Changed vs. What Stayed Historical

**Updated to PG18 as the current, authoritative statement** (this document, `5K-Database-Migration-and-Implementation.md` §5, `alembic/env.py`'s docstring, `alembic/README.md`'s CI test-image note) — these describe the platform's *current* state and must not silently keep asserting a superseded number.

**Left untouched, as accurate history** — every execution log, validation report, and narrative passage that truthfully records a PostgreSQL 16 (or 16.10) run that genuinely happened (5C/5E/5G's own historical narrative passages, `PG16_MIGRATION_VALIDATION_REPORT.md` and the other `execution_logs/`/`validation/` files predating this pass, 6I §63's "live-validated on genuine PostgreSQL 16.10" statement) is **not rewritten** — rewriting a true historical record to claim a different engine version ran would itself be the kind of fabrication these remediation passes have consistently been instructed to avoid. Where this document itself previously framed its own PostgreSQL 18.6 execution as a "disclosed deviation from the requested 16" (§1 remediation-pass-2 row, former §60/§62 language), that framing is corrected in place (this is this document's own current-state claim, not a third party's historical record) — the underlying test results and raw evidence files are unchanged and remain fully valid.

### 63.3 Environment Verification (This Pass)
...
Disposable cluster: `.tmp_pgdata_pg18v2` (initdb'd fresh for this pass, port 5557, `shared_buffers=4MB` — a Windows-sandboxed-process shared-memory constraint disclosed and worked around identically to pass 2, not a PostgreSQL 18-specific issue). Full re-validation evidence: `5K/validation/POSTGRESQL_18_BASELINE_AND_6J_FINAL_VALIDATION_REPORT.md`, `5K/execution_logs/` (prefix `20260829T220000Z_`, 7 files). Cluster stopped and its data directory removed after evidence capture, per the housekeeping discipline in §63.4.
