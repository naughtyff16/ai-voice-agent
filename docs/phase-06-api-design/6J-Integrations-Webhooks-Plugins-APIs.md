...
| **Remediation pass** | **2026-08-29 (same day) — strict corrective pass following independent engineering/security review.** 7 P0 and 17 P1/P2 findings addressed (§61 Reconciliation Table); additive migration `101_5I1.sql` authored to close the P0 database-function gaps, **not yet live-validated** (disclosed environment limitation, §60/§62). Final status: `PHASE 6J — NOT READY — P0 BLOCKERS REMAIN` (§62) — pending live validation only, not pending further design work. |
...
6. Server:  integrations.fn_redeem_oauth_callback_state(state) — PENDING → REDEEMED, exactly once,
              takes `state` ALONE (no organization_id input — the caller doesn't have one yet),
            ...
            server now has organization_id — sets it as this request's tenant context for the remainder
...
`integrations.fn_redeem_oauth_callback_state(p_state TEXT)` (§56, `101_5I1.sql`) fixes this by taking `state` **alone** and returning tenant identity as **output**:

```sql
SELECT * FROM integrations.fn_redeem_oauth_callback_state('the-state-value-from-the-query-string');
...
- `expires_at <= NOW()` → sets `EXPIRED`, raises → `410 OAUTH_STATE_EXPIRED`
- Otherwise → `PENDING → REDEEMED`, `redeemed_at = NOW()`, returns the full row context

**Why this does not require removing RLS from `oauth_attempts` (a question a naive fix might reach for):** `oauth_attempts` keeps `ENABLE + FORCE ROW LEVEL SECURITY` completely unchanged. `fn_redeem_oauth_callback_state()` is `SECURITY DEFINER`, so its internal `SELECT`/`UPDATE` execute under the function's **owning role** — `app_migration`, which was created `BYPASSRLS` in `001_5B.sql` and independently reconfirmed still-`BYPASSRLS` by the live `077_5J1` validation pass (`5K/validation/077_5J1_VALIDATION_REPORT.md` line 309). RLS enforcement was therefore never actually the obstacle inside this function — the only real defect was the signature requiring an input the caller couldn't supply. This function's `EXECUTE` grant (`app_api, app_worker, app_platform_admin`) is what an ordinary, non-BYPASSRLS `app_api` caller uses to reach it; the function's own execution context (not the caller's) is what determines RLS applicability for the queries inside it — this is standard PostgreSQL `SECURITY DEFINER` semantics, not a platform-specific exception.
...
The full adversarial security test matrix exercising every row above (localhost, RFC1918, IPv6 loopback, IPv6 unique-local, cloud metadata, DNS rebinding, redirect-to-private-IP, redirect credential leakage, TLS hostname mismatch) is enumerated in §26.
...
All rows below reflect the remediation pass (2026-08-29) corrections — no endpoint is marked `EXECUTION-BLOCKED` any longer (§56's P0 gaps are closed by migration `101_5I1.sql`, subject to the not-yet-live-validated disclosure in §60); every DB-execution note below says "direct" rather than "internal-RPC" per ADR-6J-01's revision (§55).
...
|---|---|---|---|---|
| **DEP-6J-01** | **P0 — RESOLVED, NOT YET LIVE-VALIDATED** | Originally: no SECURITY DEFINER function transitioned `integration_connections.status` `CONNECTING → ACTIVE`, `ACTIVE ⇄ DEGRADED`, or any status `→ DISCONNECTED`. | §11.2 (completion half), §11.4, §11.6, §11.7 (status half), §15.3 (result-write half) | Closed by `5K/migrations/101_5I1.sql` (§8.6): `fn_activate_integration_connection`, `fn_fail_integration_connection`, `fn_degrade_integration_connection`, `fn_disconnect_integration_connection`, `fn_update_integration_connection_config`, `fn_record_integration_sync_result`. SQL authored and traced against the applied 059-066 files; **not executed against a live PostgreSQL instance** in the authoring session (disclosed environment limitation, `5K/MIGRATION_MANIFEST.md`). Remains a blocker to `IMPLEMENTATION READY` (§60) until live-validated. |
| **DEP-6J-02** | **P0 — RESOLVED, NOT YET LIVE-VALIDATED** | Originally: no function suspended/reactivated a `plugin_installations` row, and no function updated `configuration`/`credential_ref` in place. | §27.4, §27.4a, §27.6 | Closed by `101_5I1.sql` (§27.1): `fn_suspend_plugin_installation`, `fn_reactivate_plugin_installation`, `fn_update_plugin_installation_config`, `fn_rotate_plugin_installation_credential`. Same not-yet-live-validated disclosure as DEP-6J-01. |
...
| **DEP-6J-04** | **RESOLVED, NOT YET LIVE-VALIDATED** | Originally: `oauth_attempts` had no `connection_id` column — correlation was inferred via `(organization_id, definition_id)`. | §13.2 | Closed by `101_5I1.sql`: `oauth_attempts.connection_id` (nullable FK to `integration_connections`), populated at `INSERT` time. Correlation is now explicit, remaining correct even if Decision J1 (§57) later allows multiple connections per provider. |
| **DEP-6J-05** | **RESOLVED, NOT YET LIVE-VALIDATED (was P0-adjacent — folded into the OAuth-callback tenant-bootstrap fix)** | Originally: no function marked an `oauth_attempts` row `FAILED` on provider-denied authorization, and (more severely, discovered during remediation) the redemption path itself had a P0 tenant-bootstrap circular dependency (remediation §4). | §13.5 | Closed by `101_5I1.sql`: `fn_redeem_oauth_callback_state(state)` and `fn_fail_oauth_callback_state(state, reason)` (§13.3/§13.5, ADR-6J-08) — both take `state` alone, no `organization_id` input, closing the actual P0 alongside the originally-scoped minor gap. |
...
| **DEP-6J-09** | **RESOLVED, NOT YET LIVE-VALIDATED** | Originally: `fn_create_integration_connection()` did not itself check `integration_definitions.is_active`. | §11.2, §43.1 | Closed by `101_5I1.sql`'s `CREATE OR REPLACE` on `fn_create_integration_connection()` (same signature, added `is_active` check) |

No table, column, or function name anywhere in this document is invented and presented as already existing — every reference above either cites the exact 5I/`101_5I1` migration/function or is explicitly listed here as absent. **All "RESOLVED" rows above share one caveat, restated once here rather than in each row: the resolving SQL has been authored and manually traced against source but has not been executed against a live PostgreSQL instance in the authoring session (§60) — this is why §60's final status remains `NOT READY` rather than `IMPLEMENTATION READY` despite every P0 gap having a designed, reviewed fix.**
...
- [x] Missing persistence is flagged (§56 — DEP-6J-01/02/04/05/09 **resolved by `101_5I1.sql`, not yet live-validated**; DEP-6J-06/07/08 remain genuinely open, non-blocking except DEP-6J-06 (P1)).
...
- [x] Error catalog is complete (§35.3 — 30 domain-specific codes, 3 added this remediation pass).
...
- [x] **(New this pass)** Every P0/P1 finding from the independent-review remediation is either fixed-and-disclosed-as-not-yet-live-validated, or explicitly re-classified with rationale (§61 Reconciliation Table — 24 findings, 0 defended-away, 0 silently dropped).
- [x] **(New this pass)** A concrete adversarial security test matrix is specified (§60) covering every SSRF/OAuth/webhook/inbound-callback/plugin control this document claims — not yet executed (§60's own status line), but fully enumerated so it can be run the moment a validated environment is available.

**No item above is marked "EXECUTION-BLOCKED" any longer** — every endpoint in §48 has a designed, DB-backed execution path (5I 059-066 plus the additive `101_5I1.sql`). What remains open, and is why §62's final verdict is not yet `IMPLEMENTATION READY`, is purely the **live-validation status** of `101_5I1.sql` (§56, §60) — a testing/verification gap, not a design gap.
...
- [x] Connection lifecycle specified (§8, §11) — DEP-6J-01 resolved by `101_5I1.sql`, not yet live-validated (§56, §60).
...
- [x] Installation lifecycle is complete (§27) — suspend/reactivate/config-update/credential-rotation gaps (DEP-6J-02) resolved by `101_5I1.sql`, not yet live-validated.
...
Every row below is a **required** test before `101_5I1.sql` — and therefore this document's endpoints built against it — can be represented as live-validated. **None of these have been executed in the session that authored this document** (§56, §61, §62 — disclosed, not silently assumed passing); this matrix is the concrete, runnable specification the next environment with live database + application-service access must execute.
...
| # | Finding (remediation prompt §) | Old 6J behavior | Corrected behavior | Persistence change? | Migration | Security impact | Status |
|---|---|---|---|---|---|---|---|
| 1 | §1 — phase status contradiction | Declared `IMPLEMENTATION READY` while documenting P0 blockers | Status logic corrected: `NOT READY` while any P0 is unresolved or unvalidated (§62) | No | — | Prevents a false "ready" signal from reaching an implementer | Fixed |
| 2 | §2 — DEP-6J-01, connection lifecycle | No DB path for `CONNECTING→ACTIVE`/`ACTIVE⇄DEGRADED`/`→DISCONNECTED` | 6 new SECURITY DEFINER functions (§8.6, §56) | Yes | `101_5I1.sql` | Closes a P0 functional gap; least-privilege pattern preserved (org-scoped `FOR UPDATE` in every function) | Fixed, not yet live-validated |
| 3 | §3 — DEP-6J-02, plugin lifecycle | No DB path for suspend/reactivate/config-update/credential-rotate | 4 new SECURITY DEFINER functions (§27, §56) | Yes | `101_5I1.sql` | Same pattern as #2 | Fixed, not yet live-validated |
| 4 | §4 — OAuth callback tenant bootstrap | `fn_redeem_oauth_attempt(state, organization_id)` required an input the unauthenticated callback caller cannot supply | `fn_redeem_oauth_callback_state(state)` — state alone, returns tenant identity as output; confirmed safe via `app_migration`'s `BYPASSRLS` (cited against `001_5B.sql` + live `077_5J1_VALIDATION_REPORT.md`) | Yes | `101_5I1.sql` | Closes a P0 circular-dependency defect that made the entire OAuth flow's completion step unimplementable as designed | Fixed, not yet live-validated |
| 5 | §5 — reauthorize-from-FAILED contradiction | `POST .../reauthorize` allowed on `FAILED`, contradicting the terminal-state guard | Reauthorize restricted to `ACTIVE`/`DEGRADED` only; `FAILED`/`DISCONNECTED` return `409 INTEGRATION_DISABLED` | No | — | Removes a contract that would have thrown a DB exception in practice | Fixed |
| 6 | §6 — webhook secret rotation cryptographic defect | Old secret retained but never signed with — no real grace period | Dual-signature grace: both current and unexpired-previous secrets sign every delivery during the window | Yes (2 new columns) | `101_5I1.sql` | Closes a P0 defect that made secret rotation break in-flight consumer deployments | Fixed, not yet live-validated |
| 7 | §7 — plugin/workflow version determinism | Node config stored only `plugin_installation_id`; reactivation after upgrade could silently resume with new behavior | Node config pins `plugin_version_id`, re-checked at every execution, not just publish | No (6I-owned `graph_json` field; no 6J DB change) | — (6I coordination item, ADR-6J-09) | Closes a P0 non-determinism defect | Fixed (design); depends on 6I's own publish/execution code adopting the check |
| 8 | §8 — `enabled_capabilities` field existence | Assumed possibly invented | **Verified genuinely exists** on both `integration_connections` and `plugin_installations` (migration `061_5I.sql`/`065_5I.sql`, directly re-read) | No | — | None — false alarm resolved by direct verification, not by a design change | Verified, no change needed |
| 9 | §9 — generic inbound tenant resolution (confused deputy) | Tenant resolved from unverified payload content | Opaque per-connection routing segment selects the connection before payload trust; verification confirms before tenant context is used | No (routing convention, not a schema change) | — | Closes a P0 confused-deputy/cross-tenant-routing risk | Fixed |
| 10 | §10 — idempotency fingerprint on low-entropy secrets | Bare `SHA-256(body)` | `HMAC-SHA256(platform_key, body)` | No (application-layer/Redis, not DB) | — | Closes a P1 offline-brute-force exposure for low-entropy credentials | Fixed |
| 11 | §11 — OAuth denial path | No function marked an attempt `FAILED`; folded into the P0 above | `fn_fail_oauth_callback_state`, same tenant-bootstrap-safe shape | Yes | `101_5I1.sql` | Closes P1 (subsumed into #4's P0 fix) | Fixed, not yet live-validated |
| 12 | §12 — internal-RPC pattern reconsidered | Synchronous internal-JWT RPC hop proposed for 5 pre-existing functions | 5 `GRANT EXECUTE ... TO app_api` — direct execution, no RPC | Yes (grants only) | `101_5I1.sql` | Removes an unnecessary architectural layer; security posture unchanged (functions already self-authorize) | Fixed, not yet live-validated |
| 13 | §13 — inbound dedup/enqueue race safety | Prose described the right outcome without stating enqueue-gating as a hard requirement | `INSERT ... ON CONFLICT DO NOTHING RETURNING id`; enqueue gated explicitly on a returned row | No (application code pattern) | — | Makes the race-safety guarantee an explicit implementation requirement, not an implied one | Fixed |
| 14 | §14 — fallback fingerprint ordering | Computed from "normalized_payload_bytes" (post-normalization, wrong ordering) | Computed pre-normalization, from raw verified bytes | No | — | Removes a circular/wrong-ordering description; fallback's own honest limitation (content-identical-but-distinct events) unchanged | Fixed |
| 15 | §15 — SSRF DNS/TLS/redirect completeness | Missing IPv6 unique-local, IPv4-mapped-IPv6, per-request re-resolution, TLS/SNI hostname preservation, redirect credential stripping | All added explicitly to §30.3 | No | — | Closes several P1 SSRF-adjacent gaps in the adapter's stated contract | Fixed |
| 16 | §16 — Prometheus label cardinality | `organization_id` in a metric label | Removed; tenant identity moved to traces/logs/audit only | No | — | Closes a P1 operational/cost risk (unbounded cardinality), not a security defect per se | Fixed |
| 17 | §17 — webhook DELETE/disable incoherence | `DELETE` emitted `WEBHOOK_ENDPOINT_DELETED` despite no deletion | `DELETE` now emits `WEBHOOK_ENDPOINT_DISABLED`, explicitly documented as an alias of `.../disable` | No | — | Corrects a misleading audit trail entry | Fixed |
| 18 | §18 — sync job contract honesty | Already largely honest (Option A) | Re-confirmed, `fn_record_integration_sync_result` cited by name | Yes (function only, not a job table) | `101_5I1.sql` | N/A | Confirmed correct, minor wording tightened |
| 19 | §19 — rotation idempotency vs. one-time-secret exposure | Not fully analyzed | `Idempotency-Key` on rotate-secret only replays the cached *response* (already one-time-reveal by design, §12.1 rule 6) — no separate raw-secret cache introduced; analysis added explicitly | No | — | Confirms no new secret-exposure surface from combining idempotency + one-time-reveal | Fixed (analysis added) |
| 20 | §20 — plugin callout signature scope | Body-only signature | Method + canonical path + timestamp + body, mirroring §21.1's outbound shape | No | — | Closes a P1 request-context-substitution risk | Fixed |
| 21 | §21 — `X-Platform-Tenant-Id` overstated as isolation | Implied as a tenant-isolation guarantee | Explicitly reclassified as non-enforcing communication; real guarantees (per-tenant credential, rate limit) restated | No | — | Corrects a misleading security claim | Fixed |
| 22 | §22 — provider callback rate limiting layers | Single global per-provider ceiling only | 4-layer model: edge IP, per-provider, post-verification per-connection, size/concurrency caps | No | — | Closes a P1 gap in abuse-resistance-before-identity-is-known | Fixed |
| 23 | §23 — redirect credential leakage | Not addressed | Folded into §30.3's redirect policy row (cross-origin header stripping) | No | — | Same fix as #15, listed separately per the remediation's own numbering | Fixed |
| 24 | §24 — re-evaluate all DEP severities | Original severities as first-drafted | Re-evaluated: DEP-6J-01/02/04/05/09 → resolved-not-validated; DEP-6J-06 confirmed P1 open; DEP-6J-03/07/08 confirmed non-blocking | No | — | N/A | Fixed (§56) |

**0 findings defended away or dismissed without a change.** Finding #8 is the one row confirmed as a false alarm — resolved by directly re-reading the executed migration file, not by declining to investigate.

---

## 62. Final Phase Status

`PHASE 6J — NOT READY — P0 BLOCKERS REMAIN`

**Why this is the correct verdict despite every identified P0 having a designed, reviewed fix:** per this remediation's own governing rule (§1/§29 of the remediation prompt), final status must be `NOT READY` until every P0 is not merely designed but **actually closed** — and "closed" requires the resolving migration to be executed and validated, not merely authored. `101_5I1.sql` (§56) closes DEP-6J-01, DEP-6J-02, and the OAuth-callback/webhook-rotation/plugin-version-pinning/confused-deputy P0s **by design** — every function, column, and grant is specified, cross-checked against the actual applied 059-066/001 files, and reasoned through adversarially (§60's test matrix, §61's reconciliation table) — but it has **not** been executed against a live PostgreSQL instance in this session, due to a disclosed environment limitation (no available superuser credentials for the pre-existing local Postgres service; an isolated throwaway instance failed to start due to a Windows sandboxed-process shared-memory reservation error — full detail in `5K/MIGRATION_MANIFEST.md`'s "Phase 6J Remediation Pass" section). A design that has not been executed against a real database has not been proven correct, regardless of how carefully it was traced by hand.

**What IS true as of this pass:**
- Every P0 finding in the independent review (§1 phase-status contradiction, §2 DEP-6J-01, §3 DEP-6J-02, §4 OAuth bootstrap, §6 webhook rotation, §7 plugin version pinning, §9 confused-deputy inbound routing) has a concrete, complete fix — either a migration (§56) or a corrected document contract (§13/§21/§24/§30) — with no remaining silent gap.
- Every P1 finding (§10 idempotency fingerprint, §12 internal-RPC removal, §13 race-safety, §14 fingerprint ordering, §15 SSRF completeness, §16 metric cardinality, §17 DELETE/disable coherence, §20 plugin signature scope, §21 tenant-header clarity, §22 rate-limit layering) is fixed in the document.
- DEP-6J-06 (sync job history) remains a genuinely open, disclosed P1 schema gap — out of this remediation's named scope, not silently claimed resolved.
- 24 findings tracked end-to-end in §61's reconciliation table; 0 dismissed without either a fix or an explicit, evidenced "not a real defect" determination (finding #8).

**What remains before `PHASE 6J — IMPLEMENTATION READY` can honestly be declared:**
1. Execute `alembic upgrade head` (full `001_5B → … → 101_5I1` chain) against a real, fresh PostgreSQL instance.
2. Run §60's full security test matrix (63 cases) against that live database.
3. Confirm no regression against the pre-existing 059-100 test suites referenced in `5K/execution_logs/README.md` and `5K/validation/*.md`.
4. Update `5K/MIGRATION_MANIFEST.md`'s "Phase 6J Remediation Pass" section and 5I's own §38 amendment note (both currently marked "not yet live-validated") to record the actual results.

This document does **not** declare itself `PHASE 6J — FROZEN`. Final freeze/approval — and the `NOT READY → READY` transition itself — is performed only after independent review confirms the above four items, per the governing task's explicit instruction.
