...
| DEP-6B-02 | Authentication audit-vocabulary extension: `SESSION_REVOKED`, `TOKEN_REFRESH_REUSE_DETECTED`, admin-forced-logout, **forced-revocation-denylist-write (added this pass)** `action_kind` values | 5J's `action_kind` CHECK constraint has no matching value for these 4 event kinds; using an unrelated existing value would misrepresent the event (§25 Tier 2) | Audit requirement specified (§25); telemetry-only interim signal (§26); no schema change made | Future Phase 5.x | Yes | No | Yes — for these 4 event kinds only; all other audit events are fully supported today |
