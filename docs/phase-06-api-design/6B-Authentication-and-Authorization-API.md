...
2. **The grant's own lifecycle state** (which grants are currently active, their remaining TTL, enabling release-by-`grant_id`) — this has **no durable Phase 5 table**. It is API-contract-complete (§21.4) but only backed today by an interim, non-durable Redis record (`platform_admin:break_glass:{grant_id}`, §24). Redis here is temporary process state for enforcing the TTL and locating a grant to release, not an audit-grade or durable persistence layer — if Redis is lost, the *fact* that grants were opened and closed remains in `audit.audit_events` (item 1 above), but the *live* "is this grant still active" state does not survive independently of Redis. Durable grant-lifecycle persistence is tracked as an **upstream/future Phase 5.x dependency** (§36.3, item 1), not invented here and not assumed to already exist. **⚠ CORRECTED — see §18.3b:** this paragraph's premise (no durable table) is stale as of Phase 5L/6M; a durable `organization.break_glass_grants` table now exists and Redis is downgraded to a fast-path cache in front of it.

This distinction is the basis for §4's completeness-vs-readiness split: the break-glass **API design and security model are complete**; break-glass **durable implementation is conditional** on the future Phase 5.x grant table. **⚠ CORRECTED — see §18.3b:** that condition is satisfied as of Phase 5L (`organization.break_glass_grants`, `087_5B1.sql`) and further extended by Phase 6M (`105_5B4.sql`); durable implementation is no longer conditional on a future table.
...
4. Load the grant's live state from Redis (platform_admin:break_glass:{grant_id},
   §18.3/§24 — the fast-path cache in front of the now-durable
   organization.break_glass_grants table, §18.3b; DEP-6B-01, §36.3, is
   RESOLVED as of Phase 5L and does not block this runtime contract, which
   is unchanged: this step still reads Redis first for latency, not the
   durable table, on every request).
...
**Durable grant persistence remains a future Phase 5.x dependency (DEP-6B-01, §36.3), unchanged by this section** — this section fully specifies **runtime authorization** using the same interim Redis-backed grant record already described in §18.3/§21.4/§24, adding only the fields runtime validation needs (`admin_user_id`, `session_id`, `organization_id`, `expires_at`, a released flag) to that existing Redis record shape — no new storage technology, no Phase 5 table, no schema change. **⚠ CORRECTED — see §18.3b:** DEP-6B-01 is RESOLVED (Phase 5L); this six-check contract's Redis fields now mirror durable columns on `organization.break_glass_grants` (`087_5B1.sql`/`105_5B4.sql`) rather than existing only in Redis — the runtime contract itself (the six checks, the header, the fail-closed table) is unchanged and still authoritative.

### 18.3b Break-glass durable persistence & Phase 6M additions — Controlled Cross-Phase Amendment (Phase 6M, 2026-09-03)

**Why this section exists.** §18.3 and §18.3a above (and the capability matrix, §36.3) were written describing durable break-glass grant-lifecycle persistence as a future, unbuilt Phase 5.x dependency (DEP-6B-01). That framing was already stale before Phase 6M began: `organization.break_glass_grants` (migration `087_5B1.sql`, Phase 5L, 2026-08-24) durably persists exactly the fields §18.3a's runtime contract needs — **correction (this item originally, incorrectly, listed those fields as `admin_user_id`, `organization_id`, `session_id`, `justification`, `ttl_seconds`, `granted_at`, `expires_at`, `revoked_at`; independent re-verification against `087_5B1.sql`'s actual executed SQL during the Phase 6M STRICT FINAL REMEDIATION pass found the real column list is `id`, `organization_id`, `admin_user_id`, `justification`, `session_id`, `issued_at`, `expires_at`, `status` (`'ACTIVE'|'RELEASED'`), `released_at`, `released_by` — there is no `ttl_seconds`, `granted_at`, or `revoked_at` column: `ttl_seconds` is only ever a parameter to `fn_break_glass_grant`, used to compute the stored `expires_at` at insert time, and release is recorded via `status`/`released_at`/`released_by`, not a single `revoked_at` timestamp; corrected here** — and §36.3's own dependency register (line-item DEP-6B-01) already marked this **RESOLVED** as of that date, pointing to `5B-Identity-Organization-Multitenancy-Security.md`'s Phase 5L amendment. This section is the controlled amendment that (a) makes that resolution consistent everywhere else in this document that still describes the old, stale state (§18.3 items 887/889, §18.3a's steps 4/972 above, and §21.4's endpoint contract below), and (b) records Phase 6M's own further, additive changes layered on top of that same table. Per this document's frozen-document discipline, none of the prose above is silently rewritten — each stale statement is marked `⚠ CORRECTED` in place and points here.

**1. Durable persistence — confirmed resolved, not merely asserted.** `organization.break_glass_grants` is a real, RLS-governed table (Phase 5L), not a design placeholder. Redis (`platform_admin:break_glass:{grant_id}`) remains in the architecture as a deliberate **fast-path cache** in front of it — per-request grant validation (§18.3a steps 4–5) still reads Redis first for latency, exactly as before; the durable table is the system of record a Redis eviction/restart falls back to, and is what makes "the *live* 'is this grant still active' state does not survive independently of Redis" (§18.3, item 2, as originally written) no longer true.

**2. Phase 6M addition — purpose-scoped grants (`purposes`).** `105_5B4.sql` adds a `purposes TEXT[]` column to `organization.break_glass_grants`, changing the security model §18.3/§18.3a describe from "a grant is generically all-powerful within its target tenant" to "a grant authorizes only the specific purpose(s) named at issuance." Allow-listed values: `SUPPORT_GENERAL`, `SUPPORT_BILLING`, `SUPPORT_ORG_LIFECYCLE`, `SUPPORT_QUOTA`, `SENSITIVE_MEDIA_ACCESS`, `SUPPORT_SECURITY_INCIDENT` (`chk_bgg_purposes_allowed`). A dedicated constraint (`chk_bgg_purposes_no_wildcard`) rejects `*`, `ALL`, `ALL_ACCESS`, `SUPER_ADMIN`, and any mixed-array combination containing one of them, **at issuance** (**correction: this item previously also listed `BYPASS_SENSITIVE_CHECKS` among the rejected values and omitted the plain `ALL` value — neither matches the actual `chk_bgg_purposes_no_wildcard` constraint in `105_5B4.sql`, which rejects exactly `'*'`/`'ALL'`/`'ALL_ACCESS'`/`'SUPER_ADMIN'`; corrected here**) — this is the DB-layer enforcement of this platform's standing invariant that a generic break-glass grant must never itself imply sensitive-media access (6D §25, 5B §37.6): reading recording audio or transcript content under break-glass requires a grant whose `purposes` array specifically contains `SENSITIVE_MEDIA_ACCESS`. `purposes` is immutable post-insert (a trigger blocks any `UPDATE` touching it; only the release-lifecycle columns — `status`, `released_at`, `released_by` — may change, **correction: this item previously said "only `revoked_at` may change" — no such column exists; release is recorded via those three columns instead, corrected here**). §18.3a's per-request validation contract (steps 5a–5f) is **extended, not replaced**, by Phase 6M: an elevated request whose target operation requires a specific purpose must additionally verify that purpose is present in `grant.purposes`, failing closed with the same generic `403 BREAK_GLASS_GRANT_INVALID` on mismatch (consistent with §18.3a's stated non-distinguishing-error-body rule) — the six pre-existing checks are unchanged. **⚠ CONTROLLED AMENDMENT AAM-OD-03 (API Authorization Matrix phase, 2026-09-19; owner decision AAM-OD-03, option C) — required purpose for read-oriented break-glass on tenant routes.** No frozen source previously mapped an ordinary tenant-scoped route to a required `purposes` value; this sentence closes that gap additively. It does not change the `X-Break-Glass-Grant` header, the six checks, the fail-closed table, the `105_5B4.sql` allow-list, or any route. For a read-oriented break-glass request (§18.4) to a tenant-scoped `/api/v1` route, `grant.purposes` must contain the purpose mapped to that route: (a) `GET /api/v1/billing/quotas` → `SUPPORT_QUOTA`; (b) every other 6K-owned `GET /api/v1/billing/*` route → `SUPPORT_BILLING`; (c) `GET /api/v1/organizations/{organization_id}` (the organization record and its lifecycle state) → `SUPPORT_ORG_LIFECYCLE`; (d) every other tenant-scoped read (every other `GET`, plus 6I's non-mutating `POST /api/v1/workflows/{workflow_id}/validate`) → `SUPPORT_GENERAL`. A grant carrying several purposes satisfies each route whose mapped purpose it contains. `SENSITIVE_MEDIA_ACCESS` and `SUPPORT_SECURITY_INCIDENT` satisfy no generic tenant read; sensitive media remains reachable only through 6M's dedicated guarded routes. A purpose mismatch fails closed with the same generic `403 BREAK_GLASS_GRANT_INVALID`. Tenant-scoped writes remain outside break-glass scope (§18.4) whatever the purpose. The per-route mapping is tabulated in `API-AUTHORIZATION-MATRIX.md` §9 and §13.

**3. Phase 6M addition — `organization.fn_break_glass_check()` guarded DB function.** `105_5B4.sql` adds a SECURITY DEFINER function performing a durable-table equivalent of several of §18.3a's six checks (existence, expiry, release, purpose) directly at the database layer, returning a plain boolean (never raising) so a caller lacking `is_platform_admin` context gets `FALSE`, not an error — consistent with §18.3a's fail-closed, non-distinguishing posture. This function is available as a second, DB-native verification path; it does **not** replace the application-layer Redis-first contract in §18.3a, which remains the specified runtime mechanism for every elevated HTTP request.

**4. Phase 6M fix — `organization.is_platform_admin()` fail-open defect.** Live adversarial validation during Phase 6M discovered that `001_5B.sql`'s `organization.is_platform_admin()` — `current_setting('app.is_platform_admin', true) = 'true'` — returns SQL NULL (not `FALSE`) for a session that never sets that GUC, and PL/pgSQL's `IF NOT <NULL>` silently treats that as false, skipping the guard's `RAISE EXCEPTION`. This is a pre-existing Phase 5B defect (not introduced by 105_5B4/106_5H3) affecting every guarded function using this pattern, including `087_5B1.sql`'s own break-glass functions and the RLS policies this document's §18.1–18.2 platform-admin model depends on. Fixed via `CREATE OR REPLACE FUNCTION organization.is_platform_admin()` reissued from `106_5H3.sql`, wrapping the comparison in `COALESCE(..., 'false')`, without touching or re-checksumming frozen `001_5B.sql`. This closes a genuine fail-open gap in the DB-layer half of §18.1–18.2's "platform admin base authz" mechanism; it does not change this document's API-layer contract (an unauthenticated/non-platform-admin caller was always supposed to be rejected — the fix restores that guarantee at the DB layer that backs it). Full root-cause narrative, live re-validation evidence, and the fixed SQL: `docs/phase-05-database-design/5K/validation/6M_VALIDATION_REPORT.md`, `5B-Identity-Organization-Multitenancy-Security.md`'s Phase 6M amendment, and `docs/phase-06-api-design/6M-Admin-Platform-APIs.md`.

**5. FURTHER CRITICAL FIX (Phase 6M STRICT FINAL REMEDIATION, `107_5B5.sql`) — `is_platform_admin()` self-forgery defect, and `EXECUTE` narrowing.** Independent freeze-gate re-review of item 4's `106_5H3.sql` COALESCE fix found it correct but insufficient: `COALESCE(current_setting('app.is_platform_admin', true), 'false') = 'true'` still trusts a single application-layer GUC that any ordinary `app_api` session can set for itself (`SET app.is_platform_admin = 'true'` or `SELECT set_config('app.is_platform_admin', 'true', false)`), self-authorizing as Platform Admin with no distinct database credential. `107_5B5.sql` closes this by additionally binding the check to the actual connecting database role:
...
`session_user` (not `current_user`) is required here specifically because `current_user` changes to the owning role inside a `SECURITY DEFINER` function — binding to it would make the check tautologically true for every `SECURITY DEFINER` function owned by `app_platform_admin`, regardless of who actually connected; `session_user` is fixed at connection time and reflects the real authenticated principal. This mirrors the same `session_user`-based rationale already documented in `072_5J.sql`'s `audit.fn_insert_audit_event()`. Because a caller must now hold both the `app_platform_admin` database credential *and* the GUC, `107_5B5.sql` also narrows `EXECUTE` on every Platform-Admin-only function that had been left grantable to `app_api` (`organization.fn_break_glass_check`, both `fn_break_glass_grant` overloads, `fn_break_glass_release`, `fn_platform_suspend_organization`, `fn_platform_reactivate_organization`), and adds an in-transaction `audit.fn_insert_audit_event(...)` call to every guarded mutation function so the state change and its audit record commit atomically (previously the audit write, where it existed at all, was a separate non-atomic application-layer call). **Live PostgreSQL 18 validation of `107_5B5.sql` — including the forgery test battery (A–G) and the 21-item break-glass adversarial matrix required by the Phase 6M remediation spec — is tracked in `docs/phase-05-database-design/5K/MIGRATION_MANIFEST.md`'s Row 107 section and has now completed and passes**, with a fresh `001→107` chain and a separate incremental `104→[fixtures]→105→106→107` chain both run to `EXIT=0` with zero errors and exactly one Alembic head (`107_5B5`) on freshly created PostgreSQL 18 containers. Full defect narrative and SHA-256: that manifest section and `5B-Identity-Organization-Multitenancy-Security.md`'s own Phase 6M amendment, item 5.

**6. Closing note (`108_5B6.sql`) — item 5's "STRICT FINAL REMEDIATION" label predates one further finding.** For currency only (this document does not own the finding or its fix, which are a voice-schema/partition-ACL matter, not an identity/authorization-model matter — see `5B-Identity-Organization-Multitenancy-Security.md`'s own Phase 6M amendment, item 6, and `docs/phase-06-api-design/6M-Admin-Platform-APIs.md` for the authoritative narrative): independent freeze-gate re-review after item 5 found that `107_5B5.sql`'s parent-table `REVOKE`/`GRANT` on `voice.transcript_segments` did not propagate to that table's five pre-existing RANGE-partition children, each a separate physical relation with its own ACL that still carried `018_5C.sql`'s original unrestricted grant — finding **F-26-1**. `108_5B6.sql` closes it by applying the identical column-restricted revoke/grant to every child partition (walking `pg_inherits`), with a self-verifying assertion block. This does not change anything item 5 states about `is_platform_admin()`, `session_user` binding, break-glass `EXECUTE` narrowing, or in-transaction audit atomicity — all of that remains accurate and unchanged. It means only that "STRICT FINAL REMEDIATION" in item 5's heading should be read historically (it was the final remediation *known at that time*), not as a claim that `107_5B5.sql` was the last privilege-hardening migration in the Phase 6M line — `108_5B6` is now the current Alembic head. No table, column, or function this section (§18) describes is altered by `108_5B6.sql`; it touches only `voice.transcript_segments`'s child-partition grants, which §18 never described in the first place.

**Not changed by this amendment:** §18.3a's runtime-authorization contract (the header, the six checks, the fail-closed table, session-binding, multiple-simultaneous-grants policy) is unchanged — it remains the authoritative application-layer mechanism. No new endpoint is added or renumbered. `BREAK_GLASS_GRANTED`/`BREAK_GLASS_RELEASED` (5J `action_kind`) are unchanged and unextended — the `purposes` array is carried in the existing grant event's `resource_snapshot`, not a new action kind (5J §14.3 footnote ✦).

**Trigger:** discovered and resolved during Phase 6M's mandatory live adversarial PostgreSQL validation and its required cross-phase reconciliation of the Phase 5B/5H persistence layer this document's break-glass and platform-admin model depends on.
...
- **Database:** `organization.break_glass_grants` (`⚠ CORRECTED — see §18.3b`: DEP-6B-01 is **RESOLVED**, not an open Phase 5 gap — this row previously said "No durable grant-lifecycle table exists"; that table has existed since `087_5B1.sql`, Phase 5L, and was extended with `purposes` by `105_5B4.sql`), inserted synchronously on grant issuance (`id`, `organization_id`, `admin_user_id`, `justification`, `session_id`, `issued_at`, `expires_at`, `status`, `purposes`). Redis (`platform_admin:break_glass:{grant_id}`, §18.3a/§24) remains the request-path fast-path cache in front of it, storing `admin_user_id`, `session_id`, `organization_id`, `expires_at`, and a released flag (§18.3a). `audit.audit_events` (insert, synchronous).
...
| **Break-glass runtime authorization (corrected — Phase 6M amendment)** | `X-Break-Glass-Grant` header + six-check per-request validation (existence/expiry/release/admin/session/org) extended by purpose-scoping (§18.3b item 2), fail-closed on Redis unavailability, multiple-simultaneous-grants policy | Full — Redis fast-path cache in front of the durable `organization.break_glass_grants` table (`087_5B1.sql`/`105_5B4.sql`), plus `fn_break_glass_check()` (§18.3b item 3) | CONTRACT COMPLETE / SECURITY MODEL COMPLETE | IMPLEMENTATION READY | None | §18.3a, §18.3b |
| **Break-glass durable persistence (corrected — Phase 6M amendment)** | Grant/release *action* audit is full (5J); *live lifecycle state* is now durable | **RESOLVED (Phase 5L, 2026-08-24; extended Phase 6M)** — `organization.break_glass_grants` (`087_5B1.sql`) provides durable grant-state persistence, extended by `105_5B4.sql` with a `purposes` allow-list, wildcard-rejection, and an immutability trigger; Redis is now a fast-path cache, not the sole record | CONTRACT COMPLETE | IMPLEMENTATION READY | None (was DEP-6B-01, §36.3 item 1 — RESOLVED) | §18.3b, §36.3 |
...
|---|---|---|---|---|---|---|---|
| DEP-6B-01 | Durable break-glass grant-lifecycle persistence (target org, justification, TTL, granting admin, active/released state) | Break-glass grant/release *actions* are already durably audited (`BREAK_GLASS_GRANTED`/`RELEASED`, 5J); but the *live lifecycle state* of a grant (is it still active, remaining TTL) has no durable table — only an interim Redis TTL key (§18.3, §24) | **RESOLVED (Phase 5L, 2026-08-24; extended Phase 6M, 2026-09-03)** — `organization.break_glass_grants` (migration `087_5B1.sql`) now provides durable grant-state persistence; Redis remains the fast-path cache. See `5B-Identity-Organization-Multitenancy-Security.md`'s Phase 5L amendment and `5L-Global-Database-Reconciliation.md`. Phase 6M (`105_5B4.sql`) further extends the same table with a `purposes` allow-list (wildcard/superuser rejection at issuance, immutability trigger) and a guarded `fn_break_glass_check()` function; see §18.3b and `docs/phase-06-api-design/6M-Admin-Platform-APIs.md`. ~~API contract complete (§18.3, §21.4); interim Redis mechanism specified; no durable table exists~~ | Future Phase 5.x | Yes | **No** | **No** — RESOLVED; durable grant-state persistence, purpose-scoping, and a DB-layer check function all exist as of Phase 5L/6M |
...
1. ~~**Durable break-glass grant-lifecycle persistence** (DEP-6B-01, §18.3/§18.3a/§21.4/§36.3) — the API contract, the now-fully-specified runtime authorization model, and grant/release action-audit are complete and implementable today; only the durable *lifecycle-state* table is a future Phase 5.x item.~~ **RESOLVED (Phase 5L, 2026-08-24; extended Phase 6M, 2026-09-03) — see §18.3b.** `organization.break_glass_grants` (`087_5B1.sql`, extended `105_5B4.sql`) is a durable table; this is no longer a forward dependency. Retained here, struck through, only as a historical record of what this item said before the correction — not as an open item.
...
PHASE 6B IMPLEMENTATION READINESS:           CONDITIONAL — UPSTREAM DEPENDENCIES REMAIN
                                              (DEP-6B-01 break-glass grant persistence — RESOLVED,
                                               Phase 5L/6M, see §18.3b; retained in this list only
                                               for historical continuity with the register's numbering,
                                               ...
                                               see §36.3 for the full 8-item register (now 7 open items);
                                               none of the remaining items blocks architecture/contract/security-design completeness;
...
## 39. Controlled Cross-Phase Amendment — Phase 6M Admin/Platform Control-Plane (2026-09-03)

**Scope of this amendment (fifth correction pass on this document; §38's fourth pass is unaffected and not reopened).** While designing `6M-Admin-Platform-APIs.md`, the Phase 6M pass found that this document's break-glass sections (§18.3, §18.3a) and its own dependency register (§36.3) disagreed with each other: §36.3's DEP-6B-01 row already read **RESOLVED (Phase 5L, 2026-08-24)**, citing `organization.break_glass_grants` (`087_5B1.sql`), while §18.3/§18.3a's narrative text and the capability matrix (§36.2) still described durable grant-lifecycle persistence as absent and DEP-6B-01 as open. This was a pre-existing internal inconsistency, not something Phase 6M's own additive changes introduced — but Phase 6M also layers three genuinely new changes onto the same table (`105_5B4.sql`'s `purposes` allow-list, `fn_break_glass_check()`, and a critical `is_platform_admin()` fail-open fix issued from `106_5H3.sql`), so both had to be reconciled together. Applied this pass:

1. **New §18.3b subsection** — the authoritative correction record: confirms durable persistence is resolved (Phase 5L), and documents Phase 6M's three additions (purpose-scoped grants, `fn_break_glass_check()`, the `is_platform_admin()` COALESCE fix) with full rationale.
2. **Inline `⚠ CORRECTED` markers** added in place at §18.3 (items 1–2) and §18.3a (step 4, and the closing "Durable grant persistence remains a future..." paragraph) — the original stale prose is left legible (frozen-document discipline: no silent rewrite) with a pointer to §18.3b for the corrected state.
3. **Capability matrix (§36.2)** — the "Break-glass runtime authorization" and "Break-glass durable persistence" rows rewritten to state RESOLVED and cross-reference §18.3b.
4. **Dependency register (§36.3)** — DEP-6B-01's "Current status" and "Blocks 6B implementation?" cells updated to record the Phase 6M extension alongside the pre-existing Phase 5L resolution.
5. **§38.2 item 1 and the §38.3 final-status code block** — struck through / annotated to stop presenting DEP-6B-01 as an open forward dependency, without deleting the historical record of what those blocks said before this correction.
6. **§18.3b item 6 (added during the Phase 6M stabilization/completion pass, 2026-09-05)** — a short closing note recording that `108_5B6.sql` (finding F-26-1, a `voice.transcript_segments` child-partition ACL bypass) postdates item 5's "STRICT FINAL REMEDIATION" label, so that label is not misread as claiming `107_5B5` was the last Phase 6M privilege-hardening migration. The finding and fix belong to the voice schema, not to this document's identity/authorization model, and are not otherwise detailed here.

**Explicitly confirmed for this amendment:** no endpoint was added, removed, or renumbered (still 36); §18.3a's six-check runtime-authorization contract (header, checks, fail-closed table, session-binding, multiple-simultaneous-grants policy) is unchanged, only extended with purpose-scoping as an additional check; no other section (§7–§17, §19–§35, §37) was touched; RBAC (§8), tenant isolation (§9), refresh-token CAS (§13.2), MFA (§10–§11, §15.3), and the central internal token issuer (§17.2) are unaffected. This amendment does not reopen or contradict the fourth pass's (§38) own conclusions — it corrects a narrower, independently-discovered staleness in the break-glass persistence narrative and layers in Phase 6M's genuinely new additions on top of an already-resolved dependency.

**Trigger:** discovered during Phase 6M's mandatory cross-phase controlled reconciliation of the Phase 5B/5H persistence layer this document's break-glass and platform-admin security model depends on. Full detail: `docs/phase-06-api-design/6M-Admin-Platform-APIs.md`, `docs/phase-05-database-design/5B-Identity-Organization-Multitenancy-Security.md`'s Phase 6M amendment, `docs/phase-05-database-design/5K/validation/6M_VALIDATION_REPORT.md`.
