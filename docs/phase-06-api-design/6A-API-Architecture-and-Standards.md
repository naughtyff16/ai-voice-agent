...
| **Signed/presigned media URL handling (added — Phase 6L freeze-gate remediation, security hardening)** | A signed/presigned recording or media URL (and any of its query-string parameters — signature, expiry, credential scope) is a **sensitive temporary bearer capability**, not an ordinary reference value, and the generic word-based redaction pattern above (`phone_number\|email\|token\|password\|secret`) is **not relied upon alone** to catch it — a presigned query string does not necessarily contain any of those literal words. It **must never** be written into: application logs, structured logs, trace/span attributes, metric labels, audit `resource_snapshot` payloads, exception/error payloads, or support/debug logs. Explicitly prohibited as a logged/traced value, by name: `download_url`, `upload_url`, `presigned_url`, any signature/`X-Amz-Signature`-style query parameter, storage credentials, and any other temporary security token. Any endpoint whose response body contains such a URL (e.g. 6D §16.2's `GET /recordings/{id}/download-url`) has **request/response body logging disabled** — the OTel/`structlog` pipeline for that endpoint class captures method/path/status/latency/request_id only, never the body, closing the gap a body-capturing debug log would otherwise create regardless of field-level redaction. The one legitimate place the URL exists is the single JSON response returned to the already-authorized caller — nowhere else, ever. |
...
**Decision (ADR-6A-09, §39 — FINAL):** this is the Phase 6A-approved, binding mechanism for internal service-to-service authentication, not merely a recommendation pending review. It is finalized directly in this document, rather than left open for a later phase, for three reasons specific to this mechanism (which do not apply to ADR-6A-05): (1) it introduces no Phase 5 schema change and no Phase 5 conflict — unlike WebSocket/Socket.IO, no frozen document proposes a competing mechanism for Phase 6A to override, so there is nothing to reconcile against, only a genuine gap to close; (2) it is fully self-contained within the API/auth layer (signature verification + an existing audit `actor_type` vocabulary, 5J §14.1) and reversible without a data migration if a future phase (e.g. Phase 8, Authentication & Authorization) later prefers mTLS or a service-account table — changing it later touches signing/verification code, not stored data; (3) 6B onward cannot design any internal (`/api/internal/v1/...`, §8.5) endpoint without *some* concrete answer here, and Phase 6A's purpose (§2) is precisely to settle cross-cutting questions like this once so 6B doesn't re-litigate them per-endpoint. 6B treats this as a fixed, controlled dependency: every internal endpoint it designs must authenticate via this mechanism as specified, and any future revision to it (e.g. by Phase 8) is an explicit, documented amendment to this ADR, not a silent per-endpoint deviation.
...
**Decision (ADR-6A-05, §39 — FINAL):** raw WebSocket (native FastAPI, `AsyncIO`) is the platform standard for **all** realtime channels, not just voice. Voice already implements this (3B §17–18); Phase 6A binds the identical protocol/infra to non-voice realtime (campaign progress, notifications, workflow execution progress) rather than standing up Socket.IO as a second realtime stack on the backend.

**Reconciliation with `TECH_STACK.md` (not a silent override):** `TECH_STACK.md` lists both `Socket.IO` (Frontend) and `WebSockets` (Backend) as approved — it never states which backend protocol Socket.IO-on-the-frontend is meant to talk to, and no document in Phase 1–5 ever specifies a Socket.IO *server*. The only backend realtime component either Phase 2 or Phase 3 actually designs is the Realtime Voice Gateway, and it is specified as raw `FastAPI + WebSockets` (HLA §7.13/component table), not Socket.IO. Phase 3A itself identified this exact ambiguity and explicitly assigned its resolution to this document: *"A vanilla Socket.IO client cannot talk to a raw WebSocket server... This needs to be pinned down explicitly by Phase 6 (API Design)"* (3A §3, line 53). Phase 6A is therefore not overriding an approved decision — it is exercising the resolution authority Phase 3A already delegated to it, for a question `TECH_STACK.md` itself left open. Resolution: the platform stands up exactly one backend realtime protocol (raw WebSocket, `/ws/v1/...`), for the reasons below, and does not run a second, Socket.IO-protocol (Engine.IO handshake) server. `TECH_STACK.md` is not edited by this document (out of this document's authority, §3); the frontend consumes `/ws/v1/...` via the standard browser `WebSocket` API (or an equivalent thin client), not `socket.io-client`, since there is no Socket.IO server on the other end for it to negotiate with.

Rationale for one protocol, not two: a single auth/tenant-isolation/observability path instead of two (§9.1, §23, §25 would otherwise need to be duplicated for a second stack); Socket.IO's long-polling fallback and room abstraction solve problems the platform doesn't have — connections are already tenant-scoped and cross-tab broadcast is already served by Redis pub/sub + per-tenant Redis Cluster hash-tagging (3F §16.2); and the platform's only production realtime implementation to date (Voice Gateway, 3B) already committed to raw WebSocket, so a second protocol would be inconsistency for its own sake, not a technical need. This closes the gap Phase 3A flagged; it is binding on 6B onward as written, with no further sign-off required before use.
...
| ADR-6A-05 | Raw WebSocket for all realtime, not just voice | Socket.IO backend for browser-facing channels (per `TECH_STACK.md`'s frontend entry) | Phase 3A (line 53) explicitly delegated this exact resolution to Phase 6A; no document ever specifies a Socket.IO backend, only the raw-WebSocket Voice Gateway (HLA §7.13); one auth/tenant/observability path instead of two (§27.1) | **Decided (Final)** |
...
| ADR-6A-09 | Service-to-service auth via short-lived internal-signed JWT, no new DB table | mTLS between services; new service-account table (would modify Phase 5) | No Phase 5 table exists for this and none is created; a signature-verified JWT needs no DB lookup and reuses existing `actor_type` audit vocabulary (§23.4); schema-free and reversible, no competing frozen decision to override | **Decided (Final)** — binding on 6B as a fixed dependency; revisable only via an explicit future ADR amendment (e.g. by Phase 8) |
...
|---|---|---|---|
| R-1 | **RESOLVED (this revision).** WebSocket vs. Socket.IO. `TECH_STACK.md` lists Socket.IO under Frontend with no backend counterpart ever specified; Phase 3A (line 53) flagged the ambiguity and explicitly delegated its resolution to Phase 6. ADR-6A-05 (§27.1, §39) now finalizes it: one backend realtime protocol (raw WebSocket, `/ws/v1/...`) for every channel, voice and non-voice alike; no Socket.IO server is stood up. `TECH_STACK.md` itself is not edited (out of this document's authority). | Determines the WS client library, reconnect/backoff implementation, and connection contract for every future realtime channel 6B+ designs. | Closed. Frontend consumes `/ws/v1/...` via the standard `WebSocket` API, not `socket.io-client`. Any future desire to reintroduce Socket.IO on the backend is a new ADR against this one, not a default. |
| R-2 | **RESOLVED (this revision).** Internal service-to-service auth. No prior frozen document specified a mechanism. ADR-6A-09 (§23.4, §39) finalizes a stateless, short-lived internal-signed JWT, schema-free, mapped onto the existing `actor_type` audit vocabulary (5J §14.1). | Determines how Worker/Voice-Gateway/Core-API internal calls (added in 6B+) authenticate. | Closed as a fixed dependency binding on 6B. Because it is schema-free and reversible without a data migration, it does not require pre-implementation sign-off from a not-yet-existing security-owner role; a future Phase 8 (Authentication & Authorization) document may amend it via an explicit ADR if warranted, but 6B may build against it now. |
...
All content-completeness criteria are satisfied. All ten ADRs (§39), including ADR-6A-05 and ADR-6A-09 — the two that previously carried REVIEW REQUIRED status — are now finalized as **Decided**, per the reconciliations in §23.4 and §27.1 and the resolutions recorded at R-1/R-2 (§40). No section was left incomplete or reopened to reach this state.

---

## 42. Final Approval Status

### PHASE 6A STATUS: **APPROVED / FROZEN**

All 42 required content sections are complete, internally consistent, and traceable to Phase 1–5 source documents with citations. Phase 5 was not modified. No business-domain endpoints were designed. The two decisions previously held at REVIEW REQUIRED have both been legitimately finalized from the existing architecture, not by fabricated approval:

1. **ADR-6A-05** (raw WebSocket, platform-wide, §27.1) — reconciled against Phase 2 HLA (the only backend realtime component ever specified, the Voice Gateway, is raw WebSocket) and against Phase 3A, which explicitly delegated this exact resolution to Phase 6 (3A §3, line 53). `TECH_STACK.md` is not edited or silently overridden — its Frontend `Socket.IO` entry stands untouched; this document only settles which backend protocol realtime channels use, a question `TECH_STACK.md` never answered.
2. **ADR-6A-09** (internal service-to-service JWT, §23.4) — finalized because it introduces no Phase 5 change, conflicts with no existing frozen decision (there was none to reconcile against, only a gap to close), and is schema-free and reversible without a data migration if amended later.

Both ADRs are now **Decided** (§39) and binding on 6B onward. This document is re-issued as **APPROVED / FROZEN** effective this revision. No further sign-off gates 6B from proceeding.
...
- R-1: WebSocket vs. Socket.IO — **resolved this revision**: raw WebSocket, platform-wide (ADR-6A-05).
- R-2: Service-to-service auth mechanism — **resolved this revision**: internal-signed JWT (ADR-6A-09), binding on 6B as a fixed dependency.
...
- R-1 and R-2, previously the primary risk to 6B+ velocity, are closed as of this revision — both foundational auth/realtime decisions are now final and 6B can build against them without a pending external gate.

**10. Conflicts discovered with Phase 1–5**
- `TECH_STACK.md` (Socket.IO, Frontend) vs. the Voice Gateway's already-implemented raw WebSocket (3B) — not a contradiction within Phase 1–5 itself (TECH_STACK.md is frontend-general, 3B is voice-specific backend), but a gap Phase 6 was explicitly tasked (3A line 53) with resolving; this document now resolves it (ADR-6A-05, §27.1) as the designated resolution point Phase 3A named, without editing or overriding `TECH_STACK.md` itself.
