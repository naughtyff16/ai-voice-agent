...
> **API-DESIGN DEPENDENCY / FUTURE DECISION** — the owning Phase 6B+ domain document must either point the job endpoint at an existing table or specify a lightweight Celery-result-backed job record; this document does not create one. **⚠ CONTROLLED AMENDMENT AAM-OD-04 (API Authorization Matrix phase, 2026-09-19; owner decision AAM-OD-04, option C) — authorization of `GET /api/v1/jobs/{job_id}`:** (1) authentication, session/membership validity, principal-type eligibility and tenant-context checks run first, exactly as for any tenant route; (2) a job whose organization is not the caller's organization is `404 RESOURCE_NOT_FOUND`, identical to a nonexistent job; (3) access is then granted if and only if the caller is the job's **server-recorded initiator** (an exact match on the principal type and principal id recorded by the server when the job was created) **or** the caller holds the job type's **server-defined `view_permission`**, which the owning domain contract fixes per job type; otherwise `403 AUTHORIZATION_DENIED`; (4) being the initiator bypasses no session, membership, tenant-isolation, principal-type or global control; (5) a platform-admin, API-key, internal or other principal gains nothing from an ambiguous initiator comparison — the comparison matches only an identical principal type and id; (6) the initiator id, resource type, required permission and authorization policy are server-generated and server-stored when the job is created, and are never taken from, or inferred from, client input; (7) there is no fallback to a broad generic permission. Where a job type records no initiator (of the tables named above, only `csv_import_job` records `created_by`), only that job type's `view_permission` applies; if the owning domain contract defines neither, access is denied.
