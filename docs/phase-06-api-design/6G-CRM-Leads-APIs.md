...
| Status of dependencies | Phase 5 (5A–5J, 5K, 5K.1, 5L) is **APPROVED / FROZEN**. SQL/Alembic head at Revision 1 = `092_5F12`; this Revision 2 required — and live-validated — four controlled forward migrations, new head = `096_5B2` (Phase 6G CRM Reconciliation, 2026-08-28; see §5a). 6A–6F are **APPROVED / FROZEN** and remain untouched. |
| Revision | **Revision 2** (2026-08-28) — supersedes Revision 1 in place. Triggered by an independent cross-phase reconciliation review that found three genuine physical defects and one real least-privilege gap in Revision 1; all four are resolved via new migrations `093_5D2.sql`–`096_5B2.sql`, live-validated (§5a). No 6A–6F change; 6H not started. |
...
| `docs/phase-05-database-design/5K/migrations/019–026_5D.sql`, `085_5D1.sql`, `093_5D2.sql`–`096_5B2.sql` | **Executed migration — wins over prose on any conflict.** The last four are this revision's own controlled amendment (§5a). |
...
### 5a. Phase 6G CRM Reconciliation — Revision 2 Amendment Summary

Four new forward migrations were added on top of the Revision 1 baseline (head `092_5F12`), triggered by an independent cross-phase reconciliation review. No 5A–5J table, column, constraint, index, function, or grant described in Revision 1 was altered — every change is additive, and no privilege was widened on any append-only table. All four were live-executed (fresh-DB `001_5B → 096_5B2` and existing-DB `092_5F12 → 096_5B2`, both exit code 0, single head `096_5B2`) against a disposable local PostgreSQL 18 database, including genuine multi-connection concurrency races, not simulated sequentially.

| Migration | Phase | Resolves | Physical change |
|---|---|---|---|
| `093_5D2.sql` | 5D.2 | DEP-6G-01 | `crm.contacts.merged_into_contact_id`/`merged_at`, two guard triggers, `crm.fn_merge_contacts()` |
...
Full rationale, DDL, and live-validation transcripts: `MIGRATION_MANIFEST.md`'s "Phase 6G CRM Reconciliation" entry, `5D-CRM-Schema.md`'s matching amendment section, `5B-Identity-Organization-Multitenancy-Security.md`'s matching amendment section, `5J-Analytics-Audit-Schema.md` §14.3's ¶ amendment, and `5L-Global-Database-Reconciliation.md`'s "Phase 6G CRM Reconciliation" section.
...
10. Records the marker Activity on `primary` (§10.2).
...
| Merge | Both contacts exist, not already merged/erased, not cross-tenant | `crm.fn_merge_contacts()` — locks both Contact rows (deterministic order), field-fill, repoints Deals/Tasks/Notes/Appointments, marks secondary, inserts marker Activity — all inside the function's own transaction | **Yes** (`crm.fn_merge_contacts()`, `093_5D2.sql` — centralization/atomicity, not privilege elevation) | Function return | — | `contact.merged` | `CONTACT_MERGED` (async) |
...
List endpoints universally use summary DTOs (6A §10.2). GDPR-erased/tombstone placeholder values are never rendered as if genuine (§8.4).
...
|---|---|---|---|---|---|
| DEP-6G-01 | Full 4C merge fidelity requires re-pointing `crm.activities`/`crm.lead_score_records`, but `REVOKE UPDATE` on both makes this physically impossible for any application role | 4C §6.2 vs. `022_5D.sql`/`023_5D.sql` | Resolved by reconciling the domain rule (application-layer lineage read, §10.5), not by widening the privilege: `crm.fn_merge_contacts()`, `093_5D2.sql` | **RESOLVED** | `POST /contacts/{id}/merge` |
...
**Zero BLOCKING items remain.** DEP-6G-01, DEP-6G-06, DEP-6G-07, and DEP-6G-10 — the four items this reconciliation targeted — are all **RESOLVED**, live-validated, and reflected consistently throughout this document (§10, §12.2, §19, §29.2). DEP-6G-02/03/04/05 are closed by explicit classification (not every terminology gap warrants a new permission — §5 findings 3–4) or remain honestly NON-BLOCKING because the gated capability is simply not exposed. DEP-6G-08/09/11–16 are cross-phase handoffs or already-closed items, not gaps in this document.
...
| 35 | No Phase 5 changes made | ✅ — Revision 1's 5A–5J baseline is untouched; this revision's own four migrations (`093_5D2.sql`–`096_5B2.sql`) are additive-only forward migrations, live-validated, following the exact controlled-amendment discipline already established by `085_5D1`/`077_5J1`/`087_5B1` |
| 36 | 6A–6F untouched | ✅ |
| 37 | 6H not started | ✅ |

### 43.2 Recommendation

Revision 1 identified one genuine BLOCKING item (DEP-6G-01) and three NON-BLOCKING gaps that an independent reconciliation review determined were worth closing rather than carrying forward (DEP-6G-06, DEP-6G-07, DEP-6G-10). All four are now **RESOLVED**, each via the smallest correct fix: a physical-schema amendment reconciling a domain rule with an immutability invariant (merge), a documentation-only governance amendment (audit vocabulary), locating an already-existing field (currency), and one narrowly-scoped new permission (custom fields). Two further defects were found and fixed *during* this reconciliation, before either was ever left in a broken state: a `search_path` omission reproducing a previously-documented class of bug, and an accepted-as-risk lead-score race that is now genuinely resolved rather than merely documented. Every fix was live-validated against a real, disposable PostgreSQL 18 database, including genuine multi-connection concurrency races — not asserted from design reasoning alone. No privilege was widened on any append-only table; Activities, LeadScoreRecords, and consent records remain exactly as immutable as Revision 1 left them; suppression enforcement remains exactly as authoritative. 6A–6F remain untouched; 6H was not started.
