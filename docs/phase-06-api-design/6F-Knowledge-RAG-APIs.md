...
|---|---|---|---|
| `DEP-6F-01` | FR-RAG-004 true historical rollback (re-activating a `SUPERSEDED` version) | **BLOCKING** | `fn_docver_publish()`'s `READY`-only precondition provides no path back from `SUPERSEDED` (verified against executed `034_5F.sql`). Requires either (A) product/architecture governance formally accepting forward-only publication as FR-RAG-004's satisfying interpretation, or (B) a Phase 5 amendment adding a physical rollback mechanism (§40-A). Scoped to this sub-capability only — does not block the other 18 implementation-ready endpoints — but **does** block Phase 6F's overall freeze per §43, because FR-RAG-004 is a named SRS requirement this document cannot claim closed while unresolved. |
| `DEP-6F-02` | KB-level full reindex of already-`READY` document content | **BLOCKING** | No `fn_docver_reindex()`-equivalent exists in `034_5F.sql`–`044_5F.sql`; re-ingesting identical content under a new version collides with `uq_dv_content_hash`; no dual-generation index representation exists (§22.2). The endpoint is withdrawn from the V1 implementable surface entirely (§22.3) — not offered in any form. Blocks Phase 6F's overall freeze per §43. |
| `DEP-6F-03` | Seven proposed `action_kind` values (§30.2, count corrected from the prior revision's "six") | NON-BLOCKING for 6F's technical execution; **required precondition for governance-complete audit before final freeze** | `chk_ae_action_kind` physically accepts any string of valid length (no SQL change needed) — but physical acceptance ≠ governance approval. A controlled 5J vocabulary amendment (identical precedent to 5J.1 for 6C/6D) is required before these seven values are sanctioned for use. See §44. |
...
| `DEP-6F-09` | No `SECURITY DEFINER` path exists to set `document_versions.status='FAILED'` | **BLOCKING** | Verified directly against `036_5F.sql` (`REVOKE UPDATE, DELETE ON document_versions FROM app_api, app_worker`, unconditional) and `034_5F.sql` (only `fn_docver_mark_ready()`/`fn_docver_publish()` exist — neither transitions to `FAILED`). Blocks `POST .../documents/{id}/reprocess` (§15) entirely — not merely under contention, but as a normal-path consequence of any first-ingestion failure. Requires a Phase 5 amendment: a `fn_docver_mark_failed()`-equivalent `SECURITY DEFINER` function. Blocks Phase 6F's overall freeze per §43. |
...
| `DEP-6F-14` | **New this pass.** Knowledge-base-wide duplicate-content invariant (`NoDuplicateDocumentContent`, 4E §10) is not enforced by the executed schema | **BLOCKING** | `uq_dv_content_hash` is executed as `(document_id, content_hash)` (`036_5F.sql`, with an explicit in-file correction comment), not `(knowledge_base_id, content_hash)` as the 5F design document and 4E's policy describe — `document_versions` has no `knowledge_base_id` column to scope against. Two different documents in the same KB with identical content are not rejected by any DB constraint. Requires a controlled Phase 5F reconciliation to decide the correct physical enforcement mechanism (e.g., an application-layer pre-check query, or a schema change adding a scoping column) — 6F does not invent or presume the resolution. Blocks Phase 6F's overall freeze per §43. |
| `DEP-6F-15` | **New this pass.** GDPR `DocumentVersion` erasure lifecycle function missing | **BLOCKING** | No `fn_docver_gdpr_erase()` (or equivalent) exists anywhere in `034_5F.sql`–`044_5F.sql`; `document_versions` `UPDATE`/`DELETE` is unconditionally revoked from `app_api`/`app_worker` (`036_5F.sql`). Blocks `DELETE .../documents/{document_id}` (§23.4) entirely — the action is held blocked rather than run as a partial erasure. Requires a Phase 5 amendment providing the missing `SECURITY DEFINER` function. Blocks Phase 6F's overall freeze per §43. |
| `DEP-6F-16` | **New this pass.** `fn_docver_publish()` lacks a guard against publishing onto an already-`DELETED` document | **BLOCKING** (for closing this specific race only) | Verified against executed `034_5F.sql`: the function checks `version_id`/`document_id`/`organization_id`/`status='READY'` but not `documents.status`. A concurrent/late-committing publish can revive a deleted document's `current_version_id`/`status`. Requires a Phase 5 amendment adding a `documents.status != 'DELETED'` precondition inside the function. Does not block ordinary, non-racing publish (§12.2 remains implementation-ready) but is a genuine, disclosed integrity gap, not merely an "accepted narrow race" — recorded as BLOCKING for the specific guarantee it fails to provide, and contributes to Phase 6F's overall freeze gate per §43. |

### 39.1 Blocking / Non-Blocking / Deferred Counts

| Status | Count | IDs |
|---|---|---|
| BLOCKING | 6 | `DEP-6F-01`, `DEP-6F-02`, `DEP-6F-09`, `DEP-6F-14`, `DEP-6F-15`, `DEP-6F-16` |
| NON-BLOCKING | 6 | `DEP-6F-03`, `DEP-6F-04`, `DEP-6F-05`, `DEP-6F-06`, `DEP-6F-07`, `DEP-6F-08` |
