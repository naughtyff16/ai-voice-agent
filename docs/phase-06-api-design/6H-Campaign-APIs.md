...
Revisions 4–7 of this document (§5, findings 22–28; §5b–§5d and the Revision-4/6/7 rows in the Document Control table above) repeatedly describe PostgreSQL 16 / 16.10 as "the declared production baseline" and validate their fixes against it. That phrasing was accurate **at the time those passes were run**, per the governing remediation task's own explicit instruction in force for Revisions 4–7 (cited in this document at DEP-6H-22/DEP-6H-26 and the "why a fourth pass" rationale) — it is preserved unedited everywhere it appears as the historical record of what was actually validated and why, consistent with this document's own stated policy (§5, preamble) of never rewriting a prior pass's disclosure after the fact.

It is not, however, a current statement of the platform's production database engine. Per `MIGRATION_MANIFEST.md`'s "PostgreSQL 18 Baseline Reconciliation" entry and every Phase 6 document authored after it (6K §8, 6L §19/§57, 6M §Document Control), **PostgreSQL 18 is the platform's sole authoritative production baseline as of this reconciliation pass** — an engine-version declaration only, with no DDL, grant, or contract implication for anything this document specifies. This is not a new fact for 6H specifically: Revisions 1–3 (§5a) were already validated on PostgreSQL 18 independently of the later PG16-targeted passes, so every guarantee this document makes has always had PostgreSQL 18 live evidence behind it; nothing in `098_5E1.sql`/`099_5C1.sql` is PG16-specific or requires re-validation.

No sentence elsewhere in this document is edited by this amendment. Where the phrase "the declared production baseline" (or equivalent) appears attached to a PostgreSQL 16/16.10 validation run, it should be read as *the baseline declared for that specific remediation pass, per that pass's governing instruction* — historical validation evidence, not a live claim superseding the PostgreSQL 18 baseline stated here.
...
| `docs/phase-06-api-design/6C-Core-Platform-APIs.md` | `GET /api/internal/v1/organizations/{organization_id}/compliance-policy` — the org-level calling-window/consent-purpose ceiling read 6H's scheduling validation consumes |
...
| 48 | Every new `SECURITY DEFINER` function has the minimal `search_path` its actual dependencies require, verified against `pg_proc`, not assumed | ✅ §5 finding 20, §49.5 — direct `pg_proc`/`information_schema` inspection of all 13 functions (3 `campaign.fn_*` + 10 `voice.fn_*`), re-confirmed current as of Revision 7 |
...
| 50 | No role can bypass a guarded state machine via a direct table write | ✅ Final Blocker Remediation Blockers B/C, extended by Final Admin-DML Hardening finding 28 — direct mutation privileges are removed from all normal runtime roles, including `app_platform_admin`, on `campaign.campaign_contact_identities` and `voice.call_dispatch_keys`; safety-critical mutations occur only through approved guarded functions; migration-owner/superuser capabilities remain outside the normal runtime authorization model (INV-ADMIN-06), §49.1, §49.2a, §49.9c — **live-proven on PostgreSQL 16** |
