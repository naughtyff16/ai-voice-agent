...
| Status of dependencies | Phase 5 (5A–5J, 5K, 5K.1, 5L) is **APPROVED / FROZEN**. Alembic head at time of writing: `097_5D5`. Campaign schema (`campaign.*`) is exactly migrations `027_5E.sql`–`033_5E.sql` — no later migration (034–097) touches the `campaign` schema. 6A–6G are **APPROVED / FROZEN** and are treated as binding, unmodified inputs. |
...
| Revision | **Revision 3** (2026-08-28) — supersedes Revision 2 in place. Revision 2 found and resolved three genuine concurrency/idempotency defects (CampaignContact duplicate-enqueue; Pause/Stop-vs-dispatch race; Campaign→Voice dispatch idempotency) via two new migrations, but disclosed that neither had been live-executed. **This revision closes that gap and, in doing so, found three further genuine defects by live adversarial testing**: (a) a SECURITY DEFINER `search_path` bug that would have made both migrations fail on first real execution; (b) a provider-dispatch durability hole in Revision 2's own Voice fix — a worker crash between reserving a call and invoking the telephony provider would have permanently lost the call; (c) two genuine cross-tenant/cross-campaign ownership-verification gaps in the Revision-2 functions themselves, found by live probing, not inspection. All six defects (three from Revision 2, three found in this revision) are now closed and **live-validated**: a real, disposable local PostgreSQL 18 database, a genuine `uv`-managed Python/Alembic/SQLAlchemy/psycopg environment, a full fresh-database `alembic upgrade head` (001→099), an incremental `097_5D5 → head` upgrade, direct `pg_proc`/`information_schema` inspection of every new function's `SECURITY DEFINER`/`search_path`/grants, and genuine multi-connection concurrency races (not simulated) proving CampaignContact uniqueness, dispatch-reservation exclusivity, the Pause-vs-dispatch serialization (both orderings), Voice dispatch-key exclusivity, provider-submission-claim exclusivity, crash-before-provider recovery, `AMBIGUOUS`-blocks-retry, `FAILED`-is-retryable, duplicate-task-redelivery safety, and post-fix cross-tenant rejection. Full transcript: §49. No 6A–6C/6E–6G content changed; 6D carries the same one narrow, now-updated, labeled amendment (§28.10a). |
| Date | 2026-08-28 |
...
### 18.4 In-Process Voice Idempotency AND Provider-Dispatch Durability — Blocker #3 and Blocker C, Resolved
...
**Resolved together** by a controlled, additive amendment to `6D-Voice-Call-Agent-APIs.md` §28.10a and `099_5C1.sql` (Phase 5C.1) — a genuine claim/lease/outcome protocol, not a single boolean flag. Three steps, two transactions plus the external call between them:

```sql
-- Step 1: reserve the logical call identity (idempotent; safe to call any number of times)
SELECT call_session_id, session_started_at, is_new
...
SELECT claimed, call_session_id, attempt_count, reason
FROM voice.fn_claim_dispatch_for_provider_submission(
  p_dispatch_idempotency_key => $ikey, p_organization_id => $org_id,
  p_worker_id => $worker_id, p_lease_seconds => 30, p_provider_request_ref => $ref
);
-- COMMIT. If claimed = FALSE: STOP — do not call the provider (see reasons below).

-- Step 3 (only if claimed = TRUE): TelephonyPort.place_call() OUTSIDE any transaction, then
-- exactly one of:
...
`voice.call_sessions` is itself `PARTITION BY RANGE (started_at)` (`011_5C.sql`) — the identical structural reason `campaign_contacts` cannot carry a direct `UNIQUE` constraint applies here to a dispatch key. `voice.call_dispatch_keys` (`099_5C1.sql`) is a small, non-partitioned table carrying both the `PRIMARY KEY (dispatch_idempotency_key)` claim (Step 1) *and* a `dispatch_state` column (`RESERVED | CLAIMED | CONFIRMED | AMBIGUOUS | FAILED`) tracking the provider-submission attempt itself (Steps 2–3) — deliberately independent of `voice.call_sessions.status` (the call's own conversational/telephony lifecycle, unchanged, owned by 6D).

**Caller contract, precisely:**
- Step 2 `claimed = TRUE` → this worker, and only this worker, may call `TelephonyPort.place_call()` **outside any transaction** (6A §35, unchanged), then record exactly one Step-3 outcome based on what actually happened.
- Step 2 `claimed = FALSE, reason = 'NOT_CLAIMABLE_CLAIMED'` → another worker currently holds a valid lease on this exact dispatch — do nothing; a legitimate concurrent attempt, not an error.
...
- A **stale (lease-expired) `CLAIMED`** row, or a `FAILED` row, is transparently re-claimable by the *next* call to Step 2 (by this worker or a different one) — this is what makes crash-before-provider-submission recoverable rather than a permanent loss.

**Live-proven, not merely designed (§49 has full transcripts):**
- Two concurrent Step-1 calls with the identical key produced exactly one `call_sessions` row.
- Two concurrent Step-2 claims with the identical key produced exactly one claimant.
- A Step-2 claim with a 5-second test lease, deliberately never followed by a Step-3 call (simulating a crash), was safely re-claimed by a *different* worker once the lease genuinely elapsed, and successfully confirmed — the call was not lost.
- An `AMBIGUOUS` outcome was proven to reject every subsequent claim attempt, with no lease-expiry escape hatch — a hard stop, exactly as designed.
- A `FAILED` outcome was proven safely re-claimable — the intended asymmetry against `AMBIGUOUS` is real, not just documented.

**What remains an accepted, external-system limit — stated honestly, not closed by this fix:** whether the underlying telephony *provider* actually received and acted on a single `TelephonyPort.place_call()` network call whose response leg was lost is not resolvable by any idempotency key on the platform's own side alone — no key exchanged only between Campaign and Voice can reach into the provider's own state. This is bounded by 6D's pre-existing provider-retry contract (3B §19) and, where the active provider adapter supports echoing a caller-supplied reference on callbacks, a `provider_request_ref`-based reconciliation window — a disclosed dependency on the provider-adapter layer, **not assumed universally true of every provider** (verify against the actual Exotel/other adapter contract before relying on it; 3B/6D do not currently document Exotel supporting this). This fix eliminates *platform-internal* double-dispatch and *platform-internal* permanent call loss; it does not and cannot manufacture certainty about one specific external network call's fate.

**Blocker #3 (DEP-6H-12) and Blocker C are both RESOLVED.** **Readiness: IMPLEMENTATION-READY**, live-validated (§49) — not merely design-reviewed.
...
| 17a | Retry after an ambiguous Voice response (caller genuinely cannot tell if the first attempt's provider call landed) | Covered by #15/#16's mechanism — the retry is a fresh call to `fn_initiate_outbound_call_idempotent()` with the identical key; `is_new` tells the caller definitively whether *this platform* already created a session for this dispatch attempt, removing the ambiguity at the Campaign/Voice boundary specifically (not at the provider boundary, per #15's disclosure). |
...
| Backward compatibility | Fully additive — no existing table, column, constraint, index, or grant in `027_5E.sql`–`033_5E.sql` is touched. `app_worker` gains `SELECT, INSERT` on the one new table and `EXECUTE` on the three new functions; no existing grant is widened. |
...
|---|---|
| Schema | `voice` |
| New table | `voice.call_dispatch_keys` — `PRIMARY KEY (dispatch_idempotency_key)`, plus identity columns and a full provider-dispatch state machine (`dispatch_state`, `claimed_by`, `claimed_at`, `claim_expires_at`, `attempt_count`, `provider_request_ref`, `provider_call_ref`, `submitted_at`, `confirmed_at`, `last_error`) |
| New functions | `voice.fn_new_uuid_v7()` (search_path bridge); `voice.fn_initiate_outbound_call_idempotent(...)` (§18.4 Step 1); `voice.fn_claim_dispatch_for_provider_submission(...)` (§18.4 Step 2); `voice.fn_record_dispatch_confirmed(...)`, `voice.fn_record_dispatch_ambiguous(...)`, `voice.fn_record_dispatch_failed(...)` (§18.4 Step 3) |
| Purpose | Close Blocker #3 (Campaign→Voice in-process dispatch idempotency) and Blocker C (provider-dispatch durability — a crash between reservation and provider submission must not permanently lose the call, and an ambiguous provider outcome must never be auto-retried) |
| Exact invariant | INV-CAM-02, INV-CAM-03, INV-CAM-09, INV-CAM-10 (§51) |
| Migration requirement | New, additive forward migration; `down_revision = 098_5E1` |
| Backward compatibility | Fully additive — `voice.call_sessions` itself is untouched (no new column, no changed CHECK/grant). `app_api`/`app_worker` gain `SELECT, INSERT` (deliberately no `UPDATE`, §49.2a) on the one new table and `EXECUTE` on the five new functions. |
| Existing data — dedup/backfill required? | **No.** This table records only *future* dispatch attempts going forward. |
| Deployment ordering | Must apply, and the Campaign dispatcher must be updated to call all three steps (§18.4), before any worker relies on this contract. Deploy the migration first, then roll workers. |
| Rollback implications | Dropping the five new functions and the new table is safe and non-destructive to `voice.call_sessions`. Rolling back without first reverting the Campaign-side caller breaks the in-process call entirely. |

### 49.2a Deliberately No Direct `UPDATE` Grant on `voice.call_dispatch_keys`

Every state transition (`CLAIMED`/`CONFIRMED`/`AMBIGUOUS`/`FAILED`) must go through one of the five guarded functions — `app_api`/`app_worker` hold `SELECT, INSERT` only, matching the platform's established pattern for other guarded state machines (e.g. `crm.contact_suppressions` has no direct `UPDATE` grant either — only `crm.lift_suppression()`). Confirmed live: `has_function_privilege`/grant inspection (§49.5) shows exactly this shape.
...
All 9 new functions confirmed, via a direct query against `pg_proc`/`pg_namespace`, to be `SECURITY DEFINER` with exactly the documented, minimal `search_path`:
...
`has_function_privilege()` confirmed, per function, per role (`app_api`, `app_worker`, `app_readonly`, `app_platform_admin`, `public`): every function grants `EXECUTE` only to its intended role(s) (`app_worker` alone for the internal-only functions; `app_api` + `app_worker` for the two Voice entry points a future REST-side reuse might also call); **`PUBLIC` has `EXECUTE` revoked on all 9, confirmed `f` (false) in every row** — no accidental broad grant.
...
All four were found, fixed, and the full fresh-database + incremental upgrade re-run to a clean exit-0 pass **after** each fix — the evidence in §49.4–§49.7 reflects the final, corrected SQL, not the versions that failed.
...
| Corrected contract | Three calls, not one: Step 1 reserves the logical call (`dispatch_idempotency_key` required, returns `is_new`); Step 2 (`ClaimDispatchForProviderSubmission`) claims exclusive, leased ownership of the actual provider network call (`claimed`, `attempt_count`, `reason`); Step 3 records exactly one definite outcome (`RecordDispatchConfirmed`/`Failed`/`Ambiguous`). |
| Idempotency semantics (Step 1) | Backed by `voice.fn_initiate_outbound_call_idempotent()` (`099_5C1.sql`) — a `PRIMARY KEY`-backed atomic claim; a retried call with the same key returns the pre-existing `call_session_id` and `is_new = FALSE`. |
| Provider-dispatch durability semantics (Steps 2–3, new) | Backed by `voice.fn_claim_dispatch_for_provider_submission()`/`fn_record_dispatch_{confirmed,ambiguous,failed}()` — a lease-based single-owner claim over a `RESERVED→CLAIMED→CONFIRMED\|AMBIGUOUS\|FAILED` state machine. A crash between claiming and recording an outcome is recoverable once the lease expires (re-claimable); an `AMBIGUOUS` outcome is a permanent auto-retry block. |
| Persistence requirement | `voice.call_dispatch_keys` (§49.2) — extended, not merely created, to carry the full state machine. No change to `voice.call_sessions` itself. |
| Campaign caller behavior | Step 2 `claimed=True`: proceed to `TelephonyPort.place_call()`, then record exactly one Step-3 outcome. Step 2 `claimed=False`: **must not** call the provider — inspect `reason` (already confirmed / ambiguous pending reconciliation / another worker's active lease) and reconcile accordingly. |
...
| Verification | Live-executed and race-tested against a real PostgreSQL 18 database — not merely design-reviewed. §49 has the full transcript. |
...
Enforced by `voice.call_dispatch_keys.dispatch_state`'s lease-based claim (§18.4 Step 2, `099_5C1.sql`) — an abandoned (lease-expired) `CLAIMED` row is transparently re-claimable by a different worker. **Live-proven**: a claimed-then-abandoned dispatch (a genuine 5-second lease, deliberately never followed by an outcome, simulating a crash) was safely re-claimed and confirmed by a different worker once the lease genuinely elapsed.

**INV-CAM-10 — An ambiguous provider-dispatch outcome must never be automatically retried.**
Enforced by `voice.fn_record_dispatch_ambiguous()` setting `dispatch_state = 'AMBIGUOUS'`, a state no function in `099_5C1.sql` ever transitions back to `CLAIMED` automatically — only an explicit provider-callback reconciliation or a bounded operator decision can move it forward. **Live-proven**: a fresh claim attempt against an `AMBIGUOUS` row was refused regardless of lease state, while the same attempt against a `FAILED` row (a definite, pre-acceptance rejection) succeeded — confirming the two states' intended asymmetry is real.

No invariant above is backed by a probability argument, a "single worker expected" assumption, or an "unlikely race" characterization — each cites a specific PostgreSQL-enforced mechanism (a `PRIMARY KEY`/partial-`UNIQUE` constraint, row-level locking via `SELECT ... FOR UPDATE`, a CAS `WHERE`-clause, or a lease-expiry-gated claim) that holds regardless of worker count, redelivery, or crash timing — and INV-CAM-01 through INV-CAM-05, INV-CAM-07 (materialization/dispatch halves), INV-CAM-09, and INV-CAM-10 have now been directly, empirically demonstrated against a real database, not merely argued. The one place this document's confidence is *not* absolute — the provider-level ambiguity disclosed in §18.4 and §17.5, where no platform-side idempotency key can reach into an external telephony provider's own state — is stated as exactly that: an accepted, external-system limit, not folded silently into any of the ten invariants above.
...
| 44 | Campaign→Voice in-process dispatch cannot durably double-dial on retry, AND a crash before provider submission cannot permanently lose the call | ✅ §5 findings 18–19, §18.4, §50, `099_5C1.sql` — **live-proven**, including genuine lease-expiry crash recovery and the `AMBIGUOUS`-vs-`FAILED` asymmetry (§49.7) |
...
| 48 | Every new `SECURITY DEFINER` function has the minimal `search_path` its actual dependencies require, verified against `pg_proc`, not assumed | ✅ §5 finding 20, §49.5 — direct `pg_proc`/`information_schema` inspection of all 9 new functions |

**All 48 freeze-gate items pass, live-validated.** Items 42–48 were added across this remediation's two passes, on top of the original 41, to make each blocker resolution — and the genuineness of its verification — individually auditable rather than folded silently into pre-existing items.
...
```
PHASE 6H STATUS (Revision 3 — live-validated)
...
CallJob idempotency:               APPROVED — Voice-side idempotency + provider-dispatch durability, LIVE-PROVEN (Blocker #3 / Blocker C)
...
Pause/Resume/Stop/Cancel:          APPROVED — durably serialized against dispatch, LIVE-PROVEN both orderings (Blocker #2)
...
Tenant/campaign ownership in new functions: APPROVED — two genuine gaps found by live probing, fixed, re-tested live
API-key eligibility:               APPROVED (asymmetric, justified)
Audit / events:                    APPROVED (amendment proposed, non-blocking)
Security / abuse review:           APPROVED — SECURITY DEFINER search_path/grants directly inspected live
Physical schema verification:      APPROVED — fresh-DB and incremental Alembic upgrades both exit code 0
Endpoint inventory:                APPROVED — 24/24 IMPLEMENTATION-READY

BLOCKING dependencies:              ZERO
Genuine defects found across both remediation passes: 6 (3 in the first pass, 3 more found by
  adversarially re-testing the first pass's own fixes) — ALL RESOLVED and LIVE-VALIDATED,
  including 2 real bugs (a dropped CREATE TABLE, a PL/pgSQL variable-name collision) caught
  during live execution itself and fixed before the passing run (§49.6)
Open PRE-PRODUCTION verification:   NONE remaining from this remediation's own scope — fresh-DB
  ...
  recovery guarantee have all been genuinely, directly demonstrated (§49)

Overall:
PHASE 6H — APPROVED / FROZEN, LIVE-VALIDATED
```

No genuine physical blocker was found that requires modifying Phase 5A–5J's *existing* content, and 6A–6C/6E–6G were not touched or reopened. Across two remediation passes, six genuine production-safety/security defects were found and closed with durable, PostgreSQL-enforced mechanisms — never with an in-memory lock, a probability argument, or a "single worker expected" assumption: CampaignContact duplicate-enqueue (Blocker #1), Pause/Stop-vs-dispatch race (Blocker #2), Campaign→Voice dispatch idempotency (Blocker #3), a provider-dispatch durability hole in the first fix for Blocker #3 (Blocker C), a SECURITY DEFINER `search_path` defect that would have failed on first real execution, and two cross-tenant/cross-campaign ownership-verification gaps in the first pass's own new functions. All six — and the two additional PL/pgSQL bugs found purely by attempting to execute the code — are now closed by two new, additive, forward migrations (`098_5E1.sql`, `099_5C1.sql`) plus one narrow, labeled, additive amendment to `6D-Voice-Call-Agent-APIs.md` §28.10a, and **every one of the resulting guarantees has been directly demonstrated against a real, disposable PostgreSQL 18 database** — fresh-database and incremental Alembic upgrades (both exit code 0), direct `pg_proc`/`information_schema` function and grant inspection, and genuine multi-connection concurrency races and real elapsed-time lease expiries, not simulated or narrated (§49). Three pre-existing, correctly-scoped-out-of-this-remediation open questions (recurring campaigns, DNC dispatch-proof logging, and the residual telephony-provider-side ambiguity §18.4 discloses) remain exactly as 5E/5L/this document's own honest accounting leaves them — see §46 for the precise status of each. Phase 6I (Workflow) is not started.

**STOP — Phase 6H complete, live-validated. Phase 6I not started.**
