...
| Revision | **Revision 4** (2026-08-28) — supersedes Revision 3 in place. Revision 3 found and resolved six genuine defects across two passes (CampaignContact duplicate-enqueue; Pause/Stop-vs-dispatch race; Campaign→Voice dispatch idempotency; a provider-dispatch durability hole in the first Voice fix; a SECURITY DEFINER `search_path` bug; two cross-tenant/cross-campaign ownership-verification gaps) and live-validated all of it against PostgreSQL 18. **A final, independent adversarial freeze review of Revision 3 then found four further genuine defects, the most severe of the entire remediation**: (a) **Blocker A (P0)** — Revision 3's provider-dispatch state machine still let an expired `CLAIMED` lease be reclaimed even after the provider may already have been contacted, permitting a second worker to physically dial the same customer twice; (b) **Blocker B** — `app_worker` held a direct `INSERT` grant on `campaign.campaign_contact_identities` alongside the guarded `fn_enqueue_contact()` path, permitting an orphan identity row that would permanently block future legitimate enqueues; (c) **Blocker C** — `app_api`/`app_worker` held direct `INSERT` grants on `voice.call_dispatch_keys`, permitting a caller to fabricate an arbitrary `dispatch_state`; (d) **Blocker D** — `voice.fn_initiate_outbound_call_idempotent()`'s replay path validated neither tenant nor payload, permitting a cross-tenant key-reuse information leak and a silent request-substitution under a reused key. **This revision closes all four**: a new durable `SUBMITTING` state (entered via a new function, `fn_begin_provider_submission()`, committed before the provider is ever contacted) that is never automatically reclaimed regardless of lease staleness, plus a new reconciliation function (`fn_reconcile_dispatch_outcome()`) for the resulting resolution path; both direct-`INSERT` grants removed; explicit tenant + canonical-payload-fingerprint validation added to the idempotency replay path. **All ten defects across all four passes are now live-validated — critically, on a genuinely separate PostgreSQL 16.10 instance, the declared production baseline** (the three prior passes had validated only against PostgreSQL 18): a real EDB full-installer elevation failure was encountered and disclosed, not worked around silently; a binaries-only PostgreSQL 16 distribution was used instead, with `pgvector` built from source via the locally available MSVC toolchain; full fresh-database and incremental `alembic upgrade` runs (both exit code 0, single head `099_5C1`); direct `pg_proc`/`information_schema` inspection of all 11 `SECURITY DEFINER` functions; genuine multi-connection concurrency races and real elapsed-time lease expiries proving the `SUBMITTING`-is-never-reclaimed P0 fix, both privilege bypasses now denied, both idempotency-validation paths, and a full regression pass confirming Revision 3's own guarantees (CampaignContact uniqueness, Pause/Stop serialization) are unaffected. Full transcript: §49.9. No 6A–6C/6E–6G content changed; 6D carries the same one narrow, now-updated, labeled amendment (§28.10a). |
| Date | 2026-08-28 |
...
**Blocker #3 (DEP-6H-12), Blocker C, and the Final Blocker Remediation's Blockers A/B/C/D are all RESOLVED.** **Readiness: IMPLEMENTATION-READY**, live-validated on both PostgreSQL 18 and PostgreSQL 16 (§49) — not merely design-reviewed.
...
**Zero BLOCKING items.** DEP-6H-01, -02, and -15 are **RESOLVED** within this document's own authority via documentation-level corrections and read-path specifications, no Phase 5 DDL touched. **DEP-6H-03, -12, -18, -20, -21, -22, -23, -24, and -25 (nine genuine production-safety/security defects found across three remediation passes — three in the first pass, two more found by adversarially re-testing the first pass's own fixes, and four more found by a final independent adversarial freeze review of the second pass's own fixes) are RESOLVED via two additive, controlled-amendment migrations (corrected in place across all three passes, per the disclosed migration policy — never applied to production, so never renumbered) plus one narrow labeled 6D amendment, and are now live-validated on both PostgreSQL 18 and PostgreSQL 16 (the declared production baseline), not merely design-reviewed** — see §49–§50 for full detail, including the genuine bugs found and fixed during live execution itself (§49.6, §49.9). **DEP-6H-26 is RESOLVED** — every claim in this document is now backed by PostgreSQL 16 evidence, not only PostgreSQL 18. DEP-6H-04/05/06/07/08/09/10 remain **NON-BLOCKING**, closed either by classification or by deliberately not exposing a capability the DDD never defined. DEP-6H-11/13/14/16/17 are honest cross-phase or cross-legal-domain handoffs, not gaps in this document's own contract. **DEP-6H-19 is RESOLVED** — the live-execution/concurrent-race testing it called for has been performed, with results and transcripts in §49, matching the rigor of the Phase 6G reconciliation's own live-tested amendments.
...
|---|---|
| Schema | `voice` |
| New table | `voice.call_dispatch_keys` — `PRIMARY KEY (dispatch_idempotency_key)`, plus identity columns, a `payload_fingerprint` (Blocker D), and a full provider-dispatch state machine (`dispatch_state`, `claimed_by`, `claimed_at`, `claim_expires_at`, `attempt_count`, `submission_started_at`, `provider_request_ref`, `provider_call_ref`, `confirmed_at`, `last_error`, `reconciled_by`, `reconciled_at`) |
| New functions | `voice.fn_new_uuid_v7()` (search_path bridge); `voice.fn_initiate_outbound_call_idempotent(...)` (§18.4 Step 1, now with tenant/payload validation); `voice.fn_claim_dispatch_for_provider_submission(...)` (§18.4 Step 2); `voice.fn_begin_provider_submission(...)` (§18.4 Step 3, new — the durable pre-network-call boundary); `voice.fn_record_dispatch_confirmed(...)`, `voice.fn_record_dispatch_ambiguous(...)`, `voice.fn_record_dispatch_failed(...)` (§18.4 Step 4); `voice.fn_reconcile_dispatch_outcome(...)` (new — async/identity-correlated resolution of a stuck `SUBMITTING`/`AMBIGUOUS` row) — **8 functions total**, directly counted via `pg_proc`, not asserted (§49.5) |
| Purpose | Close Blocker #3 (Campaign→Voice in-process dispatch idempotency), Blocker C (provider-dispatch durability), and the Final Blocker Remediation's Blocker A (expired-CLAIMED double-dial hazard, P0), Blocker C (direct-INSERT privilege bypass), and Blocker D (idempotency replay tenant/payload validation) |
| Exact invariant | INV-CAM-02, INV-CAM-03, INV-CAM-09, INV-CAM-10, INV-VOICE-DISPATCH-01 through -06 (§51) |
| Migration requirement | New, additive forward migration; `down_revision = 098_5E1` |
| Backward compatibility | Fully additive — `voice.call_sessions` itself is untouched (no new column, no changed CHECK/grant). `app_api`/`app_worker` hold **`SELECT`-only** (Final Blocker Remediation, Blocker C — `INSERT` removed; see §49.2a) on the one table and `EXECUTE` on the eight functions. |
...
| Corrected contract | Four calls, not one: Step 1 reserves the logical call (`dispatch_idempotency_key` required, validates tenant + payload fingerprint on replay, returns `outcome`); Step 2 (`ClaimDispatchForProviderSubmission`) claims exclusive, leased ownership of *preparing* the provider network call (`claimed`, `provider_request_ref`, `attempt_count`, `reason`); Step 3 (`BeginProviderSubmission`, new) commits the durable "submission may now begin" boundary **before** the provider is contacted; Step 4 records exactly one definite outcome (`RecordDispatchConfirmed`/`Failed`/`Ambiguous`). A fifth, asynchronous call (`ReconcileDispatchOutcome`, new) resolves a stuck row via identity correlation, not lease ownership. |
...
| Reconciliation semantics (new) | `voice.fn_reconcile_dispatch_outcome()` resolves a `SUBMITTING`/`AMBIGUOUS` row via identity correlation (e.g. a provider callback matched to `provider_request_ref`), with no dependency on the original worker's lease. |
| Persistence requirement | `voice.call_dispatch_keys` (§49.2) — extended, not merely created, to carry the full state machine plus `payload_fingerprint`. No change to `voice.call_sessions` itself. No role holds direct `INSERT`/`UPDATE`/`DELETE` on this table (§49.2a). |
| Campaign caller behavior | Step 3 `began=True`: proceed to `TelephonyPort.place_call()`, using `provider_request_ref` from Step 2, then record exactly one Step-4 outcome. Step 2 `claimed=False` or Step 3 `began=False`: **must not** call the provider. |
...
| Verification | Live-executed and race-tested against both a real PostgreSQL 18 database (Revision 3) and a genuinely separate PostgreSQL 16.10 instance, the declared production baseline (Revision 4, this pass) — not merely design-reviewed. §49.4/§49.9 have the full transcripts. |
...
**All 53 freeze-gate items pass, live-validated.** Items 42–48 were added across this remediation's second and third passes; items 49–53 were added by this fourth (Final Blocker Remediation) pass, on top of the original 41, to make each blocker resolution — and the genuineness of its verification — individually auditable rather than folded silently into pre-existing items.
...
```
PHASE 6H STATUS (Revision 4 — Final Blocker Remediation, live-validated on PostgreSQL 18 AND PostgreSQL 16)
...
Audience materialization:          APPROVED — CampaignContact uniqueness DB-enforced and LIVE-PROVEN (Blocker #1);
                                    direct-INSERT privilege bypass closed and LIVE-PROVEN (Blocker B, this pass)
...
CallJob idempotency:               APPROVED — Voice-side idempotency + provider-dispatch durability, LIVE-PROVEN
                                    ...
                                    SUBMITTING boundary, LIVE-PROVEN (Blocker A, this pass, the P0 defect);
                                    direct-INSERT privilege bypass closed, LIVE-PROVEN (Blocker C, this pass);
                                    idempotency replay tenant/payload validation added, LIVE-PROVEN (Blocker D, this pass)
...
Pause/Resume/Stop/Cancel:          APPROVED — durably serialized against dispatch, LIVE-PROVEN both orderings
                                    (Blocker #2); regression-confirmed unaffected by this pass's Voice-side changes
...
Security / abuse review:           APPROVED — SECURITY DEFINER search_path/grants directly inspected live on
                                    ...
                                    privilege bypass remains on either hardened table
Physical schema verification:      APPROVED — fresh-DB and incremental Alembic upgrades both exit code 0, on
                                    BOTH PostgreSQL 18 and PostgreSQL 16 (the declared production baseline)
Endpoint inventory:                APPROVED — 24/24 IMPLEMENTATION-READY

BLOCKING dependencies:              ZERO
Genuine defects found across four remediation passes: 10 (3 in the first pass; 3 more found by
  adversarially re-testing the first pass's own fixes, Revision 3; 4 more found by a final
  independent adversarial freeze review of Revision 3's own fixes, this pass — one P0
  double-dial hazard plus three privilege/validation gaps) — ALL RESOLVED and LIVE-VALIDATED,
  including 2 real implementation bugs (a dropped CREATE TABLE, a PL/pgSQL variable-name
  collision) caught during Revision 3's live execution and fixed before that pass's run (§49.6)
Open PRE-PRODUCTION verification:   NONE remaining from this remediation's own scope — fresh-DB
  ...
  recovery/reconciliation guarantee have all been genuinely, directly demonstrated on both
  PostgreSQL 18 (§49.1–§49.8) and PostgreSQL 16, the declared production baseline (§49.9)

Overall:
PHASE 6H — APPROVED / FROZEN, LIVE-VALIDATED ON POSTGRESQL 18 AND POSTGRESQL 16
```

No genuine physical blocker was found that requires modifying Phase 5A–5J's *existing* content, and 6A–6C/6E–6G were not touched or reopened. Across four remediation passes, ten genuine production-safety/security defects were found and closed with durable, PostgreSQL-enforced mechanisms — never with an in-memory lock, a probability argument, or a "single worker expected" assumption: CampaignContact duplicate-enqueue (Blocker #1), Pause/Stop-vs-dispatch race (Blocker #2), Campaign→Voice dispatch idempotency (Blocker #3), a provider-dispatch durability hole in the first fix for Blocker #3 (Blocker C, Revision 3), a SECURITY DEFINER `search_path` defect that would have failed on first real execution, two cross-tenant/cross-campaign ownership-verification gaps in Revision 3's own new functions, an expired-lease double-dial hazard in Revision 3's own provider-dispatch state machine (Blocker A, this pass, the single most severe defect found across all four passes — capable of physically dialing a customer twice), and two direct-table-write privilege bypasses plus one idempotency replay validation gap (Blockers B/C/D, this pass). All ten — and the two additional PL/pgSQL bugs found purely by attempting to execute Revision 3's code — are now closed by two additive, forward migrations (`098_5E1.sql`, `099_5C1.sql`, each corrected in place across all three SQL-authoring passes per the disclosed never-applied-to-production migration policy) plus one narrow, labeled, additive amendment to `6D-Voice-Call-Agent-APIs.md` §28.10a, and **every one of the resulting guarantees has been directly demonstrated against real, disposable PostgreSQL databases — both PostgreSQL 18 (Revision 3, §49.1–§49.8) and, in this pass, a genuinely separate PostgreSQL 16.10 instance, the declared production baseline (§49.9)** — fresh-database and incremental Alembic upgrades (exit code 0 on both versions), direct `pg_proc`/`information_schema` function and grant inspection, and genuine multi-connection concurrency races and real elapsed-time lease expiries, not simulated or narrated. Three pre-existing, correctly-scoped-out-of-this-remediation open questions (recurring campaigns, DNC dispatch-proof logging, and the residual telephony-provider-side ambiguity §18.4 discloses — explicitly re-checked in this pass and confirmed still undocumented for any configured provider, not assumed away) remain exactly as 5E/5L/this document's own honest accounting leaves them — see §46 for the precise status of each. Phase 6I (Workflow) is not started.
