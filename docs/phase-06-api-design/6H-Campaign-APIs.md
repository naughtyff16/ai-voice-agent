...
| Revision | **Revision 5** (2026-08-28) — supersedes Revision 4 in place. Revision 4 closed four genuine defects (Blocker A, the P0 expired-CLAIMED-lease double-dial hazard; Blockers B/C, two direct-INSERT privilege bypasses; Blocker D, idempotency replay tenant/payload validation), introducing a new `fn_reconcile_dispatch_outcome()` resolution function as part of Blocker A's fix, and live-validated all of it — for the first time in this remediation — against PostgreSQL 16, the declared production baseline. **A final, independent micro-remediation review then found that `fn_reconcile_dispatch_outcome()` itself — the one function able to convert an `AMBIGUOUS`/`SUBMITTING` submission into `FAILED`, which immediately re-opens eligibility for a fresh physical provider attempt — had granted `EXECUTE` to `app_api` and `app_worker`, the two broad, shared-by-everything application roles.** This meant ordinary application/worker code, not just a trusted reconciliation path, could unilaterally re-authorize a second physical telephony attempt for an ambiguous call — the exact class of defect this whole remediation exists to prevent, relocated into the resolution function itself rather than closed. **This revision closes that gap**: a new, narrowly-scoped PostgreSQL role (`app_voice_reconciler` — `LOGIN`, not `BYPASSRLS`, no table DML, `EXECUTE` on exactly this one function) now holds the automated provider-callback/provider-lookup reconciliation capability; the existing break-glass/operator role (`app_platform_admin`) holds the human path; `EXECUTE` is revoked from `app_api`/`app_worker`. A new required provenance field (`reconciliation_source`) distinguishes the evidentiary channel without ever functioning as an authorization input; a `FAILED` reconciliation now requires non-empty evidence; every successful reconciliation now writes a durable `VOICE_DISPATCH_RECONCILED` audit event. **Live-proven on a fresh PostgreSQL 16.10 instance**: direct calls as `app_api`/`app_worker` (including a forged `reconciled_by='admin'` argument) denied; the authorized role successfully resolves both `CONFIRMED` and evidence-backed `FAILED` outcomes, with `FAILED` genuinely re-opening retry; blank-evidence `FAILED` attempts rejected even for the authorized role; an already-`CONFIRMED` row's reconciliation attempt refused; a cross-tenant attempt refused non-disclosingly; provenance and audit both confirmed by direct query; the full prior-pass regression suite re-run unchanged. Full transcript: §49.9a. No 6A–6C/6E–6G content changed; 6D carries the same one narrow, now-updated, labeled amendment (§28.10a). |
| Date | 2026-08-28 |
...
-- Called asynchronously, e.g. by the provider status-callback handler, once it has
-- independently correlated an inbound callback to this dispatch_idempotency_key.
-- PRIVILEGED: this is a physical-redial safety authorization boundary, callable only
-- by app_voice_reconciler (the trusted, automated reconciliation service) or
-- app_platform_admin (the existing break-glass/operator role) — NOT by app_api/app_worker.
SELECT reconciled, reason FROM voice.fn_reconcile_dispatch_outcome(
  $ikey, $org_id, p_outcome => 'CONFIRMED' | 'FAILED',
  p_reconciliation_source => 'PROVIDER_CALLBACK' | 'PROVIDER_LOOKUP' | 'OPERATOR',
  p_reconciled_by => $handler_id, p_provider_call_ref => $ref, p_note => $note
);
```

`voice.call_sessions` is itself `PARTITION BY RANGE (started_at)` (`011_5C.sql`) — the identical structural reason `campaign_contacts` cannot carry a direct `UNIQUE` constraint applies here to a dispatch key. `voice.call_dispatch_keys` (`099_5C1.sql`) is a small, non-partitioned table carrying both the `PRIMARY KEY (dispatch_idempotency_key)` claim (Step 1) *and* a `dispatch_state` column (`RESERVED | CLAIMED | SUBMITTING | CONFIRMED | AMBIGUOUS | FAILED`) tracking the provider-submission attempt itself (Steps 2–4) — deliberately independent of `voice.call_sessions.status` (the call's own conversational/telephony lifecycle, unchanged, owned by 6D). No role — including `app_api`/`app_worker`, the two roles that call Steps 1–4 above — holds direct `INSERT`/`UPDATE`/`DELETE` on this table; every state transition provably requires going through one of these eight `SECURITY DEFINER` functions. **Step 5 (reconciliation) is further restricted beyond Steps 1–4**: `app_api`/`app_worker` cannot even `EXECUTE` `fn_reconcile_dispatch_outcome()` — only `app_voice_reconciler`/`app_platform_admin` can (Revision 5, finding 26) — because Steps 1–4 can only ever move a row *forward* toward a terminal or hard-stop state using the worker's own first-hand knowledge of what actually happened, while Step 5 is the one operation that can convert an *uncertain* outcome into `FAILED` and thereby re-open eligibility for a second physical telephony attempt based on someone else's asserted evidence.
...
- **Reconciliation authorization (Revision 5, finding 26): direct calls to `fn_reconcile_dispatch_outcome()` as `app_api` and `app_worker` both fail with `permission denied`**, including a forged `p_reconciled_by = 'admin'` argument from `app_api` (the parameter is never reached — the privilege check fails first). The authorized role (`app_voice_reconciler`) successfully resolves both `CONFIRMED` and evidence-backed `FAILED` outcomes; a `FAILED` reconciliation with an empty or `NULL` evidence note is rejected even for the authorized role; an attempt to reconcile an already-`CONFIRMED` row to `FAILED` is refused (`CONFIRMED → FAILED` is structurally impossible, not merely conventionally avoided); a cross-tenant reconciliation attempt is refused non-disclosingly with no mutation; a `VOICE_DISPATCH_RECONCILED` audit event is confirmed present, by direct query, for every successful reconciliation.
...
**Blocker #3 (DEP-6H-12), Blocker C, the Final Blocker Remediation's Blockers A/B/C/D, and the Final Micro-Remediation's reconciliation authorization boundary (finding 26) are all RESOLVED.** **Readiness: IMPLEMENTATION-READY**, live-validated on both PostgreSQL 18 and (twice, across two independent instances) PostgreSQL 16 (§49) — not merely design-reviewed.
...
**Zero BLOCKING items.** DEP-6H-01, -02, and -15 are **RESOLVED** within this document's own authority via documentation-level corrections and read-path specifications, no Phase 5 DDL touched. **DEP-6H-03, -12, -18, -20, -21, -22, -23, -24, -25, and -27 (ten genuine production-safety/security defects found across four remediation passes — three in the first pass, two more found by adversarially re-testing the first pass's own fixes, four more found by a final independent adversarial freeze review of the second pass's own fixes, and one more found by a final micro-remediation review of the third pass's own fix) are RESOLVED via one additive, controlled-amendment migration (`099_5C1.sql`, corrected in place across all four passes, per the disclosed migration policy — never applied to production, so never renumbered) plus one more (`098_5E1.sql`, corrected across three of those four passes) plus one narrow labeled 6D amendment, and are now live-validated on both PostgreSQL 18 and PostgreSQL 16 (the declared production baseline, across two independently built PostgreSQL 16 instances), not merely design-reviewed** — see §49–§50 for full detail, including the genuine bugs found and fixed during live execution itself (§49.6, §49.9). **DEP-6H-26 is RESOLVED** — every claim in this document is now backed by PostgreSQL 16 evidence, not only PostgreSQL 18. **DEP-6H-27 is RESOLVED** — the reconciliation function's authorization boundary now matches its actual safety criticality, live-proven, §49.9a. DEP-6H-04/05/06/07/08/09/10 remain **NON-BLOCKING**, closed either by classification or by deliberately not exposing a capability the DDD never defined. DEP-6H-11/13/14/16/17 are honest cross-phase or cross-legal-domain handoffs, not gaps in this document's own contract. **DEP-6H-19 is RESOLVED** — the live-execution/concurrent-race testing it called for has been performed, with results and transcripts in §49, matching the rigor of the Phase 6G reconciliation's own live-tested amendments.
...
| Backward compatibility | Fully additive — no existing table, column, constraint, index, or grant in `027_5E.sql`–`033_5E.sql` is touched. `app_worker` holds **`SELECT`-only** (Final Blocker Remediation, Blocker B — `INSERT` removed, §49.2a) on the one new table and `EXECUTE` on the three new functions; no existing grant is widened. |
...
| New functions | `voice.fn_new_uuid_v7()` (search_path bridge); `voice.fn_initiate_outbound_call_idempotent(...)` (§18.4 Step 1, now with tenant/payload validation); `voice.fn_claim_dispatch_for_provider_submission(...)` (§18.4 Step 2); `voice.fn_begin_provider_submission(...)` (§18.4 Step 3, new — the durable pre-network-call boundary); `voice.fn_record_dispatch_confirmed(...)`, `voice.fn_record_dispatch_ambiguous(...)`, `voice.fn_record_dispatch_failed(...)` (§18.4 Step 4); `voice.fn_reconcile_dispatch_outcome(...)` (identity-correlated resolution of a stuck `SUBMITTING`/`AMBIGUOUS` row, §18.4 Step 5 — authorization-hardened by Revision 5, finding 26) — **8 functions total**, directly counted via `pg_proc`, not asserted (§49.5) |
| New role | `app_voice_reconciler` (Revision 5, finding 26) — `LOGIN`, not `BYPASSRLS`, `USAGE` on schema `voice` plus `EXECUTE` on exactly `fn_reconcile_dispatch_outcome()`, nothing else. First role introduced by any migration other than `001_5B.sql`. |
...
| Exact invariant | INV-CAM-02, INV-CAM-03, INV-CAM-09, INV-CAM-10, INV-VOICE-DISPATCH-01 through -07 (§51) |
| Migration requirement | New, additive forward migration; `down_revision = 098_5E1` |
| Backward compatibility | Fully additive — `voice.call_sessions` itself is untouched (no new column, no changed CHECK/grant). `app_api`/`app_worker` hold **`SELECT`-only** (Final Blocker Remediation, Blocker C — `INSERT` removed; see §49.2a) on the one table and `EXECUTE` on seven of the eight functions — **not** `fn_reconcile_dispatch_outcome()`, whose `EXECUTE` grant belongs only to the new `app_voice_reconciler` role and the existing `app_platform_admin` (Revision 5, finding 26, §49.2a). |
...
All 11 functions (3 `campaign.fn_*` + 8 `voice.fn_*`, after the Final Blocker Remediation pass added `fn_begin_provider_submission()` and `fn_reconcile_dispatch_outcome()`) confirmed, via a direct query against `pg_proc`/`pg_namespace` re-run on PostgreSQL 16 (§49.9), to be `SECURITY DEFINER` with exactly the documented, minimal `search_path`:
...
`has_function_privilege()` confirmed, per function, per role (`app_api`, `app_worker`, `app_readonly`, `app_migration`, `app_platform_admin`, `app_voice_reconciler`, `public`): every function grants `EXECUTE` only to its intended role(s) (`app_worker` alone for the worker-only functions; `app_api` + `app_worker` for the Voice entry points a real-time non-campaign call or a webhook callback handler also calls). **One exception, deliberate (Revision 5, finding 26): `fn_reconcile_dispatch_outcome()` grants `EXECUTE` only to `app_voice_reconciler` and `app_platform_admin` — confirmed `f` (false) for `app_api`, `app_worker`, `app_readonly`, and `app_migration` on this one function, live-checked on PostgreSQL 16 (§49.9a), not merely read from the SQL source.** **`PUBLIC` has `EXECUTE` revoked on all 11, confirmed `f` (false) in every row** — no accidental broad grant.
...
| Reconciliation semantics | `voice.fn_reconcile_dispatch_outcome()` resolves a `SUBMITTING`/`AMBIGUOUS` row via identity correlation (e.g. a provider callback matched to `provider_request_ref`), with no dependency on the original worker's lease. |
| Reconciliation authorization boundary (Revision 5, finding 26) | `fn_reconcile_dispatch_outcome()` is **not** callable by `app_api`/`app_worker` — `EXECUTE` is granted only to a new role, `app_voice_reconciler` (the automated provider-callback/provider-lookup path), and the existing `app_platform_admin` (the operator/break-glass path). `reconciliation_source`/`reconciled_by` are evidentiary metadata only, never authorization. A `FAILED` outcome requires non-empty evidence. Every successful reconciliation writes a durable `VOICE_DISPATCH_RECONCILED` audit event. |
...
| Verification | Live-executed and race-tested against a real PostgreSQL 18 database (Revision 3) and two independently built, genuinely separate PostgreSQL 16.10 instances, the declared production baseline (Revision 4 and Revision 5, this pass) — not merely design-reviewed. §49.4/§49.9/§49.9a have the full transcripts. |
...
Enforced by `voice.fn_record_dispatch_ambiguous()` setting `dispatch_state = 'AMBIGUOUS'`, a state no function in `099_5C1.sql` ever transitions back to a claimable state automatically — only `voice.fn_reconcile_dispatch_outcome()` (an explicit provider-callback reconciliation or a bounded operator decision) can move it forward. **Live-proven**: a fresh claim attempt against an `AMBIGUOUS` row was refused regardless of lease state, while the same attempt against a `FAILED` row (a definite, pre-acceptance rejection) succeeded — confirming the two states' intended asymmetry is real.
...
`RESERVED`, `FAILED`, and a lease-expired `CLAIMED` row are the only automatically-reclaimable states — each is a state the database can prove never reached `SUBMITTING`. `SUBMITTING` and `AMBIGUOUS` require `fn_reconcile_dispatch_outcome()` instead. **Live-proven**: all four state transitions exercised directly (§49.9).

**INV-VOICE-DISPATCH-05 — Ambiguous provider outcomes require reconciliation, not blind redial.**
Enforced identically to INV-CAM-10, restated here as the general provider-dispatch-layer invariant; `fn_reconcile_dispatch_outcome()` is the only path forward from `AMBIGUOUS` (or `SUBMITTING`).
...
Enforced entirely by PostgreSQL role/`EXECUTE` privilege, never by any caller-supplied value: `app_api` and `app_worker` cannot `EXECUTE` `voice.fn_reconcile_dispatch_outcome()` at all (`REVOKE`d, Revision 5, finding 26) — the only function that can convert `SUBMITTING`/`AMBIGUOUS` into `FAILED` and thereby re-open eligibility for a fresh physical provider attempt. Only `app_voice_reconciler` (a narrowly-scoped role backing the automated, evidence-driven provider-callback/provider-lookup path) and `app_platform_admin` (the existing break-glass/operator role) hold this privilege — and even they cannot reopen an already-`CONFIRMED` row, nor record a `FAILED` outcome with no evidence. **Live-proven**: `app_api`/`app_worker` both denied with `permission denied for function fn_reconcile_dispatch_outcome`, including a forged `p_reconciled_by='admin'` argument from `app_api` (the parameter never reaches the function body — it carries no authorization weight); the authorized role's `FAILED` reconciliation is proven to genuinely reopen retry (a subsequent claim succeeds); a `CONFIRMED`-row reconciliation attempt is refused even for the authorized role; a cross-tenant attempt is refused non-disclosingly.

No invariant above is backed by a probability argument, a "single worker expected" assumption, or an "unlikely race" characterization — each cites a specific PostgreSQL-enforced mechanism (a `PRIMARY KEY`/partial-`UNIQUE` constraint, row-level locking via `SELECT ... FOR UPDATE`, a CAS `WHERE`-clause, a lease-expiry-gated claim, or a role-level `EXECUTE` grant) that holds regardless of worker count, redelivery, crash timing, or caller-supplied metadata — and INV-CAM-01 through INV-CAM-05, INV-CAM-07 (materialization/dispatch halves), INV-CAM-09, INV-CAM-10, and INV-VOICE-DISPATCH-01 through -07 have now been directly, empirically demonstrated against a real database (on PostgreSQL 18 and, across two independently built instances, PostgreSQL 16, §49.9/§49.9a), not merely argued. The one place this document's confidence is *not* absolute — the provider-level ambiguity disclosed in §18.4 and §17.5, where no platform-side idempotency key can reach into an external telephony provider's own state — is stated as exactly that: an accepted, external-system limit, not folded silently into any of the seventeen invariants above.
...
**All 57 freeze-gate items pass, live-validated.** Items 42–48 were added across this remediation's second and third passes; items 49–53 were added by the fourth (Final Blocker Remediation) pass; items 54–57 were added by this fifth (Final Micro-Remediation) pass, on top of the original 41, to make each blocker resolution — and the genuineness of its verification — individually auditable rather than folded silently into pre-existing items.
...
```
PHASE 6H STATUS (Revision 5 — Final Micro-Remediation, live-validated on PostgreSQL 18 AND PostgreSQL 16)
...
CallJob idempotency:               APPROVED — Voice-side idempotency + provider-dispatch durability, LIVE-PROVEN
                                    ...
                                    reconciliation authorization boundary closed, LIVE-PROVEN (finding 26, this pass —
                                    ordinary app_api/app_worker can no longer reopen an ambiguous submission for retry)
...
Pause/Resume/Stop/Cancel:          APPROVED — durably serialized against dispatch, LIVE-PROVEN both orderings
                                    (Blocker #2); regression-confirmed unaffected by Revision 4's and this pass's
                                    Voice-side changes
...
Audit / events:                    APPROVED (amendment proposed, non-blocking; VOICE_DISPATCH_RECONCILED added, this pass)
Security / abuse review:           APPROVED — SECURITY DEFINER search_path/grants directly inspected live on
                                    PostgreSQL 16; PUBLIC EXECUTE denied on all 11 functions; no direct-write
                                    privilege bypass remains on either hardened table; reconciliation EXECUTE
                                    restricted to a narrowly-scoped role plus the existing break-glass role, this pass
Physical schema verification:      APPROVED — fresh-DB and incremental Alembic upgrades both exit code 0, on
                                    PostgreSQL 18 and, across two independently built instances, PostgreSQL 16
                                    (the declared production baseline)
Endpoint inventory:                APPROVED — 24/24 IMPLEMENTATION-READY

BLOCKING dependencies:              ZERO
Genuine defects found across five remediation passes: 11 (3 in the first pass; 3 more found by
  ...
  remediation review of Revision 4's own reconciliation-resolution fix, this pass) — ALL
  RESOLVED and LIVE-VALIDATED, including 2 real implementation bugs (a dropped CREATE TABLE,
  a PL/pgSQL variable-name collision) caught during Revision 3's live execution and fixed
  before that pass's run (§49.6)
Open PRE-PRODUCTION verification:   NONE remaining from this remediation's own scope — fresh-DB
  ...
  recovery/reconciliation/authorization guarantee have all been genuinely, directly demonstrated
  on PostgreSQL 18 (§49.1–§49.8) and, across two independently built instances, PostgreSQL 16,
  the declared production baseline (§49.9, §49.9a)

Overall:
PHASE 6H — APPROVED / FROZEN, LIVE-VALIDATED ON POSTGRESQL 18 AND POSTGRESQL 16
```

No genuine physical blocker was found that requires modifying Phase 5A–5J's *existing* content, and 6A–6C/6E–6G were not touched or reopened. Across five remediation passes, eleven genuine production-safety/security defects were found and closed with durable, PostgreSQL-enforced mechanisms — never with an in-memory lock, a probability argument, or a "single worker expected" assumption: CampaignContact duplicate-enqueue (Blocker #1), Pause/Stop-vs-dispatch race (Blocker #2), Campaign→Voice dispatch idempotency (Blocker #3), a provider-dispatch durability hole in the first fix for Blocker #3 (Blocker C, Revision 3), a SECURITY DEFINER `search_path` defect that would have failed on first real execution, two cross-tenant/cross-campaign ownership-verification gaps in Revision 3's own new functions, an expired-lease double-dial hazard in Revision 3's own provider-dispatch state machine (Blocker A, Revision 4, one of the two most severe defects found across all five passes — capable of physically dialing a customer twice), two direct-table-write privilege bypasses plus one idempotency replay validation gap (Blockers B/C/D, Revision 4), and an overly broad reconciliation `EXECUTE` grant (finding 26, this pass — the other of the two most severe defects: without this fix, ordinary application/worker code could still authorize a second physical telephony attempt via the reconciliation path, exactly the outcome Blocker A's own fix existed to prevent, merely relocated). All eleven — and the two additional PL/pgSQL bugs found purely by attempting to execute Revision 3's code — are now closed by two additive, forward migrations (`098_5E1.sql`, `099_5C1.sql`, each corrected in place across all four SQL-authoring passes per the disclosed never-applied-to-production migration policy) plus one narrow, labeled, additive amendment to `6D-Voice-Call-Agent-APIs.md` §28.10a, and **every one of the resulting guarantees has been directly demonstrated against real, disposable PostgreSQL databases — PostgreSQL 18 (Revision 3, §49.1–§49.8) and, across two independently built instances, a genuinely separate PostgreSQL 16.10 instance, the declared production baseline (Revision 4's own instance, §49.9, and this pass's own freshly rebuilt instance, §49.9a)** — fresh-database and incremental Alembic upgrades (exit code 0 every time), direct `pg_proc`/`information_schema` function and grant inspection, genuine multi-connection concurrency races, real elapsed-time lease expiries, and genuine role-boundary privilege checks (including a forged-authorization-metadata attempt), not simulated or narrated. Three pre-existing, correctly-scoped-out-of-this-remediation open questions (recurring campaigns, DNC dispatch-proof logging, and the residual telephony-provider-side ambiguity §18.4 discloses — explicitly re-checked across multiple passes and confirmed still undocumented for any configured provider, not assumed away) remain exactly as 5E/5L/this document's own honest accounting leaves them — see §46 for the precise status of each. Phase 6I (Workflow) is not started.
