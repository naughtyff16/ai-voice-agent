...
# lookup decision) to this dispatch_idempotency_key. PRIVILEGED: callable only by the
# trusted reconciliation service or an authorized operator (see the authorization-boundary
# bullet below) — NOT by ordinary API request handling or generic worker code.
ReconcileDispatchOutcome(
    dispatch_idempotency_key, organization_id, outcome: 'CONFIRMED' | 'FAILED',
    reconciliation_source: 'PROVIDER_CALLBACK' | 'PROVIDER_LOOKUP' | 'OPERATOR',
...
- **Reconciliation semantics:** `ReconcileDispatchOutcome` is deliberately **not** CAS-guarded on the original `worker_id` — the original worker/lease is presumed gone, which is precisely why the row is stuck. It resolves a `SUBMITTING` or `AMBIGUOUS` row to a definite `CONFIRMED`/`FAILED` outcome via identity correlation only (the caller — a provider status-callback handler, or a bounded operator/provider-lookup decision — has already matched the inbound signal to this `dispatch_idempotency_key`, e.g. via `provider_request_ref`). **Live-proven:** a stuck `SUBMITTING` row was resolved to `CONFIRMED`; a stuck `AMBIGUOUS` row was resolved to `FAILED` and was then genuinely re-claimable.
- **Reconciliation is a privileged internal capability, not a normal state update (Final Micro-Remediation, closes a genuine authorization gap):** `ReconcileDispatchOutcome` is **not callable by ordinary API request handling or generic worker execution paths**. A prior pass granted this capability to the same broad roles as everything else (`app_api`/`app_worker`), which meant any ordinary application code could convert an `AMBIGUOUS` submission to `FAILED` and re-authorize a second physical telephony attempt — exactly the failure this entire amendment exists to prevent, relocated into the reconciliation function itself. Two trusted callers only: (1) **the provider-callback path** — an authenticated, signature-verified inbound provider status callback (5I's existing webhook-authenticity mechanism), correlated to a stable dispatch identity, processed by a narrowly-scoped, dedicated service credential (`app_voice_reconciler` at the DB layer — `LOGIN`, not `BYPASSRLS`, no table DML, `EXECUTE` on this one function only); (2) **the operator path** — a privileged, audited human action under a controlled procedure backed by provider-console evidence, running under the existing platform-admin/break-glass identity (`app_platform_admin`, 6B §18.3) rather than a second new role. `reconciliation_source` and `reconciled_by` are **evidentiary metadata only, never authorization** — neither is read by any authorization decision; a caller cannot gain privilege by passing `reconciled_by = 'admin'` or `reconciliation_source = 'OPERATOR'`. A `FAILED` outcome additionally requires a non-empty `note` (evidence description) — never accepted blank, though the database cannot verify the evidence is *true*, only that something was stated. **Live-proven on PostgreSQL 16:** direct calls as `app_api` and `app_worker` both fail with `permission denied`, including a forged `reconciled_by='admin'` attempt from `app_api`; the authorized path succeeds for both `CONFIRMED` and evidence-backed `FAILED` outcomes; a blank-evidence `FAILED` attempt is rejected even under the authorized role; an attempt to reconcile an already-`CONFIRMED` row to `FAILED` is refused (`CONFIRMED → FAILED` remains structurally impossible); a cross-tenant attempt is refused non-disclosingly with no mutation. Every successful reconciliation now also writes a durable `VOICE_DISPATCH_RECONCILED` audit event (via `audit.fn_insert_audit_event()`, 6D §24.0's sole legal write path) synchronously in the same transaction — organization, resource (call session), old/new state, reconciliation source, actor, and timestamp, with no phone/PII data in the snapshot.
...
**Verification status:** live-executed and race-tested against **genuinely separate PostgreSQL 16.10 instances** (the declared production baseline; the first two passes had validated only against PostgreSQL 18) — the EDB full installer failed in this environment both times ("requires elevation," disclosed rather than worked around silently); a binaries-only distribution was used instead, with `pgvector` built from source via the locally available MSVC toolchain. Fresh-database and incremental `alembic upgrade` both passed (exit code 0) on both this pass's own instance and the prior pass's; every concurrency/crash-recovery/reconciliation/authorization scenario named above was exercised as a genuine multi-connection transaction, a real elapsed-time lease expiry, or a genuine role-boundary privilege check, not simulated or narrated. Full transcripts: `docs/phase-05-database-design/5K/execution_logs/README.md`'s "Sixth batch" and "Seventh batch" and `docs/phase-05-database-design/5K/validation/VOICE_DISPATCH_VALIDATION_REPORT.md` (including its Final Micro-Remediation addendum).

**Full DDL and rationale:** `099_5C1.sql`; `docs/phase-05-database-design/5C-Voice-Schema.md`'s matching amendment sections; `docs/phase-06-api-design/6H-Campaign-APIs.md` (Revision 5) §18, §49.
