...
**Controlled Cross-Phase Amendment (Phase 6M, 2026-09-03) — status of items 1–5 above, not a redesign of this handoff.** Two of this section's own premises are now stale and are corrected in place, without deleting the original handoff text above (frozen-document discipline — the original five-item list remains the binding specification; this paragraph records what has since been delivered against it):
- **Item 4 — delivered.** `105_5B4.sql` adds a `purposes TEXT[]` column to `organization.break_glass_grants`, allow-listing (among others) `SENSITIVE_MEDIA_ACCESS` as a distinct, explicitly-grantable purpose (`chk_bgg_purposes_allowed`), with wildcard/superuser purposes (`*`, `ALL_ACCESS`, `SUPER_ADMIN`) rejected at issuance (`chk_bgg_purposes_no_wildcard`) and `purposes` immutable post-insert. This is precisely the "sensitive-media-specific purpose marker" this item called for: a generic break-glass grant (any `purposes` array not containing `SENSITIVE_MEDIA_ACCESS`) structurally cannot authorize recording/transcript-content access, satisfying this platform's standing invariant that generic break-glass alone must deny sensitive media. Full detail: 6B §18.3b, `5B-Identity-Organization-Multitenancy-Security.md`'s Phase 6M amendment.
- **The "interim Redis record with a documented future durable-table dependency, DEP-6B-01" clause in item 4 above is stale** — DEP-6B-01 is RESOLVED (Phase 5L, 2026-08-24; extended Phase 6M); `organization.break_glass_grants` is a durable table, and Redis is now a fast-path cache in front of it (6B §18.3b).
- **Items 1, 2, 3, and 5 are unchanged** — 6B §18.3a's actor-type check, per-request grant validation (now purpose-aware), and mandatory justification field were already complete and remain the binding contract; item 5's audit-write requirement is unaffected (5J's `RECORDING_ACCESS_GRANTED` ‖ marker is untouched by Phase 6M's §14.3 vocabulary amendment, which added only billing/quota/refund values, ✦ marker).
- **This paragraph does not itself add a platform-admin recording/transcript endpoint.** Whether such an endpoint is actually built is a decision for `6M-Admin-Platform-APIs.md` to make explicitly (including any owner decision on tenant visibility of sensitive-media break-glass access); this paragraph only records that the structural precondition this section named (item 4) now exists in the schema.
...
**The gap this closes, across six passes:** §28.10's `Idempotency-Key` requirement is an HTTP-layer mechanism (6A §16.2) with no meaning for an in-process caller (pass 1: `voice.fn_initiate_outbound_call_idempotent()`). Refusing to ever re-invoke the provider once a key was claimed then risked permanently losing a call on a pre-submission crash (pass 2: the claim/confirm/ambiguous/failed protocol). A final adversarial freeze review then found the pass-2 design still permitted the opposite failure — **a worker that crashed *after* the provider had already received the request** would eventually have its lease expire and be re-claimed by another worker, which would call the provider again, physically dialing the same person twice. Pass 3 closed that P0 gap with a durable pre-network-call boundary, closed two direct-table-write privilege bypasses, and added tenant/payload validation to replay — but its new `ReconcileDispatchOutcome` resolution capability was itself grantable to ordinary application/worker roles, meaning ordinary code could still authorize a second physical attempt via the reconciliation path instead of the claim path. Pass 4 closed that authorization gap by restricting `ReconcileDispatchOutcome` to two roles — but the single function still let either authorized caller freely choose which provenance category to record, so the automated reconciler could falsely record itself as an operator decision or vice versa, an audit-integrity defect. Pass 5 closed that gap by splitting the one function into two capability-specific functions, each hardcoding the provenance it is allowed to produce. Pass 6 (this one) found that, throughout all five prior passes, `app_platform_admin`'s own original direct `INSERT`/`UPDATE`/`DELETE` grant on `voice.call_dispatch_keys` had never been touched — the one remaining path that could bypass every invariant established above via a single raw `UPDATE` statement — and closes it by removing that grant entirely. All six passes are described together below as the current, single contract — no prior wording describing a "claim, then never re-invoke" rule, an "any expired lease is always safe to retry" rule, an "any caller may reconcile" rule, a "reconciliation source is caller-chosen" rule, or a "platform-admin may directly mutate dispatch state" rule still applies.
...
# Called asynchronously, NOT by the original dispatching worker — resolves a row stuck
# in SUBMITTING or AMBIGUOUS once the caller has independently correlated an inbound
# callback (or a bounded operator/provider-lookup decision) to this dispatch_idempotency_key.
# PRIVILEGED and SPLIT BY TRUSTED PATH: two functions, not one — the provenance a given
# credential can ever produce is fixed by WHICH function it can call, never by a parameter.

# (a) The automated provider-callback/provider-lookup path — callable ONLY by the
#     trusted reconciliation service credential. p_provider_source is restricted, inside
#     the function itself, to the two provider-evidence values; 'OPERATOR' is rejected
#     even if somehow supplied.
ReconcileDispatchFromProvider(
    dispatch_idempotency_key, organization_id, outcome: 'CONFIRMED' | 'FAILED',
    provider_source: 'PROVIDER_CALLBACK' | 'PROVIDER_LOOKUP',
    reconciled_by: str, provider_call_ref: str | None = None,
    note: str | None = None,  # REQUIRED (non-empty) when outcome = 'FAILED'
) -> ReconcileResult(reconciled: bool, reason: str | None)

# (b) The operator path — callable ONLY by the authorized privileged-operator credential.
#     No source parameter exists; 'OPERATOR' provenance is hardcoded inside the function.
ReconcileDispatchByOperator(
    dispatch_idempotency_key, organization_id, outcome: 'CONFIRMED' | 'FAILED',
    reconciled_by: str, provider_call_ref: str | None = None,
    note: str | None = None,  # REQUIRED (non-empty) when outcome = 'FAILED'
) -> ReconcileResult(reconciled: bool, reason: str | None)
...
- **Reconciliation semantics:** neither reconciliation function is CAS-guarded on the original `worker_id` — the original worker/lease is presumed gone, which is precisely why the row is stuck. Each resolves a `SUBMITTING` or `AMBIGUOUS` row to a definite `CONFIRMED`/`FAILED` outcome via identity correlation only (the caller — a provider status-callback handler, or a bounded operator/provider-lookup decision — has already matched the inbound signal to this `dispatch_idempotency_key`, e.g. via `provider_request_ref`). **Live-proven:** a stuck `SUBMITTING` row was resolved to `CONFIRMED` via the provider path; a stuck `AMBIGUOUS` row was resolved to `FAILED` via the provider path and was then genuinely re-claimable; the operator path was separately proven to resolve both a `CONFIRMED` and an evidence-backed `FAILED` outcome.
- **Reconciliation is a privileged internal capability, not a normal state update (Final Micro-Remediation, closes a genuine authorization gap):** neither reconciliation function is **callable by ordinary API request handling or generic worker execution paths**. A prior pass granted this capability to the same broad roles as everything else (`app_api`/`app_worker`), which meant any ordinary application code could convert an `AMBIGUOUS` submission to `FAILED` and re-authorize a second physical telephony attempt — exactly the failure this entire amendment exists to prevent, relocated into the reconciliation function itself. Two trusted callers only: (1) **the provider-callback path** — an authenticated, signature-verified inbound provider status callback (5I's existing webhook-authenticity mechanism), correlated to a stable dispatch identity, processed by a narrowly-scoped, dedicated service credential (`app_voice_reconciler` at the DB layer — `LOGIN`, not `BYPASSRLS`, no table DML, `EXECUTE` on `ReconcileDispatchFromProvider` only); (2) **the operator path** — a privileged, audited human action under a controlled procedure backed by provider-console evidence, running under the existing platform-admin/break-glass identity (`app_platform_admin`, 6B §18.3, `EXECUTE` on `ReconcileDispatchByOperator` only) rather than a second new role. `reconciled_by` is **evidentiary metadata only, never authorization** — never read by any authorization decision; a caller cannot gain privilege by passing `reconciled_by = 'admin'`. A `FAILED` outcome additionally requires a non-empty `note` (evidence description) — never accepted blank, though the database cannot verify the evidence is *true*, only that something was stated. **Live-proven on PostgreSQL 16:** direct calls as `app_api` and `app_worker` both fail with `permission denied` on either function.
- **Reconciliation provenance is non-forgeable — a capability of the function, not a parameter (Final Micro-Fix, closes a further genuine defect):** the prior pass's single `ReconcileDispatchOutcome` still let *either* authorized caller freely choose which provenance category (`PROVIDER_CALLBACK`/`PROVIDER_LOOKUP`/`OPERATOR`) to record via a plain parameter — meaning the automated reconciliation credential could falsely record itself as an operator decision, or vice versa, corrupting the audit trail for a physical-redial authorization decision. **Resolved by splitting into two functions**, each hardcoding the provenance/`actor_type` its own `EXECUTE` grant is allowed to produce: `ReconcileDispatchFromProvider` can only ever record `PROVIDER_CALLBACK`/`PROVIDER_LOOKUP` (an internal `CHECK` rejects `'OPERATOR'` even from a caller who genuinely holds `EXECUTE`) and `actor_type='WORKER'`; `ReconcileDispatchByOperator` takes no source parameter at all and hardcodes `'OPERATOR'`/`actor_type='PLATFORM_ADMIN'`. Neither role can call the other's function, so neither credential can misrepresent which trusted path made the decision. **Live-proven on PostgreSQL 16, the critical test:** `app_voice_reconciler`, while genuinely holding `EXECUTE` on `ReconcileDispatchFromProvider`, calling it with `provider_source='OPERATOR'` was rejected by the function's own internal `CHECK` — not by a missing grant; `app_voice_reconciler` calling `ReconcileDispatchByOperator` at all, and `app_platform_admin` calling `ReconcileDispatchFromProvider` at all, were both denied at the privilege layer. Genuine reconciliation through each path recorded the correct, function-determined provenance and `actor_type`, confirmed by direct query against both `voice.call_dispatch_keys` and `audit.audit_events`; `CONFIRMED → FAILED` and cross-tenant reconciliation were re-confirmed refused through *both* functions; the evidence requirement for `FAILED` was confirmed shared/identical across both paths. Every successful reconciliation writes a durable `VOICE_DISPATCH_RECONCILED` audit event (via `audit.fn_insert_audit_event()`, 6D §24.0's sole legal write path) synchronously in the same transaction — organization, resource (call session), old/new state, reconciliation source, actor, and timestamp, with no phone/PII data in the snapshot.
- **Operator reconciliation does not imply unrestricted database mutation rights (Final Admin-DML Hardening, closes the last remaining direct-mutation path):** every prior pass restricted a *different* role's access to `voice.call_dispatch_keys`, but `app_platform_admin`'s own original direct `INSERT`/`UPDATE`/`DELETE` grant on the table — present since it was first created — was never touched, and could bypass `CONFIRMED` immutability and the provenance split entirely via one raw `UPDATE` statement no guarded function ever sees (e.g. `UPDATE voice.call_dispatch_keys SET dispatch_state = 'FAILED' WHERE dispatch_state = 'CONFIRMED'`, or forging `reconciliation_source = 'PROVIDER_CALLBACK'` directly). **Resolved**: that grant is removed; `app_platform_admin` now holds `SELECT` only on this table, identical in shape to `app_api`/`app_worker`/`app_readonly`. The trusted operator path is, and has only ever been meant to be:
  ```
  authenticated platform admin -> controlled application action -> ReconcileDispatchByOperator()
  ```
  never:
  ```
  platform admin -> arbitrary UPDATE call_dispatch_keys
  ```
  `ReconcileDispatchByOperator` needs no direct table grant to keep writing — it is `SECURITY DEFINER`, owned by the migration-running role, identical in this respect to every other guarded function in this schema. **Live-proven on PostgreSQL 16**: `app_platform_admin` denied `permission denied` on direct `INSERT`, `UPDATE` (including the specific provenance-forgery attempt and the specific `CONFIRMED → FAILED` reopen attempt), and `DELETE`; a `SELECT` against a live `CONFIRMED` row still succeeds; `ReconcileDispatchByOperator` on a genuine `AMBIGUOUS` row with real evidence still succeeds, and against an already-`CONFIRMED` row still correctly refuses — the privilege removal changes nothing about the guarded path's own behavior. **Disclosed, not claimed away**: the migration-owning database role and genuine PostgreSQL superuser access remain technically able to write to this table directly — a property of the database engine's privilege model, not of this application's runtime authorization design, and explicitly outside what "no normal application/runtime role can reopen a CONFIRMED dispatch" claims to guarantee.
