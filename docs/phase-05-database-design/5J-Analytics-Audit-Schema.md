...
**Voice — Calls / Tools / Recordings / Phone Numbers** ‡: `CALL_INITIATED`, `CALL_TERMINATED`, `CALL_TRANSFERRED`, `CALL_HELD`, `CALL_RESUMED`, `TOOL_DEFINITION_CREATED`, `TOOL_DEFINITION_UPDATED`, `TOOL_DEFINITION_DEACTIVATED`, `RECORDING_DELETED`, `PHONE_NUMBER_AGENT_ASSIGNED`, `RECORDING_ACCESS_GRANTED` ‖, `TRANSCRIPT_ACCESS_GRANTED` ✧
...
**Billing:** `SUBSCRIPTION_CREATED`, `SUBSCRIPTION_PLAN_CHANGED`, `SUBSCRIPTION_CANCELLED`, `INVOICE_GENERATED`, `PAYMENT_ATTEMPTED`, `PAYMENT_SUCCEEDED`, `PAYMENT_FAILED`, `REFUND_ISSUED`, `BILLING_ADJUSTMENT_CREATED`, `QUOTA_OVERRIDE_SET` ✦, `REFUND_RESERVED` ✦, `REFUND_SETTLED` ✦, `REFUND_FAILED` ✦, `PLATFORM_CREDIT_APPLIED` ❖, `PLATFORM_BILLING_ADJUSTMENT_CREATED` ❖, `PLATFORM_PLAN_CREATED` ❖, `PLATFORM_PLAN_VERSION_CREATED` ❖, `PLATFORM_PLAN_VERSION_PUBLISHED` ❖, `PLATFORM_PLAN_PRICE_CREATED` ❖, `PLATFORM_PLAN_DEACTIVATED` ❖, `PLATFORM_CPA_CREATED` ❖, `PLATFORM_CPA_VERSION_CREATED` ❖, `PLATFORM_CPA_VERSION_ACTIVATED` ❖, `PLATFORM_CPA_VERSION_EXPIRED` ❖, `PLATFORM_TAX_CATEGORY_CREATED` ❖, `PLATFORM_TAX_RULE_CREATED` ❖
...
**Admin / Security:** `BREAK_GLASS_GRANTED`, `BREAK_GLASS_RELEASED`, `ADMIN_ACTION`, `PLATFORM_CONFIG_CHANGED`, `PLATFORM_SESSIONS_REVOKED` ❖
...
**✦ Controlled Phase 6M amendment (4 values, added by the Phase 6M Admin/Platform Control-Plane APIs pass), required because `105_5B4.sql`/`106_5H3.sql` deliberately follow `087_5B1.sql`'s established precedent of never fusing an audit write inside a guarded SECURITY DEFINER function — the state-transition and the synchronous `audit.fn_insert_audit_event()` call are two separate calls made by the application layer (5J §14.5), so the vocabulary must carry a value for each distinct guarded transition before `6M-Admin-Platform-APIs.md` can reference it:** `QUOTA_OVERRIDE_SET` (the `billing.fn_platform_set_quota_override()` upsert path — deliberately not folded into the existing `BILLING_ADJUSTMENT_CREATED`, which names a billing-ledger credit/debit, a materially different action from changing a usage-metric limit), `REFUND_RESERVED`/`REFUND_SETTLED`/`REFUND_FAILED` (the three phases of `billing.fn_platform_reserve_refund()`/`fn_platform_settle_refund()`/`fn_platform_fail_refund()`'s guarded saga, `106_5H3.sql` — each phase is its own transaction and its own audit-worthy moment, so a single `REFUND_ISSUED` would collapse three distinct, independently-timestamped events, including the terminal failure path, into one; the pre-existing `REFUND_ISSUED` value is retained unchanged for any refund-completion path outside this guarded saga and is not reused or redefined by this amendment). `ORGANIZATION_SUSPENDED`/`ORGANIZATION_REACTIVATED`/`BREAK_GLASS_GRANTED`/`BREAK_GLASS_RELEASED` — the audit actions for `105_5B4.sql`'s own guarded organization-lifecycle and break-glass-purpose functions — already existed in this vocabulary prior to this amendment (Organization/Membership and Admin/Security categories respectively) and required no addition; `105_5B4.sql`'s break-glass `purposes` extension is carried in the existing `BREAK_GLASS_GRANTED` event's `resource_snapshot`, not a new action kind. **This is a pure vocabulary/governance amendment, exactly like the `†`/`‡`/`¶`/`§`/`‖` amendments above — no SQL migration touches `audit.audit_events` or its `chk_ae_action_kind` constraint** (still `CHECK (length(action_kind) BETWEEN 1 AND 200)`, migration `072_5J.sql`, reconfirmed unchanged by this pass). No `organization.*`/`billing.*` table, column, constraint, index, function, or RLS policy is touched by this amendment — it governs only which `action_kind` strings a call to `audit.fn_insert_audit_event()` may legitimately pass as `p_action_kind` for a Phase 6M admin mutation. Full detail: `docs/phase-06-api-design/6M-Admin-Platform-APIs.md`.

**✧ Controlled Phase 6M amendment (1 value, added 2026-09-05 by the Phase 6M stabilization/completion pass), required because `107_5B5.sql` delivered a guarded purpose-bound Platform-Admin transcript-content access path (`voice.fn_platform_access_transcript(...)`) that this vocabulary had no value for — the prior `6M-Admin-Platform-APIs.md` draft had described transcript-content platform-admin access as deferred/future, which this amendment's sibling correction in that document (and in `5B-Identity-Organization-Multitenancy-Security.md`'s own Phase 6M amendment) now supersedes:** `TRANSCRIPT_ACCESS_GRANTED` — written synchronously and atomically (in the same transaction as the access check, per `107_5B5.sql`'s in-transaction-audit pattern — see the `✦` amendment's own header note on 105/106 vs. 107's differing audit-atomicity, and 6B §18.3b item 5) at the moment `voice.fn_platform_access_transcript(...)` grants a Platform Admin caller access to `voice.transcript_segments.text`/`voice.transcripts.summary_text` content under a valid `SENSITIVE_MEDIA_ACCESS`-purposed break-glass grant plus a resource-ownership cross-check — never for a denied request, and never containing the transcript text itself in `resource_snapshot` (bounded to non-secret correlation fields only: `transcript_id`/`conversation_id`, `grant_id`). Named `_GRANTED` to exactly parallel the existing `RECORDING_ACCESS_GRANTED` (`‖`) precedent — both name the capability-issuance/access-authorization moment, not a subsequent usage moment — even though, unlike a signed recording URL, transcript content is returned inline within the already-authenticated, already-logged function call itself; the `‖` amendment's own closing note ("no transcript-content-access equivalent is added... this asymmetry is a deliberate design choice") described the state of the world *before* `107_5B5.sql` existed and is now superseded specifically for the Platform-Admin break-glass path (it remains true, and is not contradicted, for the ordinary tenant-caller `transcript:access_content` path the `‖` note was actually describing, which still mints no separate capability and still needs no dedicated action_kind). **This is a pure vocabulary/governance amendment, exactly like the `†`/`‡`/`¶`/`§`/`‖`/`✦` amendments above — no SQL migration touches `audit.audit_events` or its `chk_ae_action_kind` constraint** (still `CHECK (length(action_kind) BETWEEN 1 AND 200)`, migration `072_5J.sql`, reconfirmed unchanged by this pass). No `voice.*` table, column, constraint, index, function, or RLS policy is touched by this amendment. Full detail: `docs/phase-06-api-design/6M-Admin-Platform-APIs.md`.

**❖ Controlled Phase 6M amendment (14 values, added 2026-09-05 by the Phase 6M FINAL FREEZE-CANDIDATE REMEDIATION pass), required because migration `109_5B7` (specified in `6M-Admin-Platform-APIs.md` §65) delivers fourteen new guarded/wrapper `SECURITY DEFINER` functions closing the independent freeze-readiness review's P1 findings on financial-mutation over-breadth and least-privilege support surfaces, each of which is its own distinct audit-worthy state transition with no existing vocabulary entry to reuse:** `PLATFORM_CREDIT_APPLIED` (§65.1, `billing.fn_platform_apply_credit()` — a thin Platform-Admin-only wrapper around the pre-existing `billing.fn_billing_apply_credit()`, deliberately not reusing any existing credit-related value so that platform-admin-originated credit application is distinguishable in the audit trail from `app_worker`-originated credit application using the same underlying raw function), `PLATFORM_BILLING_ADJUSTMENT_CREATED` (§65.2, `billing.fn_platform_create_billing_adjustment()`, same wrapper relationship to the pre-existing `billing.fn_create_billing_adjustment()`, and deliberately distinct from the pre-existing unguarded `BILLING_ADJUSTMENT_CREATED` value for the same reason), `PLATFORM_PLAN_CREATED`/`PLATFORM_PLAN_VERSION_CREATED`/`PLATFORM_PLAN_VERSION_PUBLISHED`/`PLATFORM_PLAN_PRICE_CREATED`/`PLATFORM_PLAN_DEACTIVATED` (§65.3, the five new guarded commands over `billing.plans`/`plan_versions`/`plan_prices`, each its own transaction — creation, draft-version creation, one-way publish, price-row creation, and taking a plan out of the active catalog are five independently timestamped moments, not one; `PLATFORM_PLAN_DEACTIVATED` is a one-way idempotent state flip — `billing.fn_platform_deactivate_plan()` — never a delete, and is written even when the plan was already inactive, since that is still a legitimate no-op Platform-Admin action worth recording), `PLATFORM_CPA_CREATED`/`PLATFORM_CPA_VERSION_CREATED`/`PLATFORM_CPA_VERSION_ACTIVATED`/`PLATFORM_CPA_VERSION_EXPIRED` (§65.4, the four new thin wrappers around `102_5H2.sql`'s pre-existing `fn_create_commercial_pricing_agreement`/`fn_create_commercial_pricing_agreement_version`/`fn_activate_commercial_pricing_agreement_version`/`fn_expire_commercial_pricing_agreement_version` — again kept distinct from any hypothetical unguarded equivalent so a Platform-Admin-originated CPA lifecycle transition is distinguishable from an `app_worker`-originated one), `PLATFORM_TAX_CATEGORY_CREATED`/`PLATFORM_TAX_RULE_CREATED` (§65.5, the two new guarded commands over `billing.tax_categories`/`tax_rules` — deliberately outside 6C's existing tenant tax-profile action-kind space, since these name platform-wide catalog administration, not a tenant's own tax-profile selection), and `PLATFORM_SESSIONS_REVOKED` (§65.6, `identity.fn_platform_revoke_all_sessions()` — the new guarded wrapper narrowing `app_platform_admin`'s previously-raw `identity.sessions` UPDATE privilege down to a single purpose-bound forced-session-revocation command; distinct from any ordinary user-initiated session-termination value, since this names a Platform-Admin-initiated action taken against another principal's sessions). §65.7 (webhook-delivery DML narrowing) adds no new `action_kind`, since it is a plain `REVOKE` with no accompanying function and no new audit-worthy transition. **This is a pure vocabulary/governance amendment, exactly like the `†`/`‡`/`¶`/`§`/`‖`/`✦`/`✧` amendments above — no SQL migration touches `audit.audit_events` or its `chk_ae_action_kind` constraint** (still `CHECK (length(action_kind) BETWEEN 1 AND 200)`, migration `072_5J.sql`, reconfirmed unchanged by this pass; migration `109_5B7` has been drafted and now confirmed emits these fourteen values via `audit.fn_insert_audit_event()` — live-verified against the fresh `001→109` and incremental `108→109` PostgreSQL 18 validation runs (`MIGRATION_MANIFEST.md` Row 109, `6M_FINAL_VALIDATION_REPORT.md` §8) — and requires no change to this constraint, since it is a length check, not an enum or `IN`-list. `109_5B7` is confirmed the final migration of the Phase 6M closure pass; no `110` follows it). No `billing.*`/`identity.*` table, column, constraint, index, function, or RLS policy other than those `109_5B7` already documents in `6M-Admin-Platform-APIs.md` §65 is touched by this amendment. Full detail: `docs/phase-06-api-design/6M-Admin-Platform-APIs.md`.
...
| Billing events | Asynchronous, **except the named Phase 6M guarded-mutation exception below** ❖ |
...
**❖ Controlled Phase 6M amendment (documentation-only, no SQL change), required because migration `109` (`6M-Admin-Platform-APIs.md` §65) implements its fourteen new guarded/wrapper functions using the same in-transaction-audit pattern as `107_5B5.sql`'s `✧` amendment (§65's own opening paragraph: each guarded function writes its `audit.fn_insert_audit_event(...)` call atomically, in the same local transaction as its mutation, before returning) — a narrow synchronous exception to the general "Billing events → Asynchronous" row above is required for exactly these fourteen values, modeled on the `‡` exception's own structure:**

- Platform credit / adjustment (`PLATFORM_CREDIT_APPLIED`, `PLATFORM_BILLING_ADJUSTMENT_CREATED`)
- Plan/PlanVersion/PlanPrice guarded commands (`PLATFORM_PLAN_CREATED`, `PLATFORM_PLAN_VERSION_CREATED`, `PLATFORM_PLAN_VERSION_PUBLISHED`, `PLATFORM_PLAN_PRICE_CREATED`, `PLATFORM_PLAN_DEACTIVATED`)
- CommercialPricingAgreement guarded commands (`PLATFORM_CPA_CREATED`, `PLATFORM_CPA_VERSION_CREATED`, `PLATFORM_CPA_VERSION_ACTIVATED`, `PLATFORM_CPA_VERSION_EXPIRED`)
- Tax guarded commands (`PLATFORM_TAX_CATEGORY_CREATED`, `PLATFORM_TAX_RULE_CREATED`)
- Forced session revocation (`PLATFORM_SESSIONS_REVOKED` — an Admin/Security-category value, already synchronous under the "Admin actions" row above; listed here only for completeness since it is one of the fourteen `❖` values, not because it changes the Billing row)

For these operations, the guarded `SECURITY DEFINER` function's own transaction invokes `audit.fn_insert_audit_event(...)` before returning — a genuinely atomic write, not merely a same-request synchronous call as the `‡` exception describes for 6D's Voice operations (5J §14.2's sole write path, unchanged). A failure raised by the audit insert rolls back the entire guarded mutation, exactly as it already does for every other synchronous category in this table. Any other `109` guarded mutation not named above, should one be added before `109` is finalized, is presumed synchronous under this same exception if and only if `6M-Admin-Platform-APIs.md` §65 documents it as writing its audit event in the guarded function's own transaction — this table is not to be treated as a closed list independent of §65.

**Reason this exception is narrow and justified, not a general loosening of the async default:** (1) these are all rare, low-volume, human-Platform-Admin-initiated financial and catalog mutations — not high-throughput tenant billing events (invoice generation, payment webhooks, usage-metered charges), which remain fully asynchronous under the unchanged default; (2) atomic in-transaction audit is the load-bearing control that makes these functions' least-privilege narrowing meaningful in the first place — a mutation and its audit record must not be separable for a Platform-Admin financial action, unlike ordinary tenant billing volume where async durability is an acceptable and already-accepted tradeoff. This clarification does **not** change the write mode for Campaign, Plugin, ordinary tenant Billing events, or any other "Configuration... lifecycle"/"Billing events" category member outside the fourteen values named above — the general async default remains the default everywhere else, exactly as the `‡` exception's own closing sentence establishes for its fifteen Voice operations.
