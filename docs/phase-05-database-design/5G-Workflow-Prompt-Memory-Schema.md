...
Both extensions live-validated identically (fresh/incremental PostgreSQL 16, full regression, zero defects reintroduced) — see `5K/MIGRATION_MANIFEST.md`'s "Phase 6I FINAL Blocker Remediation" and "Phase 6I FINAL MICRO-REMEDIATION" entries and `6I-Workflow-APIs.md` §64/§65 for the complete record. One product-policy question (whether `app_platform_admin` should retain its frozen, `041_5G.sql`-era `EXECUTE` grant on `fn_start_workflow_execution`) was reviewed and left open for the product owner — the function's own safety did not depend on which roles could call it, so this was never a schema or security gap.

*Fourth pass, same day, same file* (`6I-Workflow-APIs.md` §66): the product owner made that decision — `app_platform_admin` must not be able to directly start a live `WorkflowExecution`. `100_5G1.sql` amended in place a fourth time, changing exactly one `GRANT` statement (`app_platform_admin` removed from `fn_start_workflow_execution`'s `EXECUTE` grantees; `app_api`/`app_worker` and the `REVOKE ALL FROM PUBLIC` unchanged) — no function body, tenant-validation, `ARCHIVED`-locking, or duplicate-start-semantics change. Live-validated (fresh/incremental PostgreSQL 16, full Archive/StartExecution regression, zero defects) — see `5K/MIGRATION_MANIFEST.md`'s "Phase 6I FINAL PRIVILEGE CLEANUP" entry and `6I-Workflow-APIs.md` §66. No open product-policy or technical question remains for this document's Workflow schema.
