...
## Amendment — Phase 6H Final Blocker Remediation (2026-08-28)

A subsequent, independent adversarial freeze review found that the provider-dispatch durability state machine above, while a genuine improvement, still had a P0 gap: a `CLAIMED` row's lease expiring was the *only* signal used to decide whether re-claiming it was safe, and `CLAIMED` covered the entire span up to and including the moment the telephony provider was actually contacted. A worker that crashed **after** the provider received the request — but before writing anything back — would eventually have its row re-claimed by another worker, which would call the provider again: a genuine double-dial hazard. Three further defects, closed in this same `099_5C1.sql` file (still never applied to production, still corrected in place):

1. **Provider-submission boundary (the P0 fix).** `dispatch_state` gains a new state, `SUBMITTING`, entered only via a new function, `voice.fn_begin_provider_submission()`, which the caller's contract requires committing durably **before** ever invoking `TelephonyPort.place_call()`. `voice.fn_claim_dispatch_for_provider_submission()`'s reclaim predicate now excludes `SUBMITTING` unconditionally — only `RESERVED`, `FAILED`, and a lease-expired `CLAIMED` row (none of which have any evidence the provider was contacted) are automatically retryable. A new function, `voice.fn_reconcile_dispatch_outcome()`, gives a genuine, identity-correlated resolution path for a stuck `SUBMITTING` or `AMBIGUOUS` row via a delayed provider callback or a bounded operator/provider-lookup decision, without ever requiring the original (presumed-crashed) worker's lease. **Live-proven on PostgreSQL 16** (the declared production baseline — the prior two 6H passes had validated only against PostgreSQL 18): a `CLAIMED` row whose lease expires before reaching `SUBMITTING` is safely re-claimed; a `SUBMITTING` row whose lease expires is provably **not** re-claimable, even though nothing else has touched it — the direct empirical closure of the double-dial defect; a stuck `SUBMITTING` row was successfully resolved via reconciliation to `CONFIRMED`, and a stuck `AMBIGUOUS` row to `FAILED` (then genuinely re-claimable), proving the resolution path actually works end-to-end, not just in design. Full evidence: `docs/phase-05-database-design/5K/validation/VOICE_DISPATCH_VALIDATION_REPORT.md`.
2. **Direct INSERT bypass on `voice.call_dispatch_keys`.** `app_api`/`app_worker`'s `INSERT` grant is removed; only `SELECT` remains. Every dispatch row and state transition now provably requires going through one of the eight guarded `voice.fn_*` functions — a caller can no longer fabricate an arbitrary `dispatch_state` (including a forged `CONFIRMED` row) by inserting directly. Live-proven: direct `INSERT` as both `app_worker` and `app_api` now fails with `permission denied for table call_dispatch_keys`.
3. **Idempotency replay tenant/payload validation.** `voice.fn_initiate_outbound_call_idempotent()` now computes a canonical, versioned SHA-256 `payload_fingerprint` from the actual immutable request fields on every call (organization_id, campaign_lead_ref, to_number, from_number, agent_version_id, tenant_phone_number_id — via `public.digest()`, never accepted as a caller-supplied value) and persists it with the dispatch key. On replay: a different `organization_id` now raises a non-disclosing exception (this function is `SECURITY DEFINER` and bypasses RLS entirely, so this explicit check is the *entire* tenant-isolation guarantee for a replay, not defense-in-depth); a different `payload_fingerprint` under the same key returns `outcome = 'IDEMPOTENCY_KEY_REUSE_MISMATCH'` (reusing 6A §16.2's existing global error semantic rather than inventing new vocabulary) with no session identity disclosed; a genuine same-tenant, same-payload replay still returns the original call unchanged. All three live-proven.

**Function count correction.** This pass adds two functions (`fn_begin_provider_submission`, `fn_reconcile_dispatch_outcome`) to the six that existed before it, for a directly-queried total of **eight** `voice.fn_*` functions in `099_5C1.sql` — the stale "five new voice.fn_* functions" reference in `099_5C1.py`'s `downgrade()` docstring (already wrong even before this pass) is corrected everywhere it appears.

**What remains an accepted, disclosed limitation, unchanged from the prior pass:** whether the telephony provider itself received and acted on a single network call whose response was lost cannot be resolved by any platform-side database mechanism alone. That residual sliver is bounded by 6D's pre-existing provider-retry contract (3B §19) and `fn_reconcile_dispatch_outcome`'s dependency on the provider adapter actually supporting reference echo-back — not verified for Exotel specifically in this or any prior pass, stated plainly rather than claimed away.

**Verification status, stated plainly:** the EDB full PostgreSQL 16 installer genuinely failed in this environment ("the requested operation requires elevation" — disclosed, not smoothed over); the binaries-only distribution was used instead, `pgvector` was built from source against it via the locally available MSVC toolchain, and all three required extensions were confirmed loadable before any migration ran. Fresh-database and incremental `alembic upgrade` both passed (exit code 0) on this genuinely separate PostgreSQL 16.10 instance; every scenario named above was exercised as a real elapsed-time lease expiry or a genuine multi-connection race, not simulated or narrated. Full transcripts: `docs/phase-05-database-design/5K/execution_logs/README.md`'s "Sixth batch" and the validation reports it links.

**Full DDL and rationale:** `099_5C1.sql`'s own §-labeled header comment; `docs/phase-05-database-design/5K/MIGRATION_MANIFEST.md`'s "Phase 6H Final Blocker Remediation" entry; `docs/phase-06-api-design/6H-Campaign-APIs.md` (Revision 4).

---

## Amendment — Phase 6H Final Micro-Remediation (2026-08-28) — reconciliation authorization boundary

The prior pass's `voice.fn_reconcile_dispatch_outcome()` — the one function that can convert an `AMBIGUOUS`/`SUBMITTING` dispatch into `FAILED`, which immediately re-opens eligibility for a fresh physical provider attempt — granted `EXECUTE` to `app_api` and `app_worker`: the two broad, shared-by-everything application roles. This meant any ordinary application or worker code path could unilaterally re-authorize a second physical telephony attempt for an ambiguous call, which is precisely the class of defect this whole remediation exists to prevent, merely relocated into the reconciliation function itself. This is a **physical-redial safety authorization boundary**, not an ordinary state update, and is treated as such below.

**Authorization model.** A new PostgreSQL role, `voice.app_voice_reconciler`-equivalent (created as `app_voice_reconciler`, schema-agnostic per this catalog's convention), is `LOGIN`, **not** `BYPASSRLS`, holds no table DML anywhere, and is granted exactly `USAGE` on schema `voice` plus `EXECUTE` on this one function. The existing five-role catalog (`001_5B.sql`) was inspected first and found to have no role narrow enough for this specific capability — `app_api`/`app_worker` are too broad, `app_readonly` cannot write, and `app_platform_admin` (the existing break-glass/operator role, `087_5B1.sql`) is correct for the human/operator reconciliation path but far broader than a single automated function needs for the provider-callback/provider-lookup path. `EXECUTE` on `fn_reconcile_dispatch_outcome()` is now granted only to `app_voice_reconciler` and `app_platform_admin`; **revoked from `app_api` and `app_worker`.**

**Provenance.** A new required parameter, `p_reconciliation_source` (`'PROVIDER_CALLBACK' | 'PROVIDER_LOOKUP' | 'OPERATOR'`), persists to a new column, `voice.call_dispatch_keys.reconciliation_source`, alongside the existing `reconciled_by`/`reconciled_at` — an all-or-nothing `CHECK` constraint ties all three together. This is evidentiary metadata only, never authorization: neither `p_reconciliation_source` nor `p_reconciled_by` is ever read in any conditional that decides anything inside the function — authorization is entirely a function of which PostgreSQL role holds `EXECUTE`. **Live-proven:** an `app_api` call forging `p_reconciled_by = 'admin'` still fails with the identical `permission denied` error, before the function body ever runs.

**FAILED requires evidence.** A `FAILED` reconciliation now requires a non-empty `p_note` (evidence description), enforced both in the function body (`RAISE EXCEPTION`) and by a new table `CHECK` constraint (`chk_cdk_reconciled_failed_has_evidence`, defense in depth against a future direct-UPDATE regression). This cannot verify the evidence is *true* — no database can; that is the job of the `EXECUTE`-privilege boundary above — it only makes "FAILED with zero stated reason" structurally impossible. **Live-proven:** both an empty-string and a `NULL` `p_note` are rejected identically.

**Permitted state transitions, re-verified.** Only `SUBMITTING` and `AMBIGUOUS` are valid source states — unchanged from the prior pass, and re-confirmed live rather than merely re-read from the SQL: an authorized-role attempt to reconcile an already-`CONFIRMED` row returns `reconciled=false`, and the row remains `CONFIRMED` — no path can reopen a known-accepted physical call for redial.

**Audit.** A durable `VOICE_DISPATCH_RECONCILED` event is now written synchronously, in the same transaction, via `audit.fn_insert_audit_event()` (5J §14.2's sole legal write path to `audit.audit_events`) for every successful reconciliation — closing the prior complete absence of audit evidence for this safety-critical transition. `actor_type` is `PLATFORM_ADMIN` for an `OPERATOR`-sourced reconciliation and `WORKER` for either provider-driven source; `resource_snapshot` carries the dispatch key, old/new state, and reconciliation source only — no phone number or other PII (`voice.call_dispatch_keys` itself stores none). `action_kind` required no schema change (`audit.audit_events.action_kind` is length-`CHECK`-only, per every prior phase's own precedent for adding a value).

**Live-validated on a genuinely fresh, independent PostgreSQL 16.10 instance** (the prior pass's own instance had already been torn down): direct execution as `app_api`/`app_worker` both denied; the authorized role successfully resolves both `AMBIGUOUS → CONFIRMED` and `AMBIGUOUS → FAILED` (with the `FAILED` row then genuinely re-claimable — retry really is reopened); both no-evidence `FAILED` attempts rejected; a `CONFIRMED`-row reconciliation attempt refused; a cross-tenant attempt refused non-disclosingly with no mutation; provenance and the audit event both confirmed present and accurate; the full prior-pass regression suite re-run unchanged. Full transcripts: `docs/phase-05-database-design/5K/execution_logs/README.md`'s "Seventh batch"; `docs/phase-05-database-design/5K/validation/VOICE_DISPATCH_VALIDATION_REPORT.md`'s addendum.

**Full DDL and rationale:** `099_5C1.sql`'s `fn_reconcile_dispatch_outcome()` header comment and §B1 (the new role); `docs/phase-05-database-design/5K/MIGRATION_MANIFEST.md`'s "Phase 6H Final Micro-Remediation" entry; `docs/phase-06-api-design/6H-Campaign-APIs.md` (Revision 5).

---

## Amendment — Phase 6H Final Micro-Fix (2026-08-28) — non-forgeable reconciliation provenance

The prior amendment's `voice.fn_reconcile_dispatch_outcome()` correctly restricted *who* could reconcile (`app_voice_reconciler`/`app_platform_admin` only) but still let *either* authorized caller freely choose *which* provenance category to record, via a plain `p_reconciliation_source` parameter accepted by the single function. This meant the automated reconciliation credential could falsely record itself as an operator decision (`p_reconciliation_source = 'OPERATOR'`), or the operator credential could falsely claim provider provenance (`'PROVIDER_CALLBACK'`) — an audit-integrity defect for a decision this safety-critical: the audit trail for a physical-redial authorization must truthfully identify which trusted path actually made it.

**Split into three functions, replacing the single one:**

1. **`voice.fn_reconcile_dispatch_outcome_internal()`** — the actual state-transition and audit-writing mechanism, unchanged in logic from the prior single function, but granted `EXECUTE` to **no role at all**. It is reachable only via the two wrapper functions below, invoking it under their own `SECURITY DEFINER` owner privileges — the identical bridge-function pattern already used for `fn_new_uuid_v7()` (§A of `099_5C1.sql`), where a mechanism function is never directly callable by any application role.
2. **`voice.fn_reconcile_dispatch_from_provider()`** — `EXECUTE`: `app_voice_reconciler` only. Its `p_provider_source` parameter is restricted, by a `CHECK` inside the function body, to `PROVIDER_CALLBACK` or `PROVIDER_LOOKUP` — `'OPERATOR'` is rejected with an exception even when the caller genuinely holds `EXECUTE`, because the *value* is illegal for this function, not because the *caller* lacks privilege. Hardcodes `actor_type = 'WORKER'` for the resulting audit event.
3. **`voice.fn_reconcile_dispatch_by_operator()`** — `EXECUTE`: `app_platform_admin` only (the existing break-glass/operator role — no second new role introduced). Takes **no source parameter at all**: `'OPERATOR'` and `actor_type = 'PLATFORM_ADMIN'` are hardcoded inside the function body, so there is no way for a caller to request provider-sourced provenance through this path, even in principle.

This makes the provenance category a given credential can produce a property of the database schema itself — enforced by which function a `GRANT` exists on, and by what that function's own body will and will not accept — rather than a property of caller-supplied metadata or application-layer trust that a bug could get wrong.

**Live-validated on a third, genuinely independent PostgreSQL 16.10 instance** (the prior two amendments' own instances had each already been torn down): fresh-database and incremental `alembic upgrade` both exit code 0, single head `099_5C1` unchanged. **The critical forgery test:** `app_voice_reconciler`, while genuinely holding `EXECUTE` on `fn_reconcile_dispatch_from_provider`, calling it with `p_provider_source = 'OPERATOR'` was rejected by the function's own internal `CHECK` — not by a missing grant. `app_voice_reconciler` attempting `fn_reconcile_dispatch_by_operator` at all, and `app_platform_admin` attempting `fn_reconcile_dispatch_from_provider` at all, were both denied at the privilege layer (`permission denied`) — neither role holds a grant on the other's function. Genuine reconciliation through each path recorded the correct, function-determined `reconciliation_source`/`actor_type`, confirmed by direct query against both `voice.call_dispatch_keys` and `audit.audit_events`. `CONFIRMED → FAILED` and cross-tenant reconciliation were both re-attempted through *both* new functions and refused every time. The operator path's `FAILED`-requires-evidence rule was confirmed identical to the provider path's (both route through the same shared internal check). The full prior-pass regression suite was re-run and reproduced unchanged. Full transcripts: `docs/phase-05-database-design/5K/execution_logs/README.md`'s "Eighth batch"; `docs/phase-05-database-design/5K/validation/VOICE_DISPATCH_VALIDATION_REPORT.md`'s second addendum.

**Full DDL and rationale:** `099_5C1.sql`'s `fn_reconcile_dispatch_outcome_internal()`/`fn_reconcile_dispatch_from_provider()`/`fn_reconcile_dispatch_by_operator()` header comments; `docs/phase-05-database-design/5K/MIGRATION_MANIFEST.md`'s "Phase 6H Final Micro-Fix" entry; `docs/phase-06-api-design/6H-Campaign-APIs.md` (Revision 6).

---

## Amendment — Phase 6H Final Admin-DML Hardening (2026-08-29) — removing the platform-admin direct DML bypass

`voice.call_dispatch_keys` is **read-only to platform-admin runtime credentials; all safety-critical mutations occur through guarded Voice functions.** Every prior privilege pass restricted a *different* role — `app_api`/`app_worker`'s direct `INSERT` (Blocker C), then `EXECUTE` on the reconciliation functions for `app_api`/`app_worker` (Revision 5), then the provenance split itself (Revision 6) — but `app_platform_admin`'s own original `GRANT SELECT, INSERT, UPDATE, DELETE`, present since this table was first created, was never touched. That grant was the one remaining path that could bypass every invariant built on top of it: a direct `UPDATE ... SET dispatch_state = 'FAILED' WHERE dispatch_state = 'CONFIRMED'` would reopen a known-accepted call for a second physical telephony attempt, and a direct `UPDATE ... SET reconciliation_source = 'PROVIDER_CALLBACK'` would forge the provenance boundary Revision 6 just established — both entirely invisible to, and unenforced by, any guarded function, since neither statement ever calls one.

**The fix:** `app_platform_admin`'s `INSERT`/`UPDATE`/`DELETE` grant on `voice.call_dispatch_keys` is removed. `SELECT` is retained — an explicit, still-legitimate diagnostics/support need, not removed. `fn_reconcile_dispatch_by_operator()` requires no direct table grant to keep writing: it is `SECURITY DEFINER`, owned by the migration-running role, identical in this respect to every other guarded function in this schema (`fn_enqueue_contact()`'s own zero-direct-grant operation for `app_worker` is the established precedent). The trusted operator path remains exactly:

```
authenticated platform admin -> controlled application action -> fn_reconcile_dispatch_by_operator()
```

never:

```
platform admin -> arbitrary UPDATE call_dispatch_keys
```

The identical fix was applied to `campaign.campaign_contact_identities` (5E's own companion table) — see `5E-Campaign-Schema.md`'s matching amendment.

**Live-validated on a fourth, genuinely independent PostgreSQL 16.10 instance** (the three prior amendments' own instances had each already been torn down): fresh-database and incremental `alembic upgrade` both exit code 0, single head `099_5C1` unchanged. Direct catalog inspection (`information_schema.role_table_grants`) confirms `app_platform_admin` holds `SELECT` only, before any test runs. `app_platform_admin` is denied `permission denied` on direct `INSERT`, `UPDATE` (including the specific provenance-forgery attempt and the specific `CONFIRMED → FAILED` reopen attempt), and `DELETE`; a `SELECT` against a live `CONFIRMED` row succeeds. The legitimate guarded paths are unaffected: `fn_reconcile_dispatch_by_operator()` on a genuine `AMBIGUOUS` row with real evidence succeeds, recording correct provenance; the same function against an already-`CONFIRMED` row correctly refuses; `fn_reconcile_dispatch_from_provider()` (`app_voice_reconciler`) succeeds identically. `app_api`/`app_worker`/`app_voice_reconciler` remain denied on direct DML, unchanged. The full prior-pass regression suite was re-run and reproduced unchanged. Full transcripts: `docs/phase-05-database-design/5K/execution_logs/README.md`'s "Ninth batch"; `docs/phase-05-database-design/5K/validation/VOICE_DISPATCH_VALIDATION_REPORT.md`'s third addendum.
