...
A subsequent, independent adversarial freeze review of the amendment above found one further genuine defect, closed in the same `098_5E1.sql` file (still never applied to any production database, so still corrected in place rather than superseded by `100_5E2.sql`): `app_worker` held a direct `INSERT` grant on `campaign.campaign_contact_identities` alongside the intended guarded path through `campaign.fn_enqueue_contact()`. Because this table's `PRIMARY KEY (campaign_id, contact_id)` is the *entire* uniqueness guarantee the enclosing invariant depends on, a caller using the direct grant instead of the guarded function could create an orphan identity row with no corresponding `campaign_contacts` row — permanently blocking every future legitimate enqueue attempt for that pair, since `ON CONFLICT DO NOTHING` would always lose to the orphan. The grant is now `SELECT`-only for `app_worker`/`app_api`/`app_readonly`; the guarded function, being `SECURITY DEFINER` and owned by the migration-running role, needs no such grant to keep writing. **Live-proven on PostgreSQL 16** (the declared production baseline — the two prior 6H passes had validated only against PostgreSQL 18): direct `INSERT` as `app_worker` now fails with `permission denied for table campaign_contact_identities`; `fn_enqueue_contact()` and its pre-existing cross-tenant guard remain fully functional, unaffected by the privilege change. Full evidence: `docs/phase-05-database-design/5K/validation/CAMPAIGN_PRIVILEGE_VALIDATION_REPORT.md` and `docs/phase-05-database-design/5K/validation/PG16_MIGRATION_VALIDATION_REPORT.md`.

**Full DDL and rationale:** `098_5E1.sql`'s own §-labeled header comment (the privilege note is inline above the `GRANT` statement); `docs/phase-05-database-design/5K/MIGRATION_MANIFEST.md`'s "Phase 6H Final Blocker Remediation" entry; `docs/phase-06-api-design/6H-Campaign-APIs.md` (Revision 4).

---

## Amendment — Phase 6H Final Admin-DML Hardening (2026-08-29)

A final review found that the fix above never touched `app_platform_admin`'s own direct `INSERT, UPDATE, DELETE` grant on `campaign.campaign_contact_identities`, present since the table was first created. This table's entire purpose is a `PRIMARY KEY`-backed uniqueness claim for `campaign.fn_enqueue_contact()`'s own atomic claim-then-insert; there is no documented admin workflow anywhere in this document or 6H that writes to it directly, and doing so risks the identical orphan-identity hazard described above. `app_platform_admin`'s `INSERT`/`UPDATE`/`DELETE` is now also removed; `SELECT` is retained. **Live-proven on a fourth, independent PostgreSQL 16.10 instance:** direct `INSERT` as `app_platform_admin` now fails with `permission denied for table campaign_contact_identities`, identically to `app_worker`'s own prior denial; `fn_enqueue_contact()` remains fully functional (it needs no direct grant, being `SECURITY DEFINER`). Full evidence: `docs/phase-05-database-design/5K/validation/CAMPAIGN_PRIVILEGE_VALIDATION_REPORT.md`'s addendum.

**Full DDL and rationale:** `098_5E1.sql`'s own §-labeled header comment; `docs/phase-05-database-design/5K/MIGRATION_MANIFEST.md`'s "Phase 6H Final Admin-DML Hardening" entry; `docs/phase-06-api-design/6H-Campaign-APIs.md` (Revision 7).
