...
| 101 | 5I.1 | `101_5I1.sql` | `100_5G1` | transactional | 36717 | `4f706e556f3b0bbe80efb02c682cffba6f6619b8ebc08f0933cb8b7e54cf77c6` |

---

## Phase 6J Remediation Pass (2026-08-29) — `101_5I1.sql` added, NOT LIVE-VALIDATED (disclosed)

Row 101 (`101_5I1.sql`, Alembic revision `101_5I1`, `down_revision = "100_5G1"`) is a new, additive-only forward migration closing P0/P1 findings from the independent-review remediation pass against `docs/phase-06-api-design/6J-Integrations-Webhooks-Plugins-APIs.md` (2026-08-29). No row 001-100 is edited, renumbered, or reordered.

**What 101_5I1 adds** (schemas `integrations`, `plugins`, `webhooks` only — no other schema touched):

- 2 new nullable columns on `integrations.oauth_attempts` (`connection_id`, `failure_reason`) + 1 length CHECK + 1 partial index.
- 2 new nullable columns on `webhooks.webhook_endpoints` (`previous_signing_secret_ref`, `previous_secret_expires_at`) + 2 CHECKs.
- 9 new `SECURITY DEFINER` functions in `integrations` (`fn_activate_integration_connection`, `fn_fail_integration_connection`, `fn_degrade_integration_connection`, `fn_disconnect_integration_connection`, `fn_update_integration_connection_config`, `fn_record_integration_sync_result`, `fn_redeem_oauth_callback_state`, `fn_fail_oauth_callback_state`) closing DEP-6J-01 (P0) and the OAuth-callback tenant-bootstrap defect (P0).
- 4 new `SECURITY DEFINER` functions in `plugins` (`fn_suspend_plugin_installation`, `fn_reactivate_plugin_installation`, `fn_update_plugin_installation_config`, `fn_rotate_plugin_installation_credential`) closing DEP-6J-02 (P0).
- 1 new `SECURITY DEFINER` function in `webhooks` (`fn_rotate_webhook_secret`) closing the webhook dual-signature rotation defect (P0).
- 1 `CREATE OR REPLACE` on the pre-existing `integrations.fn_create_integration_connection` (061_5I) — same signature, body extended with an `integration_definitions.is_active` check (closes DEP-6J-09). PostgreSQL preserves the function's existing grants across `CREATE OR REPLACE`; no `REVOKE`/`GRANT` statement was needed or added for this one function.
- 5 `GRANT EXECUTE ... TO app_api` statements widening pre-existing (059-066) `app_worker`-only functions (`plugins.fn_activate_plugin`, `plugins.fn_uninstall_plugin`, `plugins.fn_upgrade_plugin`, `integrations.fn_rotate_integration_credential`, `webhooks.fn_replay_webhook_delivery`) — no function body touched, grants only. This removed the need for the synchronous internal-RPC pattern the pre-remediation 6J document had proposed (its own ADR-6J-01), since every one of these five functions already performs its own `organization_id`-scoped `SELECT ... FOR UPDATE` authorization internally.

**Why the OAuth fix did not require removing RLS from `oauth_attempts`:** `integrations.oauth_attempts` and `integrations.integration_connections` both keep `ENABLE + FORCE ROW LEVEL SECURITY` unchanged. Every `SECURITY DEFINER` function in this schema (059-066 and this amendment alike) executes under its **owning role's** privileges for the full duration of the call — and `app_migration` (the role migrations are applied as) was created `BYPASSRLS` in `001_5B.sql` and independently reconfirmed still-`BYPASSRLS` in the `077_5J1` live validation pass (`validation/077_5J1_VALIDATION_REPORT.md` line 309: *"`BYPASSRLS` role list unchanged: `app_migration`, `app_platform_admin` (+ the connecting superuser)"*). RLS was therefore never the actual obstacle inside a `SECURITY DEFINER` function body — the pre-remediation defect was purely that `fn_redeem_oauth_attempt`'s signature required `organization_id` as an **input** the unauthenticated browser-callback caller cannot supply. `fn_redeem_oauth_callback_state(state)` fixes this by taking `state` alone (itself the security boundary — 256-bit, `UNIQUE`, single-use, 10-minute TTL, exactly like a password-reset token) and returning `organization_id` as **output**. `fn_redeem_oauth_attempt` (059-066) is untouched and still exists; it is simply not the function 6J's callback endpoint calls.

**Live-database validation status — DISCLOSED LIMITATION:** unlike every prior amendment in this manifest (077, 078-100), **101_5I1 was NOT executed against a live PostgreSQL instance in the session that authored it.** A locally-installed PostgreSQL 18 Windows service was present but no superuser credentials were available to the authoring session, and an isolated throwaway data directory (`initdb` + a non-default port, deliberately never touching the existing service, to avoid any risk to it) failed to start its backend processes with a Windows-specific shared-memory reservation error (`could not reserve shared memory region ... error code 487`), a known class of issue in sandboxed process environments — not a defect in the SQL itself, but a genuine environment limitation of the authoring session. **What WAS done instead:** every statement in `101_5I1.sql` was manually traced against the byte-for-byte contents of the actual applied `059_5I.sql`-`066_5I.sql`/`001_5B.sql` files (re-read directly from disk in this same pass, not from the 6J document's prior paraphrase of them) — table/column names, CHECK constraint names, existing grant baselines, and the `app_migration`/`app_platform_admin` `BYPASSRLS` fact were all confirmed against primary source (migration files and the `077_5J1` validation report) before being relied upon. No claim of "fresh-database `upgrade head` PASS" is made anywhere for this row, in contrast to every other amendment row above — **this is an explicit, disclosed gap**, not an oversight: `101_5I1.sql` requires a genuine `alembic upgrade head` run (fresh DB, `001_5B → … → 101_5I1`) plus the adversarial security test matrix in `6J-Integrations-Webhooks-Plugins-APIs.md` §26 before it can be represented as live-validated, matching the bar every other row in this manifest was held to.
