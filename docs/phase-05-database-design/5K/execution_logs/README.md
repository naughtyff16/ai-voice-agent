...
| `09_schema_tables_full.txt` | query against `information_schema.tables` / `pg_class` joined with `pg_namespace`, all 16 schemas | Full table inventory post-upgrade: 199 rows (includes partition parent + child tables). Per-schema breakdown (199 across 16 schemas), plus FK/PK/unique/check/index/view/matview/function/trigger/enum/sequence/RLS-table/policy counts (50/197/94/2,541/821/2/0/219/105/0/0/91/103 respectively) and the 4 installed extensions (`pgcrypto`, `vector`, `pg_stat_statements`, `plpgsql`) were captured in companion single-query files `02,03,06,07,08,10-23` at the time of this batch; those 19 files were removed in the repository cleanup pass (2026-09-06) as pure single-COUNT captures whose results are quoted verbatim in this row and independently re-verified batch-by-batch through the rest of this directory (notably file `38`'s reconciliation below and the Sixth/Seventh/Eighth/Ninth-batch full structural re-audits) — see `../../../REPOSITORY_CLEANUP_MANIFEST.md`. |
...
one active installation (fixture-setup script and load log, formerly
`scripts/..._fixtures_2.sql` and log `26`/`31`, removed in the repository
cleanup pass — see `../../../REPOSITORY_CLEANUP_MANIFEST.md`); Org B
(`bbbb...`, pre-existing) gets one workflow definition + published version 1
(fixture-setup script and load log, formerly `scripts/..._fixtures_orgb.sql`
and log `33`, likewise removed), used only for the row-7 cross-tenant race.
...
| ~~`36_security_row5_worker_row13_readonly.txt`~~ | — | **Removed in the repository cleanup pass (2026-09-06).** Was a self-described superseded initial attempt at §18 row 5 using `SET ROLE app_worker` from the `postgres` connection, which failed for a benign, already-understood reason (not a feature defect): `audit.fn_insert_audit_event`'s platform-event check deliberately reads `session_user`, not `current_user`, specifically so `SET ROLE` cannot spoof it (see the function's own inline comment) — `SET ROLE` changes `current_user`, not `session_user`, so that attempt exercised the wrong role. Its corrected replacement, `36b` below, is the file of record; see `../../../REPOSITORY_CLEANUP_MANIFEST.md`. |
...
*The `68_fixture_setup.sql` fixture script and `70_reserve_dispatch_tests.sql`
regression script (both inputs, not results — their outcomes are captured in
the results files `69` and `71` below) were removed in the repository
cleanup pass; see `../../../REPOSITORY_CLEANUP_MANIFEST.md`.*
...
| File | Command | Purpose |
|---|---|---|
| `81_final_reconciliation_privilege_and_provenance_output.txt` | ~20 sequential `psql` statements, `SET ROLE app_api` / `SET ROLE app_worker` / `SET ROLE app_voice_reconciler` for genuine role-boundary enforcement | The core test suite for this pass — see the Results summary below. |
| `83_final_regression_output.txt` | Re-run of the sixth batch's own crash-recovery and idempotency scenarios | Confirms this pass's changes (entirely inside `fn_reconcile_dispatch_outcome()` plus one new role) did not regress anything the sixth batch already proved. |

*The setup/gate/command inputs for this batch — `74_final_pg16_fresh_upgrade.txt`,
`75_final_alembic_heads.txt`, `76_final_alembic_current.txt`,
`77_final_pg16_incremental_upgrade_to_097.txt`,
...
and `82_final_regression_tests.sql` (the fresh/incremental-chain gate and the
fixture/test/regression command scripts, as distinct from their `81`/`83`
result transcripts retained above) — were removed in the repository cleanup
pass; see `../../../REPOSITORY_CLEANUP_MANIFEST.md`.*
...
| File | Command | Purpose |
|---|---|---|
| `91_final_provenance_output.txt` | ~30 sequential `psql` statements, `SET ROLE app_api` / `app_worker` / `app_voice_reconciler` / `app_platform_admin` for genuine role-boundary and forgery-attempt testing | The core test suite for this pass — see the Results summary below. |
| `93_final_regression_output.txt` | Re-run of the sixth/seventh batches' own crash-recovery and idempotency scenarios | Confirms this pass's changes (splitting one function into three, no change to any other function) did not regress anything previously proven. |

*The setup/gate/command inputs for this batch — `84_final_pg16_fresh_upgrade.txt`,
`85_final_alembic_heads.txt`, `86_final_alembic_current.txt`,
`87_final_pg16_incremental_upgrade_to_097.txt`,
`88_final_pg16_incremental_upgrade_097_to_head.txt`,
`89_final_provenance_fixture_setup.sql`, `90_final_provenance_tests.sql`, and
`92_final_regression_tests.sql` (the fresh/incremental-chain gate and the
fixture/test/regression command scripts, as distinct from their `91`/`93`
result transcripts retained above) — were removed in the repository cleanup
pass; see `../../../REPOSITORY_CLEANUP_MANIFEST.md`.*
